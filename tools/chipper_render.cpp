...
struct Options
{
    ...
    std::array<bool, 4> opmOperatorDt1Provided {};
    std::array<int, 4> opmOperatorDt2 { 0, 0, 0, 0 };
    std::array<bool, 4> opmOperatorDt2Provided {};
...
bool opmOperatorDetuneIndexForArg(const std::string& arg, std::string_view field, size_t& index)
{
    static constexpr auto prefix = "--opm-op";
    if (arg.rfind(prefix, 0) != 0)
        return false;
...
bool fmOperatorAttackRateIndexForArg(const std::string& arg, size_t& index)
{
    ...
    for (const auto* prefix : prefixes)
    {
        ...
        const auto suffix = arg.substr(std::strlen(prefix));
        const auto marker = suffix.find("-attack-rate");
        if (marker == std::string::npos || marker == 0 || marker + std::strlen("-attack-rate") != suffix.size())
            return false;
...
bool fmOperatorDecayRateIndexForArg(const std::string& arg, size_t& index)
{
    ...
    for (const auto* prefix : prefixes)
    {
        ...
        const auto suffix = arg.substr(std::strlen(prefix));
        const auto marker = suffix.find("-decay-rate");
        if (marker == std::string::npos || marker == 0 || marker + std::strlen("-decay-rate") != suffix.size())
            return false;
...
bool parseOpmOperatorDetuneChoice(const std::string& text, int maxNativeValue, int& out)
{
    const auto key = normalizedToken(text);
    if (key == "follow" || key == "preset" || key == "macro" || key == "auto" || key == "default" || key == "f")
    {
    ...
    int nativeValue = 0;
    if (! parseNumber(key, nativeValue) || nativeValue < 0 || nativeValue > maxNativeValue)
        return false;
...
bool parseFmOperatorDecayRateChoice(const std::string& text, int& out)
{
    const auto key = normalizedToken(text);
    if (key == "follow" || key == "preset" || key == "macro" || key == "auto" || key == "default" || key == "f")
    {
...
bool parseFmFeedbackChoice(const std::string& text, float& out)
{
    auto key = normalizedToken(text);
    if (key.rfind("fb", 0) == 0)
        key.erase(0, 2);
    ...
    int feedback = 0;
    if (! parseNumber(key, feedback) || feedback < 0 || feedback > 7)
        return false;

    out = static_cast<float>(feedback) / 7.0f;
    return true;
}

bool parseOpmLfoWaveformChoice(const std::string& text, int& out)
{
    const auto key = normalizedToken(text);
    if (key == "preset" || key == "follow" || key == "auto" || key == "default")
    {
        out = 0;
        return true;
    }
    if (key == "saw" || key == "sawtooth")
        out = 1;
    else if (key == "square")
        out = 2;
...
bool parseOpmLfoSensitivityChoice(const std::string& text, int maximum, int& out)
{
    const auto key = normalizedToken(text);
    if (key == "preset" || key == "follow" || key == "auto" || key == "default")
    {
    ...
    int sensitivity = 0;
    if (! parseNumber(key, sensitivity) || sensitivity < 0 || sensitivity > maximum)
        return false;
    out = sensitivity + 1;
    return true;
}
bool parseWaveShape(const std::string& text, int& out)
{
    ...
    const auto key = normalizedToken(text);
    if (key == "ram" || key == "manual" || key == "trace" || key == "macro" || key == "follow" || key == "auto" || key == "default" || key == "custom" || key == "user" || key == "userpatch" || key == "opllcustom")
        out = 0;
    else if (key == "tri" || key == "triangle")
        out = 1;
...
bool parseDmgStereoRoute(const std::string& text, int& out)
{
    ...
    if (key == "macro" || key == "follow" || key == "auto" || key == "default")
        ...
        out = 3;
             ...
             || key == "alt" || key == "alternate" || key == "alternating")
        out = 4;
    else
        return false;
...
bool parseOplRhythmMode(const std::string& text, int& out)
{
    ...
    if (key == "macro" || key == "follow" || key == "auto" || key == "default")
        ...
        out = 3;
    else if (key == "4op" || key == "4operator" || key == "fourop" || key == "fouroperator" || key == "4oppair" || key == "4operatorpair" || key == "fouropair" || key == "fouroperatorpair" || key == "pair" || key == "pairs")
        out = 4;
    else
        return false;
...
bool parseOpnSsgEnvelopeMode(const std::string& text, int& out)
{
    uint32_t numeric = 0;
    if (parseNumber(text, numeric))
    {
    ...
    const auto key = normalizedToken(text);
    if (key == "macro" || key == "follow" || key == "auto" || key == "default" || key == "preset")
        out = 0;
    else if (key == "fall" || key == "down" || key == "decay")
        out = 1;
    else if (key == "rise" || key == "up" || key == "attack")
        out = 2;
    else if (key == "saw" || key == "sawdown" || key == "repeat")
        out = 3;
    else if (key == "tri" || key == "triangle")
        out = 4;
    else
...
bool parseArgs(int argc, char** argv, Options& options)
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            const auto* value = requireValue("--opna-rhythm-rom");
            if (value == nullptr)
                return false;
            options.opnaRhythmRomPath = value;
            continue;
        }

        if (arg == "--opna-rhythm-rom-hex")
        {
            const auto* value = requireValue("--opna-rhythm-rom-hex");
            if (value == nullptr)
                return false;
            options.opnaRhythmRomHex = value;
            continue;
        }

        if (arg == "--opna-adpcm-b-sample")
        {
            const auto* value = requireValue("--opna-adpcm-b-sample");
            if (value == nullptr)
                return false;
            options.opnaAdpcmBSamplePath = value;
            continue;
        }

        if (arg == "--opna-adpcm-b-hex")
        {
            const auto* value = requireValue("--opna-adpcm-b-hex");
            if (value == nullptr)
                return false;
            options.opnaAdpcmBHex = value;
            continue;
        }

        if (arg == "--opnb-adpcm-a-sample")
        {
            const auto* value = requireValue("--opnb-adpcm-a-sample");
            if (value == nullptr)
                return false;
            options.opnbAdpcmASamplePath = value;
            continue;
        }

        if (arg == "--opnb-adpcm-a-hex")
        {
            const auto* value = requireValue("--opnb-adpcm-a-hex");
            if (value == nullptr)
                return false;
            options.opnbAdpcmAHex = value;
            continue;
        }

        if (arg == "--opnb-adpcm-b-sample")
        {
            const auto* value = requireValue("--opnb-adpcm-b-sample");
            if (value == nullptr)
                return false;
            options.opnbAdpcmBSamplePath = value;
            continue;
        }

        if (arg == "--opnb-adpcm-b-hex")
        {
            const auto* value = requireValue("--opnb-adpcm-b-hex");
            if (value == nullptr)
                return false;
            options.opnbAdpcmBHex = value;
            continue;
        }
        ...
        if (size_t fmOperatorIndex = 0; fmOperatorAttackRateIndexForArg(arg, fmOperatorIndex))
        {
            const auto* value = requireValue(arg.c_str());
            int parsed = 0;
            if (value == nullptr || ! parseFmOperatorAttackRateChoice(std::string(value), parsed))
                return false;
            options.fmOperatorAttackRates[fmOperatorIndex] = std::clamp(parsed, 0, 32);
            options.fmOperatorAttackRateProvided[fmOperatorIndex] = true;
            continue;
        }

        if (size_t fmOperatorIndex = 0; fmOperatorDecayRateIndexForArg(arg, fmOperatorIndex))
        {
            const auto* value = requireValue(arg.c_str());
            int parsed = 0;
            if (value == nullptr || ! parseFmOperatorDecayRateChoice(std::string(value), parsed))
                return false;
        ...
        if (size_t opmOperatorIndex = 0; opmOperatorDetuneIndexForArg(arg, "-dt1", opmOperatorIndex))
        {
            const auto* value = requireValue(arg.c_str());
            int parsed = 0;
            if (value == nullptr || ! parseOpmOperatorDetuneChoice(std::string(value), 7, parsed))
                return false;
            options.opmOperatorDt1[opmOperatorIndex] = parsed;
            options.opmOperatorDt1Provided[opmOperatorIndex] = true;
            continue;
        }

        if (size_t opmOperatorIndex = 0; opmOperatorDetuneIndexForArg(arg, "-dt2", opmOperatorIndex))
        {
            const auto* value = requireValue(arg.c_str());
            int parsed = 0;
            if (value == nullptr || ! parseOpmOperatorDetuneChoice(std::string(value), 3, parsed))
                return false;
            options.opmOperatorDt2[opmOperatorIndex] = parsed;
            options.opmOperatorDt2Provided[opmOperatorIndex] = true;
            continue;
        }

        if (arg == "--opm-lfo-waveform" || arg == "--ym2151-lfo-waveform")
        {
            const auto* value = requireValue(arg.c_str());
            if (value == nullptr || ! parseOpmLfoWaveformChoice(std::string(value), options.opmLfoWaveform))
                return false;
            continue;
        }

        if (arg == "--opm-pms" || arg == "--ym2151-pms")
        {
            const auto* value = requireValue(arg.c_str());
            if (value == nullptr || ! parseOpmLfoSensitivityChoice(std::string(value), 7, options.opmLfoPms))
                return false;
            continue;
        }

        if (arg == "--opm-ams" || arg == "--ym2151-ams")
        {
            const auto* value = requireValue(arg.c_str());
            if (value == nullptr || ! parseOpmLfoSensitivityChoice(std::string(value), 3, options.opmLfoAms))
                return false;
            continue;
        }
        ...
        else if (arg == "--stereo-spread" || arg == "--opm-lfo-depth" || arg == "--ym2151-lfo-depth" || arg == "--opm-lfo" || arg == "--opn2-lfo-depth" || arg == "--ym2612-lfo-depth" || arg == "--opn2-lfo")
        {
            const auto* value = requireValue(arg.c_str());
        ...
        else if (arg == "--paula-sample")
        {
            ...
            if (value == nullptr)
                return false;
            options.paulaSamplePaths.emplace_back(value);
        }
        ...
        else if (arg == "--dmg-stereo-route" || arg == "--opl-route" || arg == "--opl3-route")
        {
            const auto* value = requireValue(arg.c_str());
        ...
        else if (arg == "--ym-channel-a-mix" || arg == "--opn-ssg-a-mix" || arg == "--opn-channel-a-mix")
        {
            const auto* value = requireValue(arg.c_str());
        ...
        else if (arg == "--ym-channel-b-mix" || arg == "--opn-ssg-b-mix" || arg == "--opn-channel-b-mix")
        {
            const auto* value = requireValue(arg.c_str());
        ...
        else if (arg == "--ym-channel-c-mix" || arg == "--opn-ssg-c-mix" || arg == "--opn-channel-c-mix")
        {
            const auto* value = requireValue(arg.c_str());
        ...
        else if (arg == "--sn-noise-mode" || arg == "--opn-ssg-envelope" || arg == "--opn-ssg-env" || arg == "--opn-ssg-shape")
        {
            const auto* value = requireValue(arg.c_str());
            const auto parsed = value != nullptr
                && (arg == "--sn-noise-mode"
                        ? parseSnNoiseMode(std::string(value), options.snNoiseMode)
                        : parseOpnSsgEnvelopeMode(std::string(value), options.snNoiseMode));
            if (! parsed)
                return false;
            options.snNoiseModeProvided = true;
        }
...
void applyMacroTemplateDefaults(Options& options)
{
    ...
    for (size_t i = 0; i < controls.size(); ++i)
    {
        ...
        if (! options.opmOperatorDt1Provided[i])
            options.opmOperatorDt1[i] = 0;
        if (! options.opmOperatorDt2Provided[i])
            options.opmOperatorDt2[i] = 0;
        if (! options.fmOperatorAttackRateProvided[i])
...
const char* toJsonString(chipper::ChipParameterRole role)
{
    switch (role)
    {
        ...
        case chipper::ChipParameterRole::fmOperator1AttackRate: return "fmOperator1AttackRate";
        case chipper::ChipParameterRole::fmOperator2AttackRate: return "fmOperator2AttackRate";
        case chipper::ChipParameterRole::fmOperator3AttackRate: return "fmOperator3AttackRate";
        case chipper::ChipParameterRole::fmOperator4AttackRate: return "fmOperator4AttackRate";
        case chipper::ChipParameterRole::fmOperator1DecayRate: return "fmOperator1DecayRate";
        ...
        case chipper::ChipParameterRole::opmOperator1Dt1: return "opmOperator1Dt1";
        case chipper::ChipParameterRole::opmOperator2Dt1: return "opmOperator2Dt1";
        case chipper::ChipParameterRole::opmOperator3Dt1: return "opmOperator3Dt1";
        case chipper::ChipParameterRole::opmOperator4Dt1: return "opmOperator4Dt1";
        case chipper::ChipParameterRole::opmOperator1Dt2: return "opmOperator1Dt2";
        case chipper::ChipParameterRole::opmOperator2Dt2: return "opmOperator2Dt2";
        case chipper::ChipParameterRole::opmOperator3Dt2: return "opmOperator3Dt2";
        case chipper::ChipParameterRole::opmOperator4Dt2: return "opmOperator4Dt2";
        case chipper::ChipParameterRole::opmLfoWaveform: return "opmLfoWaveform";
        case chipper::ChipParameterRole::opmLfoPms: return "opmLfoPms";
        case chipper::ChipParameterRole::opmLfoAms: return "opmLfoAms";
...
const char* chipModeKey(chipper::ChipMode mode)
{
    switch (mode)
    {
        ...
        case chipper::ChipMode::nesVrc6: return "nesVrc6";
        ...
        case chipper::ChipMode::zxSpectrumBeeper: return "zxSpectrumBeeper";
    }
                    ...
                    size_t noteEventCount)
{
    ...
    out << ",\n"
        ...
        << "  \"opmOperatorDt1\": [" << patch.opmOperatorDt1[0] << ", "
        << patch.opmOperatorDt1[1] << ", "
        << patch.opmOperatorDt1[2] << ", "
        << patch.opmOperatorDt1[3] << "],\n"
        << "  \"opmOperatorDt2\": [" << patch.opmOperatorDt2[0] << ", "
        << patch.opmOperatorDt2[1] << ", "
        << patch.opmOperatorDt2[2] << ", "
        << patch.opmOperatorDt2[3] << "],\n"
        << "  \"opmOperatorDt1Provided\": [" << options.opmOperatorDt1Provided[0] << ", " << options.opmOperatorDt1Provided[1] << ", " << options.opmOperatorDt1Provided[2] << ", " << options.opmOperatorDt1Provided[3] << "],\n"
        << "  \"opmOperatorDt2Provided\": [" << options.opmOperatorDt2Provided[0] << ", " << options.opmOperatorDt2Provided[1] << ", " << options.opmOperatorDt2Provided[2] << ", " << options.opmOperatorDt2Provided[3] << "],\n"
        << "  \"fmOperatorAttackRates\": [" << patch.fmOperatorAttackRates[0] << ", "
        << patch.fmOperatorAttackRates[1] << ", "
        << patch.fmOperatorAttackRates[2] << ", "
        << patch.fmOperatorAttackRates[3] << "],\n"
        << "  \"fmOperatorDecayRates\": [" << patch.fmOperatorDecayRates[0] << ", "
        ...
        << "  \"opmLfoWaveform\": " << patch.opmLfoWaveform << ",\n"
        << "  \"opmLfoPms\": " << patch.opmLfoPms << ",\n"
        << "  \"opmLfoAms\": " << patch.opmLfoAms << ",\n"
    ...
    out << ",\n"
        << "  \"opnaAdpcmBSamplePath\": ";
    writeJsonString(out, options.opnaAdpcmBSamplePath.string());
    out << ",\n"
        << "  \"opnbAdpcmASamplePath\": ";
    writeJsonString(out, options.opnbAdpcmASamplePath.string());
    out << ",\n"
        << "  \"opnbAdpcmBSamplePath\": ";
    writeJsonString(out, options.opnbAdpcmBSamplePath.string());
    out << ",\n"
...
int main(int argc, char** argv)
{
    try
    {
        ...
        if (! options.opnaRhythmRomPath.empty() && ! options.opnaRhythmRomHex.empty())
            throw std::runtime_error("Use either --opna-rhythm-rom or --opna-rhythm-rom-hex, not both");
        if ((! options.opnaAdpcmBSamplePath.empty() || ! options.opnaAdpcmBHex.empty()) && options.chip != chipper::ChipMode::ym2608)
            throw std::runtime_error("--opna-adpcm-b-sample is only valid with --chip ym2608");
        ...
        if (! options.opnaRhythmRomPath.empty())
            ...
            core->setExternalSampleData(parseHexBytes(options.opnaRhythmRomHex));
        if (! options.opnaAdpcmBSamplePath.empty())
            core->setExternalAdpcmBData(loadBinaryFile(options.opnaAdpcmBSamplePath));
        else if (! options.opnaAdpcmBHex.empty())
            core->setExternalAdpcmBData(parseHexBytes(options.opnaAdpcmBHex));
        if (! options.opnbAdpcmASamplePath.empty())
            core->setExternalSampleData(loadBinaryFile(options.opnbAdpcmASamplePath));
        else if (! options.opnbAdpcmAHex.empty())
            core->setExternalSampleData(parseHexBytes(options.opnbAdpcmAHex));
        if (! options.opnbAdpcmBSamplePath.empty())
            core->setExternalAdpcmBData(loadBinaryFile(options.opnbAdpcmBSamplePath));
        else if (! options.opnbAdpcmBHex.empty())
            core->setExternalAdpcmBData(parseHexBytes(options.opnbAdpcmBHex));
        if (! options.paulaSamplePaths.empty())
        {
            std::vector<chipper::ExternalSampleData> bank;
            bank.reserve(options.paulaSamplePaths.size());
            for (const auto& path : options.paulaSamplePaths)
                bank.push_back(loadPaulaSampleFile(path));
        ...
        auto patch = chipper::makePatchConfig(options.chip,
                                                    options.macro,
                                                    options.control1,
                                                    options.control2,
                                                    options.control3,
                                                    ...
                                                    {},
                                                    options.opmLfoWaveform,
                                                    options.opmLfoPms,
                                                    options.opmLfoAms);
        patch.opmOperatorDt1 = options.opmOperatorDt1;
        patch.opmOperatorDt2 = options.opmOperatorDt2;
        core->setPatch(patch);
        ...
        const auto hasAudibleDmcDirect = (options.chip == chipper::ChipMode::nes
            ...
            || options.chip == chipper::ChipMode::nesMmc5)
