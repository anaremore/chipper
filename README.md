...
- DAW project saves carry bounded, checksummed fallback copies of user-owned sample assets, including each indexed OPNA/OPNB ADPCM-A region, so moved DMC/SPC700/Paula/OPN2/OPNA/OPNB sources can be relinked without losing the saved sound; shareable `.chipperpreset` files remain reference-only.
...
| YM2612 / Genesis FM | Verified-partial BSD-3-Clause `ymfm` OPN2 adapter with native LFO Depth, Operator Tone DT1, and channel-6 DAC playback from distinct generated recipes or VST/renderer-loaded WAV, AIFF, and raw unsigned bytes. Schema 9 preserves rate/root/trim/center-or-hold state and portable/embedded recall; deterministic gates cover write cadence and preset layering, while independent oracle/hardware and analog-ladder validation remain planned |
...
| YM2608 / OPNA | Verified-partial BSD-3-Clause `ymfm` OPNA adapter with six FM lanes, three embedded SSG lanes, an 8 KiB generated rhythm ROM, and a six-card ADPCM-A editor for the fixed BD/SD/Top Cym/HH/Tom/Rim hardware windows. Each card accepts an exact-size encoded region or deterministically converts WAV/AIFF at its native `/432` or `/864` rate, previews decoded audio, and can reset to generated Chipper data. Legacy packed-ROM import remains supported; starting region editing from one requires confirmation and replaces it only after the chosen region validates. Independent YM2608-LLE gates cover bounded ADPCM-A BD/Tom behavior; broader ADPCM-A traces, ADPCM-B, analog, and hardware validation remain planned |
| YM2610 / OPNB | Verified-partial BSD-3-Clause `ymfm` OPNB adapter with four FM lanes, three embedded SSG lanes, and a six-card logical ADPCM-A editor. Each region accepts 256-byte-page-aligned encoded data or deterministic WAV/AIFF conversion at 18.519 kHz; populated regions pack consecutively into the 1 MiB window while empty logical slots keep their identity. Legacy packed-bank import and the raw-or-converted ADPCM-B layer remain supported; starting region editing from a packed bank requires confirmation and commits only after validation. Independent YM2608-LLE gates cover bounded ADPCM-A addressing, routing, and decode behavior; broader traces, ADPCM-B, analog, and hardware validation remain planned |
| YM2610B / OPNB2 | Verified-partial BSD-3-Clause `ymfm` OPNB2 adapter using the YM2610 six-FM channel mask, three embedded SSG lanes, the same six-logical-region ADPCM-A editor/packer as OPNB, and raw or WAV/AIFF-converted ADPCM-B memory; legacy packed-bank import remains supported; independent YM2608-LLE gates cover bounded ADPCM-A behavior, while broader traces, ADPCM-B, analog, and hardware validation remain planned |
| OPL2 / OPL3 | Verified-partial BSD-3-Clause `ymfm` YMF262/OPL3 adapter with a unified nine-lane topology surface for native rhythm, paired 18-channel layers, and `$104` 4-op pairs; the shared OP1-OP4 matrix exposes all four algorithms plus native multiplier, total-level, and AR/DR/SL/RR fields. Per-lane patches, OPL LFO/flag depth, dedicated 18-card editing, and stronger reference validation remain planned |
