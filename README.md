...
The command-line renderer exposes the same NES Pulse 2 Duty override as `--nes-pulse2-duty macro|12.5|25|50|75` and the same YM channel mixer overrides as `--ym-channel-a-mix`, `--ym-channel-b-mix`, and `--ym-channel-c-mix`. Debug JSON reports the stored choices, resolved register values, and per-channel tone/noise enables so UI, MIDI, renderer, and engine behavior can be tested against one contract.
