...
Factory presets can pin SID filter-mode combinations, filter routing, per-voice waveforms, and per-voice ADSR nibble choices, so choosing a preset updates the visible SID controls to match the rendered patch rather than hiding those decisions inside the engine.
