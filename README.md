...
YM2413 / OPLL preset FM is backed by vendored MIT `emu2413` for a verified-partial melodic adapter. Instrument Bias is on CC76, Pitch Spread is on CC77, Motion is on CC78, Channel Volume is on CC79, and explicit Instrument choice is on CC89 with Follow plus Bell, Bass, Lead, and Organ musical groups mapped to YM2413 ROM preset instruments. The YM2413 UI presents Instrument as a dropdown so the selected or template-resolved `$30` instrument and volume nibble are readable; rhythm mode, VRC7/YMF281 patch sets, and custom Voice 0 editing remain planned.
