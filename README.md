...
Noise Mode is chip-aware: NES/RP2A03 resolves Follow, Long LFSR, or Short LFSR into `$400E`; Game Boy/DMG resolves Follow, 15-bit LFSR, or 7-bit LFSR into `NR43`; and SN76489 resolves Follow or PSG noise-control choices into its noise register. NES Pulse 1 Duty stays on Native Control 1 / CC76, while NES Pulse 2 Duty is a dedicated register choice on CC104: Follow preserves the musical template, and explicit choices write pulse channel 2 duty bits independently. NES DMC Direct Level is on CC116 and maps to the 7-bit `$4011` DAC load for direct grit; full DPCM sample playback remains a documented gap. DMG Wave Level is on CC92 and maps to `NR32` channel-3 output level choices: Follow, Mute, 100%, 50%, and 25%. DMG Stereo Routing is also on CC94 in DMG mode and maps to `NR51` choices: Follow, Both, Left, Right, and Split.
