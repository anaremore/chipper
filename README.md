...
SPC700 BRR import supports a user-loaded `.brr` file or folder bank in the plugin UI. The first 32 readable BRR files become dropdown slots, the selected slot is recalled by host state as file paths, and CC117 selects the active BRR slot in SNES mode. Sample bytes are not embedded in the project or committed to the repo, and WAV-to-BRR conversion remains planned.
