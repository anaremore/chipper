...
| 89 | Wave Shape |
| 90 | YM Envelope Shape / SID Filter Mode / SPC700 Envelope / OPN2 Envelope Shape |
...
SPC700 sample-bank status now includes an ARAM budget cue for the checked playable slots. The UI reports the active bank payload against a 64 KB SNES audio-RAM world and warns when the bank is near or over that ceiling before driver code, echo buffers, sequence data, and sample directory overhead. This is a practical composition warning, not an exact cartridge memory allocator yet.
...
SPC700 Envelope Shape is on CC90 and appears in the Envelope section as a chip-aware segmented control. Follow resolves from the selected SPC700 template, while Pluck, Lead, Pad, and Perc write distinct clean-room ADSR/gain-style contours and matching debug metadata. This makes imported BRR/WAV/AIFF sample banks immediately playable as keys, pads, leads, and percussion without claiming exact S-DSP envelope timing.
