...
- Wavetable source decks should use a tight control-derived budget: header reserve plus compact card rows. HuC6280, Namco WSG, and SCC need enough room for waveform preview, a standard-height wave selector, and a visible Level lane, but should not stretch each source card into tall empty panels. Current default target is intentionally close to the visible controls, not a proportional fill of the whole editor.
