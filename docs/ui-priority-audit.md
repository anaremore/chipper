...
- The editor canvas is capped to stay DAW-friendly, with compact chip pages using the conservative default height and sample-heavy pages using a modest bounded maximum. Chipper should favor readable channel-local controls and waveform/sample previews, but chip-specific layouts must stay inside the 680 px maximum rather than expanding the plugin off-screen in high-DPI DAW windows. Sampler modes are allowed a slightly taller fixed surface because Paula and SPC700 need visible per-channel sample selectors plus a real sample-bank waveform surface without colliding with Performance Macros.
...
- The same editor-size smoke test cycles through every exposed chip mode before forcing an oversized host-restore, so chip-specific layouts cannot quietly reintroduce an off-screen VST after a mode switch. Most generator/FM/wavetable pages must clamp back to 620 px tall even if the host restores a taller window; NES, SPC700, and Paula may use 680 px because DMC/sample-bank waveform editors are primary controls.
...
- NES / RP2A03 is the DAW-fit guardrail for sample-heavy chip pages: DMC needs a real waveform preview, but the editor must still open inside a normal host window. Keep the NES channel row, DMC sample row, and Performance Macros bounded together instead of letting the DMC preview or macro strip expand the full plugin off-screen. NES uses a compact two-row Performance Macro layout so APU Decay and Output sit side-by-side instead of stacking into the full editor height.
