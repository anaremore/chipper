...
- Wavetable source decks should use a tight control-derived budget: header reserve plus compact card rows. HuC6280, Namco WSG, and SCC need enough room for waveform preview, a standard-height wave selector, and a visible Level lane, but should not stretch each source card into tall empty panels.
...
- Wavetable-family source decks should stay compact enough to scan. HuC6280, Namco WSG, and SCC prefer a one-row lane deck at the default editor size so wave selectors and level controls remain visible without each lane becoming a tall empty card. Grow only when new visible per-lane controls require it.
- SPC700 and Paula sample-bank panels now require enough height before switching into the two-column editor layout. Pitch, playback, root, loop, and file/folder/bank controls keep standard control height; if the sample bank is too short, the layout falls back instead of letting controls cover the waveform or Performance Macros. Current guardrails: SPC700 needs at least 360 px of sample-bank height for two-column layout, Paula needs at least 300 px.
- Paula sample-bank layout has priority over macro-strip height. The four channel cards still need readable per-channel shape/sample/level controls, but the shared sample-bank waveform/editor row must stay tall enough to avoid collisions with Performance Macros; shrink the macro strip or rebalance channel-card height before shrinking the sample editor.
- Paula sampler layout should balance three visible layers without overlap: per-channel sample cards, the shared sample-bank waveform/editor, and compact Performance Macros. The sample bank is the protected editor; macros are secondary.
- Paula uses a compact Performance Macros strip so the shared Sample Bank editor remains the layout driver. If future Paula macros need more room, grow the editor or add a dedicated secondary view instead of taking height back from file/folder/bank controls or the waveform preview.
