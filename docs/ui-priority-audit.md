...
- The editor canvas is capped to stay DAW-friendly. Every chip page, including NES / RP2A03, SPC700, and Paula, must open at the normal 880 px default height with an 820 px minimum clamp: channel rows, sample waveform previews, and Performance Macros must rebalance inside that height instead of expanding the plugin off-screen or crushing controls into unreadable strips. If a sampler needs more detail later, prefer an explicit secondary view or user resize path rather than a taller launch height.
- The editor resize limit must also advertise the same 880 px maximum height. Hosts should not be given a taller default resize ceiling that can reopen Chipper off-screen even if `resized()` later clamps the content.
- The editor timer also re-applies the current chip's width/height clamp after host restore. This is intentional defensive behavior for DAWs that reopen a plugin at a stale oversized window rectangle before the normal resize path settles.
- Editor size is now a smoke-test contract, not just a visual preference. `chipper_editor_size_smoke` checks the default editor dimensions, verifies that oversized host-restored bounds are clamped back to the DAW-friendly maximum, and walks visible child geometry so dropdowns/buttons do not silently shrink below readable size or drift outside the editor bounds.
- The same editor-size smoke test cycles through every exposed chip mode before forcing an oversized host-restore, so chip-specific layouts cannot quietly reintroduce an off-screen VST after a mode switch. All chip pages must clamp back to 880 px tall even if the host restores a taller window; sample-heavy pages need to protect their waveform/editor controls by rebalancing content, not by making the plugin taller by default.
...
- NES / RP2A03 is the DAW-fit guardrail for sample-heavy chip pages: DMC needs a real waveform preview, but the editor must still open at the 880 px default inside a normal host window. Keep the NES channel row, DMC sample row, and Performance Macros bounded together instead of letting the DMC preview or macro strip expand the full plugin off-screen. NES uses a compact stacked Performance Macro layout: first-row macros, a dedicated APU Decay row, then Output/scope. APU Decay and Output must never share a cramped horizontal row or collapse into wire-like slivers.
