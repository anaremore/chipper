...
- The editor canvas is capped to stay DAW-friendly. Every chip page, including NES / RP2A03, SPC700, and Paula, must open at the normal 800 px default height with a 700 px minimum clamp: channel rows, sample waveform previews, and Performance Macros must rebalance inside that height instead of expanding the plugin off-screen or crushing controls into unreadable strips. If a sampler needs more detail later, prefer an explicit secondary view or user resize path rather than a taller launch height.
- The editor resize limit must also advertise the same 800 px maximum height. Hosts should not be given a taller default resize ceiling that can reopen Chipper off-screen even if `resized()` later clamps the content.
...
- NES / RP2A03 is the DAW-fit guardrail for sample-heavy chip pages: DMC needs a real waveform preview, but the editor must still open at the 800 px default inside a normal host window. Keep the NES channel row, DMC sample row, and Performance Macros bounded together instead of letting the DMC preview or macro strip expand the full plugin off-screen. NES uses a compact two-row Performance Macro layout with enough row height that APU Decay, Output, and the first-row macros remain readable rather than collapsing into wire-like slivers.
