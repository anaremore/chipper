...
- NES / RP2A03 is the DAW-fit guardrail for sample-heavy chip pages: DMC needs a real waveform preview, but the editor should launch compactly enough for normal host windows inside the 820 px cap. Keep the NES channel row, DMC sample row, and Performance Macros bounded together instead of letting the DMC preview or macro strip expand the full plugin off-screen. NES uses a compact stacked Performance Macro layout: first-row macros, a dedicated APU Decay row, then Output/scope. The macro row needs enough height for labels/readouts, while APU Decay and Output must remain separate rows and never collapse into wire-like slivers.
- NES / RP2A03 DMC layout is now covered by editor-size smoke checks: playback mode, sample slot, root note, loop toggle, rate selector, waveform preview, APU Decay, Output, and the output scope must remain readable, owned by their panel, and non-overlapping inside the 1240 x 820 editor cap.
...
- YM2149/AY Tone/Noise Mix is a segmented hardware mixer choice, not a slider. Its Performance Macro slot must reserve a dedicated label, segmented choice row, and resolved register readout so Noise/Tone/Both remains readable at the fixed 1240 x 820 editor size.
