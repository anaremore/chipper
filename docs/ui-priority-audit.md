...
- Current wavetable source cards use a compact cap close to the real control stack. Do not re-expand them just to fill the row; only add height when a new visible per-lane control is added and the Level lane would otherwise be hidden.
