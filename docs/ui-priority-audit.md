...
- The editor canvas is capped to stay DAW-friendly. Chip pages launch at the shared 1240 x 820 default and must rebalance inside that height instead of expanding the plugin off-screen or crushing controls into unreadable strips. The default and minimum clamp are intentionally the same for now so hosts do not restore a half-cut layout. If a sampler needs more detail later, prefer an explicit secondary view or user resize path rather than a taller launch height.
- The editor resize limit must advertise the same 820 px maximum height. Hosts should not be given a taller default resize ceiling that can reopen Chipper off-screen.
...
- The same editor-size smoke test cycles through every exposed chip mode before forcing an oversized host-restore, so chip-specific layouts cannot quietly reintroduce an off-screen VST after a mode switch. All chip pages must clamp back to 820 px tall if the host restores a taller window; sample-heavy pages need to protect their waveform/editor controls by rebalancing content, not by making the plugin taller by default.
