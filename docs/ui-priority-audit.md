...
- The editor canvas is capped to stay DAW-friendly, with compact chip pages using the conservative default height and sample-heavy pages using the bounded maximum height. Chipper should favor readable channel-local controls and waveform/sample previews, but chip-specific layouts must stay inside the 720 px maximum rather than expanding the plugin off-screen in high-DPI DAW windows. Sampler modes are allowed a slightly taller fixed surface because Paula and SPC700 need visible per-channel sample selectors plus a real sample-bank waveform surface without colliding with Performance Macros.
- Editor size is now a smoke-test contract, not just a visual preference. `chipper_editor_size_smoke` checks the default editor dimensions and verifies that oversized host-restored bounds are clamped back to the DAW-friendly maximum before layout work continues.
- The same editor-size smoke test cycles through every exposed chip mode before forcing an oversized host-restore, so chip-specific layouts cannot quietly reintroduce an off-screen VST after a mode switch. NES and most generator/FM/wavetable pages must clamp back to 660 px tall even if the host restores a taller window; SPC700 and Paula may use 720 px because their sample-bank waveform editors are primary controls.
