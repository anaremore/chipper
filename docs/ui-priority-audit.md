...
- Wavetable and sampler channel cards should stack controls in the same order users hear them: enable/header, waveform preview, per-voice wave or sample selector, then level. Do not strand level sliders at the bottom of tall cards where they look missing.
...
- Namco WSG and Konami SCC now have explicit helper-envelope modules instead of letting the decay helper appear under Motion/Wave copy. This keeps their no-native-ADSR status visible without hiding the musical amp-envelope control from users.
