...
- The editor canvas is capped to stay DAW-friendly. Chip pages may launch at chip-specific compact heights, but the user can resize up to the normal 880 px maximum and the minimum clamp stays readable at 720 px. Channel rows, sample waveform previews, and Performance Macros must rebalance inside those limits instead of expanding the plugin off-screen or crushing controls into unreadable strips. If a sampler needs more detail later, prefer an explicit secondary view or user resize path rather than a taller launch height.
