...
- HuC6280, Namco WSG, and Konami SCC now expose the shared helper envelope as a visible `Global Amp Env` module with enough height for a normal slider/readout instead of a dead-looking title-only placeholder. The source cards remain the home for per-channel shape/sample/level editing, but the cards should stay compact: show preview, waveform selector, and level trim without stretching the whole editor vertically. Future per-channel gate helpers should move into those cards if the engine supports them.
