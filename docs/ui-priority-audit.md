...
- visible dropdowns remain at least 48x24 px and visible buttons at least 24x18 px in `chipper_editor_size_smoke`
...
- The editor canvas is capped to stay DAW-friendly. Every chip page, including NES / RP2A03, SPC700, and Paula, must open at the normal 720 px default height: channel rows, sample waveform previews, and Performance Macros must rebalance inside that height instead of expanding the plugin off-screen. If a sampler needs more detail later, prefer an explicit secondary view or user resize path rather than a taller launch height.
- The editor resize limit must also advertise the same 720 px maximum height. Hosts should not be given a taller default resize ceiling that can reopen Chipper off-screen even if `resized()` later clamps the content.
...
- NES / RP2A03 is the DAW-fit guardrail for sample-heavy chip pages: DMC needs a real waveform preview, but the editor must still open at the 720 px default inside a normal host window. Keep the NES channel row, DMC sample row, and Performance Macros bounded together instead of letting the DMC preview or macro strip expand the full plugin off-screen. NES uses a compact two-row Performance Macro layout so APU Decay and Output sit side-by-side instead of stacking into the full editor height.
