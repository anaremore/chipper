...
- The editor canvas is capped to stay DAW-friendly. Chip pages launch at chip-specific heights: deep SID and sampler pages may use the normal 880 px height, while compact PSG/FM/wavetable pages launch at 820 px and still resize up to 880 px. The minimum clamp stays readable at 720 px. Channel rows, sample waveform previews, and Performance Macros must rebalance inside those limits instead of expanding the plugin off-screen or crushing controls into unreadable strips. If a sampler needs more detail later, prefer an explicit secondary view or user resize path rather than a taller launch height.
