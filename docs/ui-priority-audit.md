...
- NES / RP2A03 is the DAW-fit guardrail for sample-heavy chip pages: DMC needs a real waveform preview, but the editor must still open at the 880 px default inside a normal host window. Keep the NES channel row, DMC sample row, and Performance Macros bounded together instead of letting the DMC preview or macro strip expand the full plugin off-screen. NES uses a compact two-row Performance Macro layout with enough row height that APU Decay, Output, and the first-row macros remain readable rather than collapsing into wire-like slivers.
