...
Review status: synced on 2026-06-14. The audit now treats FM held-tail and NES DMC loop-off as protected release gates, while the active UI work focuses on chip-owned controls, readable source cards, sample/wave/operator editors, and screenshot regressions.
...
- Paula's macro strip is a protected secondary workspace: it receives enough height for readable controls, but the four channel cards and Sample Bank waveform remain the primary layout drivers.
- Paula's sampler layout now treats Performance Macros as a reserved strip instead of leftover space. The Sample Bank preview is capped inside its module, standard-height sample controls stay in the left column, and the macro strip receives enough height to avoid overlapping the file/import editor.
