...
- The editor canvas is capped to stay DAW-friendly. Chip pages launch at the shared 1240 x 880 default and must rebalance inside that height instead of expanding the plugin off-screen or crushing controls into unreadable strips. The minimum clamp stays readable at 720 px. If a sampler needs more detail later, prefer an explicit secondary view or user resize path rather than a taller launch height.
- The editor resize limit must advertise the same 880 px maximum height. Hosts should not be given a taller default resize ceiling that can reopen Chipper off-screen.
...
- NES / RP2A03 is the DAW-fit guardrail for sample-heavy chip pages: DMC needs a real waveform preview, but the editor should launch compactly enough for normal host windows and still resize up to 880 px when the user wants inspection room. Keep the NES channel row, DMC sample row, and Performance Macros bounded together instead of letting the DMC preview or macro strip expand the full plugin off-screen. NES uses a compact stacked Performance Macro layout: first-row macros, a dedicated APU Decay row, then Output/scope. APU Decay and Output must never share a cramped horizontal row or collapse into wire-like slivers.
