...
- Wavetable-family source decks should stay compact enough to scan. HuC6280, Namco WSG, and SCC now cap the two-row source deck height so wave selectors and level lanes stay readable without each lane becoming a tall empty card. Grow only when new visible per-lane controls require it.
