...
Review status: synced on 2026-06-20 after chip-aware editor heights, chip-owned control placement, sampler/wavetable source-card guards, and chip-switch state preservation were rechecked. The focused release gate passed 26/26 on 2026-06-14, so this audit treats FM held-tail, NES DMC loop-off, and helper-envelope honesty as protected release gates. Active UI work focuses on richer chip editors, preset/state workflows, sample/wave/operator depth, and any current screenshot regression that still reproduces in the installed build. If a fixed audio issue or misleading label is suspected again, verify the gate first; do not turn it into a UI todo unless the current screen gives users a misleading or inaccessible control path.
...
- SPC700/SNES voice cards must reserve a readable level lane after the sample dropdown. Each S-DSP voice is a playable source with its own output level, so hidden, clipped, or detached level sliders are usability regressions.
...
- Chipper helper envelopes must name the underlying chip path when practical and must be honest about scope. POKEY should say **AUDV Gate** and Paula should say **Tracker Amp Env**. HuC6280, SCC, and Namco WSG currently expose one shared musical volume helper, so the visible control says **Shared Amp Env** while per-channel shape/sample/level controls stay in the source cards. Avoid generic ADSR/Decay wording unless the chip really exposes that behavior.
...
- HuC6280, Namco WSG, and Konami SCC now expose the shared helper envelope as a visible `Shared Amp Env` module with enough height for a normal slider/readout instead of a dead-looking title-only placeholder. The source cards remain the home for per-channel shape/sample/level editing, but the cards should stay compact: show preview, waveform selector, and level trim without stretching the whole editor vertically. Future per-channel gate helpers should move into those cards if the engine supports them.
...
- Wavetable decks should use a scannable four-column card grid at the default width. HuC6280, Namco WSG, and SCC all keep per-lane waveform preview, wave selector, and Level lane in the source cards without letting individual waveforms become oversized horizontal strips.
- Wavetable decks should not spend extra vertical budget on empty card bodies. HuC6280, Namco WSG, and SCC source rows now cap close to the actual control stack: header, waveform, standard wave selector, and a 36 px embedded Level lane. The cap must still be tall enough for a normal selector row and level lane; otherwise controls become technically present but unreadable. Extra height should go to meaningful editors or macros, not blank space inside source cards.
- The editor canvas is capped to stay DAW-friendly. Most chip pages launch at the shared 1240 x 860 default and must rebalance inside that height instead of expanding the plugin off-screen or crushing controls into unreadable strips. SID is the current fixed-height exception at 1240 x 880 because per-voice ADSR is central and otherwise collides with Performance Macros. If another chip needs more detail later, prefer an explicit secondary view or a documented fixed-height exception with smoke coverage.
...
- Current wavetable source cards use a compact cap close to the real control stack. Do not re-expand them just to fill the row; only add height when a new visible per-lane control is added and the Level lane would otherwise be hidden. HuC6280, Namco WSG, and SCC cards should stay close to selector-plus-level height rather than becoming blank two-row slabs.
...
- SPC700 and Paula sample-bank panels require enough height before switching into the two-column editor layout. Pitch, playback, root, loop, and file/folder/bank controls keep standard control height; if the sample bank is too short, the layout must compact or fall back instead of letting controls cover the waveform or Performance Macros. These sampler pages currently open inside the shared 1240 x 860 DAW-friendly cap, so their layouts should protect the shared sample-bank waveform while keeping source cards and macros concise.
...
- HuC6280, Namco WSG, and SCC use a compact source-grid plus side-by-side helper/output row at the default editor size. This keeps per-lane waveform selectors and level lanes visible while avoiding the oversized blank cards that made the editor feel too tall.
...
- PSG, FM, POKEY, and wavetable pages should keep Performance Macros compact but stacked. YM2149/AY, SN76489, YM2612, YM2151, OPL, OPLL, POKEY, HuC6280, Namco WSG, and SCC must not squeeze long register hints into the same header row as slider labels; compact macro cells reserve separate rows for label, slider, and resolved register readout before any chip-owned source/operator panel is compressed. The editor-size smoke test now checks that these macro readouts remain visible below their sliders.
...
1. Preset browser and preset sharing
   - Issue: the top dropdown now groups the current chip's presets by category, includes a chip-local Init Patch, and can filter factory sounds by role. The next browser pass should add search text, engine/tag filters, favorites, and richer user-preset affordances. User presets should remain simple flat files that are easy to share.
