...
- The editor default canvas is capped to stay DAW-friendly, with a conservative default height and a bounded maximum height. Chipper should favor readable channel-local controls and waveform/sample previews, but chip-specific layouts must first rebalance sections inside the default viewport rather than expanding the plugin off-screen. Sampler modes are still layout drivers because Paula and SPC700 need visible per-channel sample selectors plus a real sample-bank waveform surface without colliding with Performance Macros.
...
- The same editor-size smoke test cycles through every exposed chip mode before forcing an oversized host-restore, so chip-specific layouts cannot quietly reintroduce an off-screen VST after a mode switch.
