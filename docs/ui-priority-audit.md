...
- The editor canvas is capped to stay DAW-friendly, with compact chip pages using the conservative default height and sample-heavy pages using the bounded maximum height. Chipper should favor readable channel-local controls and waveform/sample previews, but chip-specific layouts must stay inside the 820 px maximum rather than expanding the plugin off-screen. Sampler modes are allowed a taller fixed surface because Paula and SPC700 need visible per-channel sample selectors plus a real sample-bank waveform surface without colliding with Performance Macros.
