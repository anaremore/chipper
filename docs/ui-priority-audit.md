...
- SPC700/SNES reserves enough vertical space for three different concepts to remain separately readable: the top sample/envelope controls, the eight per-voice sample cards, and the shared sample-bank waveform editor. The Loop While Held setting is a full-row toggle, not a dropdown-style row, because it is a binary playback-lifetime choice.
...
- HuC6280, Namco WSG, and Konami SCC now expose the shared helper envelope as a visible Performance Macro slider/readout instead of a dead standalone module. The source cards remain the home for per-channel shape/sample/level editing; future per-channel gate helpers should move into those cards if the engine supports them.
...
- Paula now reserves a larger protected Sample Bank row and a slightly slimmer Performance Macro strip. The sample-bank waveform, playback controls, and per-channel sample cards take priority because Paula is fundamentally a four-channel sample instrument; macro controls should never visually overlap the shared sample editor.
