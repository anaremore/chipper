...
- Paula channel cards place the generated shape and sample-bank selector side by side at standard dropdown height where space allows. This keeps both per-channel sample decisions readable without pushing the protected bottom level lane out of view.
...
- Wavetable-family source decks should stay compact enough to scan. HuC6280, Namco WSG, and SCC cap the two-row source deck height and use a compact embedded level lane so wave selectors and level controls stay readable without each lane becoming a tall empty card. Grow only when new visible per-lane controls require it.
