...
- The editor canvas is capped to stay DAW-friendly. NES / RP2A03 is part of the normal 620 px default-height contract even with DMC sample controls: the channel row, DMC waveform preview, and Performance Macros must rebalance inside that height instead of expanding the plugin off-screen. Dedicated sampler modes may still use a bounded 680 px surface because Paula and SPC700 need visible per-channel sample selectors plus a real sample-bank waveform surface without colliding with Performance Macros.
...
- The same editor-size smoke test cycles through every exposed chip mode before forcing an oversized host-restore, so chip-specific layouts cannot quietly reintroduce an off-screen VST after a mode switch. Generator/FM/wavetable pages and NES must clamp back to 620 px tall even if the host restores a taller window; only SPC700 and Paula currently use 680 px because their sample-bank waveform editors are primary controls.
