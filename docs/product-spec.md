...
- Paula sample banks accept WAV, AIFF, and uncompressed IFF/8SVX files. The Paula UI treats all four DAC channels as independent sample voices: generated sample-shape controls and optional loaded sample-slot pins live in each channel card, while the shared Sample Bank section handles external file/folder/bank loading, note-map/manual playback, and waveform preview. Follow Bank uses the global manual slot or note map; slot pins let channels 1-4 keep different loaded samples at the same time. The renderer exposes the same generated-shape idea through `--paula-shape1..4` for regression coverage. Compressed 8SVX, MOD sample extraction, loop-point import, per-channel renderer sample-slot flags, and tracker effect metadata remain planned.
