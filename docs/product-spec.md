...
- **YM2608/OPNA:** six-channel FM plus three embedded SSG tone/noise/envelope lanes backed by `ymfm`, shared OPN-family controls, generated or user-owned encoded ADPCM-A rhythm banks, and one ADPCM-B layer. The VST preserves raw encoded bytes or deterministically downmixes/resamples WAV/AIFF to 5200 Hz Yamaha ADPCM-B, aligns memory to four bytes, previews decoded audio, and retains bounded encoded project fallback. Six-region ADPCM-A editing and broader validation remain follow-ups.
- **YM2610/OPNB:** four-channel FM plus three embedded SSG lanes backed by `ymfm`, shared OPN-family controls, a user-packed six-region encoded ADPCM-A bank, and one ADPCM-B layer. The VST preserves raw encoded bytes or converts WAV/AIFF to 5208.333 Hz Yamaha ADPCM-B with 256-byte alignment, decoded preview, and bounded encoded project fallback. ADPCM-A bank editing and stronger validation remain follow-ups.
- **YM2610B/OPNB2:** six-channel FM plus three embedded SSG lanes backed by the `ymfm` YM2610 six-FM channel mask, the same explicit six-region ADPCM-A bank contract, and the same raw or WAV/AIFF-converted ADPCM-B workflow. ADPCM-A bank editing and stronger validation remain follow-ups.
...
- Non-parameter Motion Lab project state: per-chip enable, 1-8 step length, 1/8-1/64 rate, eight -24..+24 semitone offsets, eight 0-15 post-chip levels, and Hold/Trig/Cut gates. These are not APVTS automation slots; the engine publishes edits lock-free and saves them in current schema-v7 project/preset state.
...
- DAW project state keeps external paths authoritative but also carries bounded, checksummed fallback bytes for user-owned NES DMC, SPC700, Paula, OPN2, OPNA, and OPNB sample assets. If a source moves, the project copy restores encoding, bank order, loop/source metadata, and a visible relink status; corrupt bank entries remain silent tombstones so mapped notes never shift. Flat `.chipperpreset` files stay reference-only. Exact payload limits are part of [the compatibility contract](compatibility-contract.md).
