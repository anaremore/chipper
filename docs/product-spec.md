...
Review status: synced on 2026-07-27 after the shared Wave Lab, global Motion Lab, and shared OPL3 four-operator editor landed with versioned state, exact native-register evidence, and two-width visual coverage. FM held-note fade-out, NES DMC one-shot loop-off, state compatibility, dense MIDI handling, editor geometry, and motion real-time behavior are release-gate concerns rather than open feature requests unless reproduced. Current product evolution should prioritize deeper sample/FM authenticity, chip-native motion/SFX destinations, preset quality, and final release confidence.
