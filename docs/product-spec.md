...
Review status: synced on 2026-07-27 after the shared Wave Lab and global Motion Lab landed with versioned non-parameter project state. FM held-note fade-out, NES DMC one-shot loop-off, state compatibility, dense MIDI handling, editor geometry, and motion real-time behavior are release-gate concerns rather than open feature requests unless reproduced. Current product evolution should prioritize the complete OPL3 four-operator editor, deeper sample/FM authenticity, preset quality, and final release confidence.
...
- **OPL2/OPL3:** two-operator FM, rhythm mode, waveform variants, envelopes, OPL3 stereo output, explicit paired 18-channel Layer mode, and first-pass `$104` 4-op Pair mode.
