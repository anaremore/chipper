...
| SNES SPC700-style | Lo-fi sample playback | Clean-room partial sample-voice model; Game_Music_Emu, Furnace, and MAME remain reference or audit targets only | Internal; no SPC700, S-DSP, BRR, SNES, or tracker-player source code is vendored | Partial core with eight generated lo-fi sample voices, simplified pitch/volume/ADSR/gain/key-on state, first-four UI/source exposure, musical echo-color helper, tested generated sample-shape choices, tested descriptor/preset/render smoke behavior, and tested Chip Poly allocation across the four exposed voices; BRR decoding, source directory/sample memory addressing, S-DSP noise, pitch modulation, FIR echo, exact ADSR/gain envelope timing, SPC700 CPU timing, output filtering, and hardware validation still required |
...
| Arcade Hybrid | Cross-chip arcade SFX performance layer | Original Chipper composition layer over simple tone, stepped-wave, sample/transient, and noise/SFX lanes | Internal; no arcade-board or third-party emulator code is vendored | Verified-partial hybrid layer with four audible source lanes, source enables/levels, generated source shapes, pitch gestures for coin/jump/laser/power-up templates, one-shot decay helper, tested factory presets, tested descriptor/render smoke behavior, and tested Chip Poly allocation across the four lanes; this is not a single hardware chip, arcade board, or cycle-accurate emulator, and exact board mixers, DAC curves, source-chip routing, cabinet response, and hardware validation are outside the current claim |
...
- Arcade Hybrid is an original Chipper performance layer, not a hardware emulation target. It may use implemented Chipper chip behaviors as future embeddable sources, but it must continue to avoid single-board or cycle-accuracy claims.
