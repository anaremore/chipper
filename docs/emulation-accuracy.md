...
| YM2149 / AY | Three tones, shared noise, mixer, volume/envelope | Clean-room register model | Internal | Partial core with tone periods, tested shared noise period endpoint mapping through register 6, register 7 mixer choices for noise-only/tone-only/tone+noise, source-aware mixer masks, fixed volume/envelope shape registers, tested envelope period writes through registers 11/12, YM-style 32-step envelope counter behavior, tested envelope shape-register reset behavior, and tested pitched-channel Chip Poly allocation across A/B/C; exact noise polynomial timing, output curve, AY/YM variant differences, and hardware validation still required |
