...
- Complete: YM2151 / OPM presents all eight four-operator channels in a two-row bank, keeps `$0F` operator-4 noise inside channel 8, exposes Algorithm/Feedback/Operator Tone/FM Level as one shared patch beside the editable operator matrix, and groups native LFO Depth, waveform, PMS, AMS, and per-lane pan routing before a clock/output-only footer.
- Complete: OPL2 / OPL3 presents nine topology-aware lanes plus a shared OP1-OP4 matrix; all four YMF262 algorithms write both linked `$C0` bits, carrier/modulator roles remain explicit, and multiplier/TL/AR/DR/SL/RR controls stay readable at both supported widths.
...
- Complete: YM2610 / OPNB presents its four non-contiguous FM lanes above a centered SSG A-C row, keeps each SSG Tone/Noise selector in its lane, discloses ADPCM-A segment ownership across FM1-4 and SSG A-B in Drum/Hit, and separates the shared FM patch/operator matrix, shared SSG generator, external ADPCM-A/B memory, and compact terminal footer.
- Complete: YM2610B / OPNB2 presents six FM and three SSG lanes in a stable 3x3 bank, keeps each SSG Tone/Noise selector in its lane, maps Drum/Hit ADPCM-A1-6 ownership exactly to FM1-6, and separates the shared FM patch/operator matrix, shared SSG generator, external ADPCM-A/B memory, and compact terminal footer.
...
- Complete: snapshot tooling names the primary surface `editor` and captures `browser`, `motion`, and the focused `opl4op` algorithm fixture explicitly rather than multiplying obsolete workspace variants.
