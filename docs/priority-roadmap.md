...
Review status: planning docs synced on 2026-06-29 after the YM2610/OPNB ADPCM-A/B sample-memory checkpoint reached renderer and VST loading/state paths and the YM2610B/OPNB2 six-FM first pass landed as a separate VST-fit chip mode. The editor height contract remains chip-aware: most modes stay at 1240 x 820, SID uses 1240 x 880 so its per-voice ADSR surface does not collide with Performance Macros, and the UI smoke gate checks every chip at the 1180 px compact-width contract. The focused UI gate includes default editor size, host-restored oversize clamping, readable visible controls, standard dropdown sizing, NES DMC sample controls, NES Performance Macro visibility, all-chip compact layout geometry, SID ADSR separation from Performance Macros at default and compact widths, a readable SID ADSR panel minimum, sampler source-card Level lanes, and OPNA/OPNB ADPCM controls that must remain visible, owned, and close to their relevant preview. Fixed regressions are release gates, not active roadmap items; broad workflow ideas, visual audit rules, fixed-bug details, and license evidence live in their owning docs. The current forward bias is concrete chip-aware improvements: OPL3 four-op/18-channel depth, OPLL custom patch/rhythm polish, OPM/OPN2 modulation/operator depth, and OPNA/OPNB ADPCM validation, then larger slices on sample/wave editors and presets. When we deliberately go wide again, the remembered next new-chip candidates are NES + Namco 163 after wave-editor groundwork, then SegaPCM, RF5C68, and OKI MSM6258/MSM6295 after sample-bank hardening.
...
| 2 | YM2608 / OPNA | 8 | Implemented current FM-family slice | Natural YM2203 superset with PC-98 value. The current slice covers six FM lanes, embedded SSG tone/noise/envelope lanes, generated/user-owned ADPCM-A rhythm ROM support, and first-pass encoded ADPCM-B sample memory through ymfm; WAV/AIFF conversion, sample editing, and stronger validation remain follow-up work. |
| 3 | YM2610 / OPNB and YM2610B / OPNB2 | 8 | Implemented current OPNB/OPNB2 slice | Strong Neo Geo identity and close to OPNA concepts. The current vertical slice covers YM2610 four-FM and YM2610B/OPNB2 six-FM plus embedded SSG tone/noise/envelope lanes through ymfm; renderer/VST encoded ADPCM-A/B sample memory is in place for both; conversion/editing and golden validation remain major follow-up milestones. |
...
The current implementation wave has landed **NES + VRC6**, **YM2608/OPNA**, the first **YM2610/OPNB** playable slice, **YM2610B/OPNB2** six-FM support, **NES + FDS**, **NES + Sunsoft 5B**, **NES + MMC5**, **NES + VRC7**, **SAA1099**, **PC Speaker**, **ZX Spectrum beeper**, **OPN/OPNA/OPNB SSG noise/envelope polish**, **OPNA ADPCM-A rhythm** with generated defaults plus renderer/VST support for user-owned ROM bytes, first-pass **OPNA ADPCM-B encoded sample memory**, and first-pass **OPNB/OPNB2 ADPCM-A/B encoded sample memory** for Drum/Hit macros through renderer and VST workflows. If we prefer depth over more labels, the best slices are **OPL3 four-op/18-channel depth**, **OPLL custom patch/rhythm polish**, **OPM/OPN2 modulation/operator depth**, **OPNA/OPNB ADPCM conversion/editing/golden validation**, **VRC7 custom patch/rhythm/mixer polish**, **PC Speaker/ZX reference and golden validation**, and **SAA1099 golden/reference validation plus envelope/noise polish**. The remembered next new-chip targets are **NES + Namco 163** after wave-editor groundwork and **SegaPCM**, **RF5C68**, and **OKI MSM6258/MSM6295** after the sample-bank workflow feels boringly reliable.

## Current Next-Best Priority List

This is the active implementation order after NES expansion modes, SAA1099, PC Speaker, ZX Spectrum beeper, OPN/OPNA/OPNB SSG noise/envelope polish, OPNB ADPCM sample memory, and the first YM2610B/OPNB2 six-FM pass. The broader table above is still the all-candidate Furnace evaluation; this list is the order to execute when no P0 regression is blocking release.

| Rank | Candidate | Confidence (1-10) | First shippable slice | Why this fits Chipper now |
| ---: | --- | ---: | --- | --- |
| 1 | YMF262 / OPL3 full 4-op + 18-channel depth | 8 | Expose OPL3 new-mode voice layout, 2-op/4-op pairing, stereo routing, and rhythm clarity. | The core is already present; the missing product value is real OPL3 editing rather than another OPL label. |
| 2 | YM2413 / OPLL custom patch + rhythm polish | 8 | Add a shared custom-patch editor for OPLL/VRC7 and make rhythm/drum behavior clearer. | emu2413 is already vendored, and custom OPLL/VRC7 editing is high musical value with contained scope. |
| 3 | YM2151 / OPM LFO, detune, noise, and operator depth | 8 | Add LFO PM/AM, DT1/DT2-oriented controls, better channel-8 noise controls, and deeper original presets. | OPM is already implemented, distinctive, and important for arcade/X68000 sounds. |
| 4 | YM2612 / OPN2 DAC, LFO, detune, and operator preset depth | 8 | Deepen channel-6 DAC drums, LFO/AMS/PMS, detune, and original Genesis presets. | Genesis FM is a flagship mode; improving its existing workflow beats adding another near-duplicate FM chip. |
| 5 | YM2608 / OPNA and YM2610 / OPNB ADPCM conversion/editing/golden validation | 7 | Keep the existing ADPCM-A/B byte-memory paths stable, then add conversion/editing only when the sample workflow is dependable. | OPNA and OPNB now have first-pass ADPCM memory; remaining value is WAV/AIFF conversion, editing, and stronger validation rather than another loader. |
| 6 | SID 6581/8580 PWM/filter/sync/ring depth | 8 | Expand original presets, validate filter/model behavior, and sharpen voice-3/sync/ring workflows. | The SID surface already matches Chipper's chip-native philosophy and has high user payoff. |
| 7 | Game Boy / DMG zombie envelope, sweep, and noise polish | 8 | Add truthful advanced envelope/sweep/noise behavior and presets without pretending to be a full tracker. | DMG is already implemented; these features make existing lanes more authentic and playable. |
| 8 | POKEY AUDCTL/high-pass/distortion validation | 8 | Deepen AUDCTL pairing/high-pass/distortion controls, traces, and Atari-specific presets. | POKEY already has a clear partial model, and register-oriented polish makes it feel less generic. |
| 9 | Shared wave editor for HuC6280, SCC/SCC+, and Namco WSG | 8 | Build one reusable wave-RAM edit/preview/copy/paste/recall pattern and apply it to the existing wavetable chips. | This unlocks multiple implemented chips and should happen before adding more wavetable modes. |
| 10 | SPC700 BRR loop, ADSR/GAIN, echo/FIR, and sample-bank polish | 8 | Make BRR loops, per-voice envelope state, echo/FIR helpers, note maps, and missing-file states boringly reliable. | Sample-chip quality rises sharply once asset handling and recall are dependable. |
| 11 | Amiga / Paula sample-loop, tracker-map, and MOD polish | 8 | Improve loop metadata, kit mapping, retrigger behavior, and next MOD/sample-bank coverage. | Paula is the best proving ground for later sample-chip workflow because it is already useful and compact. |
| 12 | NES + Namco 163 | 7 | Add a NES-attached N163 vertical slice after the shared wave editor exists. | It is the best next new chip, but voice-count/wave-RAM tradeoffs should be first-class instead of bolted on. |
| 13 | SegaPCM | 7 | Add a focused arcade sample-lane slice after sample-bank hardening. | The 16-channel identity is strong, but it depends on robust sample slots, loops, and missing-file behavior. |
| 14 | Ricoh RF5C68 | 7 | Add banked sample playback, loop behavior, and Sega CD/FM Towns presets after the sampler pattern is proven. | High musical value, same workflow dependency as SegaPCM. |
| 15 | OKI MSM6258/MSM6295 | 7 | Add one-shot ADPCM drum/SFX lanes after ADPCM audition/import conventions are settled. | Excellent drums/SFX payoff, but quality depends on kit and provenance workflow. |
| 16 | AY8930 and OKI MSM5232 | 6 | Treat as compact PSG/tone-chip follow-ups after AY/SAA1099 validation. | Both fit Chipper, but they are not better than deepening the existing PSG and beeper families first. |
| 17 | Capcom QSound, YMZ280B, Konami K007232/K053260, Irem GA20, Namco C140/C219, and Ensoniq ES5506 | 6 | Queue as arcade sample-chip follow-ups after SegaPCM/RF5C68/OKI prove the lane and bank model. | Valuable later, but all want mature sample memory, mixing, loop, and preset infrastructure. |
| 18 | WonderSwan, X1-010, Mikey, Virtual Boy, and GBA DMA | 6 | Queue as handheld/wavetable/sample follow-ups after the shared wave and sample editors exist. | Good character, lower immediate pull than the implemented families and N163. |
| 19 | OPZ, OPL4, Y8950, and ESS ESFM | 5 | Queue after OPN/OPM/OPL editor depth and ADPCM/sample workflows are stable. | They are attractive, but each depends on groundwork we should finish first. |
| 20 | TIA, VIC, TED, PET, PV-1000, Pokemon Mini, Tiger Game.com, Watara Supervision, Nintendo DS, and software engines such as SFX-like, QuadTone, TIunA, and MinMod | 4 | Defer or translate into motion/SFX/preset tools rather than standalone chip modes for now. | Interesting references, but lower VST payoff or too broad for Chipper's current chip-instrument scope. |
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM/OPN/OPNA/OPNB/OPNB2 now have editable Operator/SSG Tone where relevant, FM/SSG Level, four neutral-default per-operator Level trims, four neutral-default per-operator Multiplier menus, neutral-default per-operator Attack Rate, Decay Rate, Sustain Rate, and Release Rate menus, feedback menus, OPN/OPNA/OPNB/OPNB2 SSG tone/noise/envelope exposure, OPNA generated/user-owned ADPCM-A rhythm plus encoded ADPCM-B memory coverage, and OPNB/OPNB2 encoded ADPCM-A/B memory coverage with renderer/VST state recall. The next step should be OPL3/OPLL/OPM/OPN2 depth, OPNA/OPNB ADPCM conversion/editing validation, chip-specific modulation, or clearer operator-grid visualization.
...
| 10 | YM2608 / OPNA | PC-98 six-FM plus embedded SSG vocabulary with a clear route toward sample-backed depth. | First BSD-3-Clause `ymfm` OPNA adapter plus generated/user-owned ADPCM-A rhythm overlay and encoded ADPCM-B sample memory are in place for renderer and VST workflows. | Six FM strips plus three SSG tone/noise/envelope lanes now; next add ADPCM-B conversion/editing and golden comparisons. | 8 |
| 11 | YM2610 / OPNB and YM2610B / OPNB2 | Neo Geo four-FM/six-FM plus embedded SSG vocabulary with a clear route toward stronger ADPCM workflow. | BSD-3-Clause `ymfm` OPNB and OPNB2 adapters are in place, with renderer/VST encoded ADPCM-A/B sample memory. | Four-FM OPNB and six-FM OPNB2 strips plus three SSG tone/noise/envelope lanes now; next add ADPCM conversion/editing and golden comparisons. | 8 |
