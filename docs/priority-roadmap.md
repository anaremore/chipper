...
| ---: | --- | ---: | --- |
| 1 | YM2203 / OPN | 9 | Best next new Yamaha chip: reuses the OPN2/ymfm path, adds SSG overlap with the AY/YM2149 family, and has strong PC-88/arcade preset value. |
...
The next best new chip target is YM2203/OPN. The next best broader feature target is NES expansion audio. The next best small-scope wins are SAA1099 and PC Speaker/ZX beeper. The next big sample-chip targets are SegaPCM, RF5C68, and OKI after sampler-bank workflow hardening.
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM now have editable Operator Tone, FM Level, four neutral-default per-operator Level trims, four neutral-default per-operator Multiplier menus, and neutral-default per-operator Attack Rate, Decay Rate, Sustain Rate, and Release Rate menus with full automation, preset, renderer, MIDI, compact Operator EG UI coverage, and curated original factory balances for the level slice. The next step should be chip-specific modulation and clearer operator-grid visualization.
