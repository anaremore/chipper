...
| ---: | --- | ---: | --- |
| 1 | YM2203 / OPN follow-up | 9 | First FM-only `ymfm` slice is implemented; next value is exposing/mixing the embedded SSG honestly and deciding how it shares UI with AY/YM2149-style controls. |
| 2 | NES expansion audio pack: VRC6, FDS, MMC5, Sunsoft 5B, Namco 163 | 8 | Builds directly on existing NES controls and preset vocabulary; present as NES Expansion choices instead of unrelated one-off chips. |
| 3 | YM2608 / OPNA | 8 | Natural YM2203 superset with PC-98 identity; high value, but its FM + SSG + ADPCM + rhythm scope should follow a smaller OPN slice. |
| 4 | YM2610 / OPNB and YM2610B | 8 | Strong Neo Geo/arcade value and close to OPNA concepts; ADPCM-A/B bank workflow makes it a second-phase FM/sample hybrid. |
| 5 | VRC7 / Family Noraebang OPLL | 8 | Fits because YM2413/OPLL already exists; implement after OPLL patch/rhythm polish so preset FM stays honest. |
| 6 | SAA1099 | 8 | Compact PSG-style win with stereo, dual noise, and envelopes; likely a clean vertical slice with useful computer/arcade presets. |
| 7 | PC Speaker / ZX beeper | 8 | Very Chipper-friendly as tiny SFX and bit-bang engines with gesture presets, as long as they are not oversold as broad synths. |
| 8 | YM3526 / YM3812 / Y8950 depth | 8 | Improve the OPL2 family before adding more labels; Y8950 ADPCM becomes valuable after sample flow is stronger. |
...
| 10 | AY8930 | 7 | Easy AY-family extension with moderate payoff; lower pull than YM2203 or SAA1099. |
...
The next best new-chip follow-up is YM2203/OPN SSG exposure. The next best broader feature target is NES expansion audio. The next best full new chips are YM2608/OPNA and YM2610/OPNB if we want to stay in Yamaha FM, or SAA1099 and PC Speaker/ZX beeper for smaller-scope wins. The next big sample-chip targets are SegaPCM, RF5C68, and OKI after sampler-bank workflow hardening.
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM/OPN now have editable Operator Tone, FM Level, four neutral-default per-operator Level trims, four neutral-default per-operator Multiplier menus, neutral-default per-operator Attack Rate, Decay Rate, Sustain Rate, and Release Rate menus, and feedback menus with automation, preset, renderer, MIDI, compact Operator EG UI coverage, and curated original factory balances. The next step should be YM2203 SSG integration, chip-specific modulation, or clearer operator-grid visualization.
...
| 9 | YM2203 / OPN SSG follow-up | Complete the first OPN implementation by exposing the embedded PSG/SSG side without muddying the FM operator surface. | Continue the BSD-3-Clause `ymfm` adapter already used for the FM slice. | Three FM voices plus a clear SSG mixer/control surface, likely borrowing AY/YM2149 tone/noise/envelope vocabulary where honest. | 9 |
| 10 | YM2413 / OPLL | Preset-FM quick sounds and UI/game chimes. | Audit `ymfm` and `emu2413`. | Preset instrument selector plus limited editable patch/rhythm controls. | 7 |
| 11 | SCC deeper pass | Turn the emu2212-backed partial Konami SCC/SCC+ core into a first-class five-channel wavetable instrument. | Continue the emu2212 adapter; add golden/reference comparisons before stronger accuracy claims. | Five wavetable strips with wave edit/slot, visible level, tuning, key mask, SCC/SCC+ behavior, and stack/spread macros. | 8 |
| 12 | HuC6280 deeper pass | Turn the partial PC Engine core into a first-class six-channel wavetable instrument. | Continue clean-room; audit MAME/GME/Furnace/HuSIC before any reuse. | Six wavetable channel strips with wave RAM preview/editing, visible level, native noise, Ch 1/2 LFO behavior, channel balance, and source scopes. | 6 |
| 13 | Namco WSG deeper pass | Turn the partial Namco WSG core into a first-class arcade wavetable instrument. | Continue clean-room; audit any MAME/Furnace-derived references before reuse. | Wavetable lane banks with 4-bit waveform memory, visible levels, voice-count variants, enable masks, tuning, and source scopes. | 6 |
| 14 | POKEY deeper pass | Improve the partial Atari core. | Continue clean-room; web-pokey/MAME/Furnace reference-only unless audited. | Add AUDCTL channel pairing, high-pass filters, stronger polynomial validation, and source-card visuals. | 6 |
