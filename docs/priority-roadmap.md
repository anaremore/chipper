...
3. **OPN2/OPM FM operator editor slice.** Start with YM2612 because ymfm is already vendored and six lanes are visible. The first passes made Operator EG role-aware, moved the stable Operator Tone and FM Level controls into Operator EG, added four APVTS/MIDI/preset/renderer-backed FM Operator Level trims that offset native total-level registers for OPN2 and OPM, and added four neutral-default per-operator Multiplier menu controls that write native YM2612/YM2151 multiplier nibbles while `Follow` preserves Operator Tone. The operator rows now carry compact level and multiplier controls beside the carrier/modulator register readouts, and the first original FM preset-balance pass uses the level trims for OPN2/OPM basses, leads, keys, pads, bells, drums, and SFX. The first diagnostic-driven OPN2 expansion added original DAC snare, operator-envelope bell, and chord-pad presets, with descriptor/catalog/render JSON gates. Next, continue into envelope-rate controls and chip-specific modulation while keeping renderer JSON and held-tail checks green.
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM now have editable Operator Tone, FM Level, four neutral-default per-operator Level trims, and four neutral-default per-operator Multiplier menus with full automation, preset, renderer, MIDI, compact Operator EG UI coverage, and curated original factory balances for the level slice. The next step should be deeper operator-grid controls for envelope rates and chip-specific modulation.
