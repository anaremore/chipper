...
Review status: planning docs synced on 2026-06-29 after the PC Speaker beeper slice joined the implemented chip set. The editor height contract remains chip-aware: most modes stay at 1240 x 820, SID uses 1240 x 880 so its per-voice ADSR surface does not collide with Performance Macros, and the UI smoke gate checks every chip at the 1180 px compact-width contract. The focused UI gate includes default editor size, host-restored oversize clamping, readable visible controls, standard dropdown sizing, NES DMC sample controls, NES Performance Macro visibility, all-chip compact layout geometry, SID ADSR separation from Performance Macros at default and compact widths, a readable SID ADSR panel minimum, and sampler source-card Level lanes that must remain visible, owned, and close to their sample/wave selectors. Fixed regressions are release gates, not active roadmap items; broad workflow ideas, visual audit rules, fixed-bug details, and license evidence live in their owning docs. The current forward bias is concrete chip-aware improvements: finish the remaining layout/readability debt, then spend larger slices on sample/wave editors, presets, and FM operator editing. When we deliberately go wide again, the remembered next best new-chip candidate is ZX Spectrum beeper.
...
| 2 | YM2608 / OPNA | 8 | Implemented current FM-family slice | Natural YM2203 superset with PC-98 value. The first vertical slice covers six FM lanes plus embedded SSG tone lanes through ymfm; native rhythm, ADPCM-A, ADPCM-B, and deeper SSG controls remain follow-up work. |
| 3 | YM2610 / OPNB and YM2610B / OPNB2 | 8 | Implemented first OPNB slice | Strong Neo Geo identity and close to OPNA concepts. The first vertical slice covers YM2610 four-FM plus embedded SSG tone lanes through ymfm; external ADPCM-A/B and YM2610B/OPNB2 six-FM behavior remain major follow-up milestones. |
...
| 10 | ZX Spectrum beeper | 8 | Implement next new-chip beeper slice | Strong SFX and arpeggio identity; should be framed as beeper engines and gestures rather than a broad synth. |
| 11 | YM2203 / OPN, YM2608 / OPNA, and YM2610 / OPNB SSG noise and envelope polish | 8 | Deepen existing modes | FM plus SSG tone lanes are in place for OPN, OPNA, and OPNB; next value is honest SSG noise/envelope UI and presets without pretending the SSG side is a full AY editor yet. |
...
The current implementation wave has landed **NES + VRC6**, **YM2608/OPNA**, the first **YM2610/OPNB** playable slice, **NES + FDS**, **NES + Sunsoft 5B**, **NES + MMC5**, **NES + VRC7**, **SAA1099**, and **PC Speaker**. The next best new-chip candidate is **ZX Spectrum beeper** for another high-confidence beeper/SFX win. If we prefer depth over more labels, the best slices are **OPN/OPNA/OPNB SSG noise/envelope**, **OPNA/OPNB ADPCM**, **OPL/OPLL/OPM/OPN2 operator/rhythm/custom-patch polish**, **VRC7 custom patch/rhythm/mixer polish**, **PC Speaker reference/golden validation**, and **SAA1099 golden/reference validation plus envelope/noise polish**. The next sample-chip targets are **SegaPCM**, **RF5C68**, and **OKI MSM6258/MSM6295**, but only after the sample-bank workflow feels boringly reliable.
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM/OPN/OPNA/OPNB now have editable Operator Tone, FM/SSG Level where relevant, four neutral-default per-operator Level trims, four neutral-default per-operator Multiplier menus, neutral-default per-operator Attack Rate, Decay Rate, Sustain Rate, and Release Rate menus, feedback menus, and OPN/OPNA/OPNB SSG tone-lane exposure with automation, preset, renderer, MIDI, compact Operator EG UI coverage, and curated original factory balances. The next step should be OPN/OPNA/OPNB SSG noise/envelope polish, OPNA rhythm/ADPCM, OPNB ADPCM/OPNB2 behavior, chip-specific modulation, or clearer operator-grid visualization.
6. If the next slice should be a new chip instead of depth, implement **ZX Spectrum beeper** next. Keep it scoped as an honest beeper/SFX instrument with gesture presets and renderer trace coverage, not a broad synth engine.
...
| 8 | SNES SPC700-style | 8 | Current code is internal clean-room partial. emu-rs/snes-apu and snes-echo are permissive-looking audit candidates; C700, blargg snes_spc, GME, Furnace, and MAME remain reference or license-sensitive audit targets only. | Deepen the current eight-voice sample surface with clearer per-voice loop/envelope/noise/echo state, exact BRR loop semantics, Gaussian/output validation, and honest echo/FIR controls. |
| 9 | Atari POKEY | 7 | Current code is internal clean-room partial; web-pokey remains a permissive candidate/reference until audited. | Expand beyond the first AUDF/AUDC/AUDV pass with clearer distortion-code UI, AUDCTL pairing/high-pass controls, and Atari preset vocabulary. |
| 10 | Amiga Paula | 8 | Current code is internal clean-room partial; pt2-clone/libxmp/Furnace remain reference or audit targets only. | Deepen the current four-channel tracker sample surface with visible period/loop/volume state, MOD-style retrigger vocabulary, imported uncompressed 8SVX, WAV `smpl`, AIFF `MARK`/`INST`, and first-pass ProTracker MOD sample loop metadata, and remaining full-MOD playback/effect workflow checks. |
| 11 | PC Engine HuC6280 | 7 | Current code is internal clean-room partial; MAME/GME/Furnace/HuSIC remain reference or audit targets only. | Deepen the current six-lane wavetable surface with editable 32-sample Wave RAM, native noise, Ch 1/2 LFO clarity, per-lane pitch/pan where appropriate, and PC Engine preset vocabulary. |
| 12 | Namco arcade WSG | 7 | Current code is internal clean-room partial; MAME/Furnace remain reference or audit targets only. | Deepen the current eight-lane WSG surface with 4-bit Wave RAM editing, enable/volume masks, voice-count variants, tuning controls, and arcade preset vocabulary. |
| 13 | Konami SCC | 7 | Current audio path is backed by vendored MIT emu2212; exact SCC/SCC+ quirks still need golden/reference validation. | Deepen the current five-lane SCC wavetable surface with Wave RAM editing, key/volume controls, SCC/SCC+ distinctions, and deeper Konami preset vocabulary. |
| 14 | YM2612 / OPN2, YM2203 / OPN, YM2608 / OPNA, YM2610 / OPNB, OPL2/OPL3, YM2151 / OPM, and YM2413 / OPLL | 8 | ymfm is now vendored for first YM2612, YM2203, YM2608, YM2610, OPL2, and YM2151 melodic adapters; emu2413 is vendored for OPLL. Keep OPN/OPNA/OPNB SSG noise/envelope polish, OPNA rhythm/ADPCM, OPNB ADPCM/OPNB2 behavior, OPL3-specific, custom OPLL patch, and rhythm behavior inside the audited source/licensing plan. | Move from macro/operator summaries to real operator/algorithm UI with envelope visuals, carrier/modulator clarity, preserved held-tail regression coverage, and chip-specific FM/SSG artifacts. |
...
| 9 | YM2203 / OPN SSG polish | Complete the OPN SSG side by adding honest noise/envelope controls without muddying the FM operator surface. | Continue the BSD-3-Clause `ymfm` adapter already used for the FM+SSG tone slice. | Three FM voices plus three SSG tone voices are in place; next add a clear SSG noise/envelope control surface borrowing AY/YM2149 vocabulary where honest. | 8 |
| 10 | YM2608 / OPNA | PC-98 six-FM plus embedded SSG vocabulary with a clear route toward rhythm and ADPCM. | First BSD-3-Clause `ymfm` OPNA adapter is in place. | Six FM strips plus three SSG tone lanes now; next add SSG noise/envelope, native rhythm, ADPCM-A/B, and golden comparisons. | 8 |
| 11 | YM2610 / OPNB | Neo Geo four-FM plus embedded SSG vocabulary with a clear route toward ADPCM and OPNB2. | First BSD-3-Clause `ymfm` OPNB adapter is in place. | Four FM strips plus three SSG tone lanes now; next add external ADPCM-A/B, YM2610B/OPNB2 six-FM behavior, SSG noise/envelope, and golden comparisons. | 8 |
