...
| ---: | --- | ---: | --- | --- |
| 1 | YM2608 / OPNA ADPCM-B + rhythm provenance polish | 8 | Keep the generated/user-owned ADPCM-A rhythm overlay, add polished plugin rhythm-ROM UI, then add ADPCM-B/user sample import only once the sample workflow is dependable. | YM2608 already rides the vendored ymfm path and now has native rhythm register coverage; remaining value is truthful sample/provenance handling, plugin ergonomics, and validation. |
...
| 10 | YM2608 / OPNA | PC-98 six-FM plus embedded SSG vocabulary with a clear route toward sample-backed depth. | First BSD-3-Clause `ymfm` OPNA adapter plus generated/user-owned ADPCM-A rhythm overlay is in place. | Six FM strips plus three SSG tone/noise/envelope lanes now; next add ADPCM-B/user samples, polished plugin rhythm-ROM UI, and golden comparisons. | 8 |
