...
Review status: planning docs synced on 2026-06-29 after the YM2610/OPNB ADPCM-A/B sample-memory checkpoint reached renderer and VST loading/state paths. The editor height contract remains chip-aware: most modes stay at 1240 x 820, SID uses 1240 x 880 so its per-voice ADSR surface does not collide with Performance Macros, and the UI smoke gate checks every chip at the 1180 px compact-width contract. The focused UI gate includes default editor size, host-restored oversize clamping, readable visible controls, standard dropdown sizing, NES DMC sample controls, NES Performance Macro visibility, all-chip compact layout geometry, SID ADSR separation from Performance Macros at default and compact widths, a readable SID ADSR panel minimum, sampler source-card Level lanes, and OPNA/OPNB ADPCM controls that must remain visible, owned, and close to their relevant preview. Fixed regressions are release gates, not active roadmap items; broad workflow ideas, visual audit rules, fixed-bug details, and license evidence live in their owning docs. The current forward bias is concrete chip-aware improvements: OPNB2 and ADPCM validation, then larger slices on sample/wave editors, presets, and FM operator editing. When we deliberately go wide again, the remembered next new-chip candidates are NES + Namco 163 after wave-editor groundwork, then SegaPCM, RF5C68, and OKI MSM6258/MSM6295 after sample-bank hardening.
...
| 3 | YM2610 / OPNB and YM2610B / OPNB2 | 8 | Implemented current OPNB ADPCM slice | Strong Neo Geo identity and close to OPNA concepts. The current vertical slice covers YM2610 four-FM plus embedded SSG tone/noise/envelope lanes through ymfm; renderer/VST encoded ADPCM-A/B sample memory is in place; YM2610B/OPNB2 six-FM behavior, conversion/editing, and golden validation remain major follow-up milestones. |
...
The current implementation wave has landed **NES + VRC6**, **YM2608/OPNA**, the first **YM2610/OPNB** playable slice, **NES + FDS**, **NES + Sunsoft 5B**, **NES + MMC5**, **NES + VRC7**, **SAA1099**, **PC Speaker**, **ZX Spectrum beeper**, **OPN/OPNA/OPNB SSG noise/envelope polish**, **OPNA ADPCM-A rhythm** with generated defaults plus renderer/VST support for user-owned ROM bytes, first-pass **OPNA ADPCM-B encoded sample memory**, and first-pass **OPNB ADPCM-A/B encoded sample memory** for Drum/Hit macros through renderer and VST workflows. If we prefer depth over more labels, the best slices are **OPNB2 plus OPNB ADPCM conversion/editing/golden validation**, **OPNA ADPCM-B conversion/editing/golden validation**, **OPL/OPLL/OPM/OPN2 operator/rhythm/custom-patch polish**, **VRC7 custom patch/rhythm/mixer polish**, **PC Speaker/ZX reference and golden validation**, and **SAA1099 golden/reference validation plus envelope/noise polish**. The remembered next new-chip targets are **NES + Namco 163** after wave-editor groundwork and **SegaPCM**, **RF5C68**, and **OKI MSM6258/MSM6295** after the sample-bank workflow feels boringly reliable.
...
| ---: | --- | ---: | --- | --- |
| 1 | YM2610B / OPNB2 six-FM behavior + OPNB ADPCM validation | 8 | Add YM2610B/OPNB2 six-FM behavior, then strengthen ADPCM conversion/editing and golden validation around the renderer/VST sample-memory path. | Neo Geo identity is now much closer with ADPCM recall; the highest-impact remaining gap is the OPNB2 voice model plus confidence-building ADPCM tooling and references. |
| 2 | YMF262 / OPL3 full 4-op + 18-channel depth | 8 | Expose OPL3 new-mode voice layout, 2-op/4-op pairing, stereo routing, and rhythm clarity. | The core is already present; the missing product value is real OPL3 editing rather than another OPL label. |
| 3 | YM2413 / OPLL custom patch + rhythm polish | 8 | Add a shared custom-patch editor for OPLL/VRC7 and make rhythm/drum behavior clearer. | emu2413 is already vendored, and custom OPLL/VRC7 editing is high musical value with contained scope. |
| 4 | YM2151 / OPM LFO, detune, noise, and operator depth | 8 | Add LFO PM/AM, DT1/DT2-oriented controls, better channel-8 noise controls, and deeper original presets. | OPM is already implemented, distinctive, and important for arcade/X68000 sounds. |
| 5 | YM2612 / OPN2 DAC, LFO, detune, and operator preset depth | 8 | Deepen channel-6 DAC drums, LFO/AMS/PMS, detune, and original Genesis presets. | Genesis FM is a flagship mode; improving its existing workflow beats adding another near-duplicate FM chip. |
| 6 | YM2608 / OPNA ADPCM-B conversion/editing/golden validation | 7 | Keep the generated/user-owned ADPCM-A rhythm overlay and encoded ADPCM-B loader stable, then add conversion/editing only when the sample workflow is dependable. | YM2608 now has first-pass ADPCM-B sample memory; remaining value is WAV/AIFF conversion, editing, and stronger validation rather than another loader. |
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM/OPN/OPNA/OPNB now have editable Operator/SSG Tone where relevant, FM/SSG Level, four neutral-default per-operator Level trims, four neutral-default per-operator Multiplier menus, neutral-default per-operator Attack Rate, Decay Rate, Sustain Rate, and Release Rate menus, feedback menus, OPN/OPNA/OPNB SSG tone/noise/envelope exposure, OPNA generated/user-owned ADPCM-A rhythm plus encoded ADPCM-B memory coverage, and OPNB encoded ADPCM-A/B memory coverage with renderer/VST state recall. The next step should be OPNB2 behavior, OPNA/OPNB ADPCM conversion/editing validation, chip-specific modulation, or clearer operator-grid visualization.
...
| 11 | YM2610 / OPNB | Neo Geo four-FM plus embedded SSG vocabulary with a clear route toward ADPCM and OPNB2. | First BSD-3-Clause `ymfm` OPNB adapter is in place, with renderer/VST encoded ADPCM-A/B sample memory. | Four FM strips plus three SSG tone/noise/envelope lanes now; next add YM2610B/OPNB2 six-FM behavior, ADPCM conversion/editing, and golden comparisons. | 8 |
