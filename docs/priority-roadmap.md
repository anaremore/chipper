...
The current implementation wave has landed **NES + VRC6**, **YM2608/OPNA**, and the first **YM2610/OPNB** playable slice. The next best candidates are **NES + FDS/Sunsoft 5B/MMC5/VRC7** if we want to keep building the NES expansion pack, **SAA1099/PC Speaker/ZX beeper** for smaller high-confidence wins, or **OPN/OPNA/OPNB SSG noise/envelope plus OPNA/OPNB ADPCM** if we want to deepen the Yamaha family before adding more chip labels. The next sample-chip targets are **SegaPCM**, **RF5C68**, and **OKI MSM6258/MSM6295**, but only after the sample-bank workflow feels boringly reliable.
