...
The current implementation wave has landed **NES + VRC6**, **YM2608/OPNA**, the first **YM2610/OPNB** playable slice, **NES + FDS**, and **NES + Sunsoft 5B**. The next best new-chip candidate is **NES + MMC5** because it extends the same NES expansion architecture with modest UI/test scope. The next alternate branch is **NES + VRC7 / Family Noraebang OPLL** if we want another NES expansion that reuses OPLL work, followed by **SAA1099**, **PC Speaker**, and **ZX Spectrum beeper** for compact high-confidence PSG/beeper wins. If we prefer depth over more labels, the best slices are **OPN/OPNA/OPNB SSG noise/envelope**, **OPNA/OPNB ADPCM**, and **OPL/OPLL/OPM/OPN2 operator/rhythm/custom-patch polish**. The next sample-chip targets are **SegaPCM**, **RF5C68**, and **OKI MSM6258/MSM6295**, but only after the sample-bank workflow feels boringly reliable.
