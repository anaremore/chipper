...
- Keep the plugin DAW-fit first. Every chip page now opens, clamps, and advertises the 620 px default height as the resize maximum so high-DPI hosts do not reopen Chipper off-screen; any future layout work has to fit inside that budget or introduce a deliberate secondary view.
