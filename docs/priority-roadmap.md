...
| ---: | --- | ---: | --- | --- |
| 1 | YM2608 / OPNA ADPCM-B + rhythm provenance polish | 8 | Keep the generated ADPCM-A rhythm overlay, add renderer/UI trace polish, then add ADPCM-B/user sample import only once the sample workflow is dependable. | YM2608 already rides the vendored ymfm path and now has native rhythm register coverage; remaining value is truthful sample/provenance handling and validation. |
...
| 10 | YM2608 / OPNA | PC-98 six-FM plus embedded SSG vocabulary with a clear route toward sample-backed depth. | First BSD-3-Clause `ymfm` OPNA adapter plus generated ADPCM-A rhythm overlay is in place. | Six FM strips plus three SSG tone/noise/envelope lanes now; next add ADPCM-B/user samples, optional user-owned rhythm ROM loading, and golden comparisons. | 8 |
