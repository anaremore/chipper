...
3. **OPN2/OPM FM operator editor slice.** Start with YM2612 because ymfm is already vendored and six lanes are visible. The first passes made Operator EG role-aware, moved the stable Operator Tone and FM Level controls into Operator EG, and added four APVTS/MIDI/preset/renderer-backed FM Operator Level trims that offset native total-level registers for OPN2 and OPM. Next, give those trims a proper compact operator-grid UI, then use them to curate richer original FM presets while keeping renderer JSON and held-tail checks green.
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM now have editable Operator Tone, FM Level, and four neutral-default per-operator Level trims with full automation, preset, renderer, and MIDI coverage. The next step should be the visible operator-grid/editor layout and then original FM preset curation that proves those controls are useful.
