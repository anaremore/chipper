...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM now have editable Operator Tone, FM Level, and four neutral-default per-operator Level trims with full automation, preset, renderer, MIDI, compact Operator EG UI coverage, and curated original factory balances. The next step should be deeper operator-grid controls for multiplier, envelope rates, and chip-specific modulation.
