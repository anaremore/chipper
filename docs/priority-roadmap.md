...
3. **OPN2 FM operator editor slice.** Start with YM2612 because ymfm is already vendored and six lanes are visible. The first pass now makes the existing Operator EG surface role-aware, labeling resolved four-op slots as carrier or modulator while keeping multiplier, total-level, and envelope register bytes visible. Next, add editable operator/algorithm controls only where APVTS parameters, MIDI CCs, presets, renderer JSON, and held-tail checks can move together.
