...
| 2 | YM2608 / OPNA | 8 | Implemented current FM-family slice | Natural YM2203 superset with PC-98 value. The current slice covers six FM lanes, embedded SSG tone/noise/envelope lanes, and generated ADPCM-A rhythm through ymfm; ADPCM-B, user samples, rhythm ROM provenance, and stronger validation remain follow-up work. |
...
| ---: | --- | ---: | --- | --- |
| 1 | YM2608 / OPNA ADPCM-B + rhythm provenance polish | 8 | Keep the generated/user-owned ADPCM-A rhythm overlay and its renderer/VST loader stable, then add ADPCM-B/user sample import only once the sample workflow is dependable. | YM2608 already rides the vendored ymfm path and now has native rhythm register coverage plus state-restored user ROM overrides; remaining value is truthful ADPCM-B/sample handling and validation. |
...
| 10 | YM2608 / OPNA | PC-98 six-FM plus embedded SSG vocabulary with a clear route toward sample-backed depth. | First BSD-3-Clause `ymfm` OPNA adapter plus generated/user-owned ADPCM-A rhythm overlay is in place for renderer and VST workflows. | Six FM strips plus three SSG tone/noise/envelope lanes now; next add ADPCM-B/user samples and golden comparisons. | 8 |
