...
| 3 | YM2610 / OPNB and YM2610B / OPNB2 | 8 | Next FM/sample hybrid candidate | Strong Neo Geo identity and close to OPNA concepts; ADPCM-A/B bank handling makes it the next major FM/sample hybrid milestone after OPNA. |
| 4 | NES + Famicom Disk System | 8 | Next NES expansion candidate | Single expressive wavetable/modulation expansion that fits the existing NES page and future wave editor direction. |
| 5 | NES + Sunsoft 5B | 8 | Implement after FDS or alongside AY polish | AY-style expansion is a good fit because Chipper already has YM2149/AY vocabulary and source-card patterns. |
| 6 | NES + MMC5 | 8 | Implement after FDS or alongside NES polish | Extra pulse channels and PCM are a clear NES expansion win with modest new UI complexity. |
...
| 11 | YM2203 / OPN and YM2608 / OPNA SSG noise and envelope polish | 8 | Deepen existing modes | FM plus SSG tone lanes are in place for both OPN and OPNA; next value is honest SSG noise/envelope UI and presets without pretending the SSG side is a full AY editor yet. |
...
The current implementation wave is **YM2608/OPNA** after **NES + VRC6** landed as the first expansion slice. After OPNA lands, the next best candidates are **YM2610/OPNB** if we want another Yamaha FM/sample hybrid, **NES + FDS/Sunsoft 5B/MMC5** if we want to keep building the NES expansion pack, or **SAA1099/PC Speaker/ZX beeper** for smaller high-confidence wins. The next sample-chip targets are **SegaPCM**, **RF5C68**, and **OKI MSM6258/MSM6295**, but only after the sample-bank workflow feels boringly reliable.
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM/OPN/OPNA now have editable Operator Tone, FM/SSG Level where relevant, four neutral-default per-operator Level trims, four neutral-default per-operator Multiplier menus, neutral-default per-operator Attack Rate, Decay Rate, Sustain Rate, and Release Rate menus, feedback menus, and YM2203/YM2608 SSG tone-lane exposure with automation, preset, renderer, MIDI, compact Operator EG UI coverage, and curated original factory balances. The next step should be OPN/OPNA SSG noise/envelope polish, OPNA rhythm/ADPCM, chip-specific modulation, or clearer operator-grid visualization.
