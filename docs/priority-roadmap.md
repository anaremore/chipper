...
The current implementation wave has landed **NES + VRC6**, **YM2608/OPNA**, the first **YM2610/OPNB** playable slice, **NES + FDS**, **NES + Sunsoft 5B**, and **NES + MMC5**. The next best new-chip candidate is **NES + VRC7 / Family Noraebang OPLL** because it reuses the existing OPLL path while adding a distinct NES expansion identity. The next compact new-chip branch is **SAA1099**, followed by **PC Speaker** and **ZX Spectrum beeper** for high-confidence PSG/beeper wins. If we prefer depth over more labels, the best slices are **OPN/OPNA/OPNB SSG noise/envelope**, **OPNA/OPNB ADPCM**, and **OPL/OPLL/OPM/OPN2 operator/rhythm/custom-patch polish**. The next sample-chip targets are **SegaPCM**, **RF5C68**, and **OKI MSM6258/MSM6295**, but only after the sample-bank workflow feels boringly reliable.
