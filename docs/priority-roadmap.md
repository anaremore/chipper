...
1. **Preset browser metadata and filtering.** Role, engine, and tag metadata are exported and validated, the VST header now has compact role plus search filtering, and a first original coverage pack fills thin role gaps plus neutral Patch entries without importing Furnace data. The next preset pass should derive a Furnace-inspired quality matrix of chip x role x engine/tag targets, then fill it with original Chipper snapshots, engine/tag filters, favorites, and richer user preset affordances while keeping user presets as portable `.chipperpreset` files.
