...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM/OPN now have editable Operator Tone, FM/SSG Level where relevant, four neutral-default per-operator Level trims, four neutral-default per-operator Multiplier menus, neutral-default per-operator Attack Rate, Decay Rate, Sustain Rate, and Release Rate menus, feedback menus, and YM2203 SSG tone-lane exposure with automation, preset, renderer, MIDI, compact Operator EG UI coverage, and curated original factory balances. The next step should be YM2203 SSG noise/envelope polish, chip-specific modulation, or clearer operator-grid visualization.
