...
| 5 | Build real FM editor surfaces for YM2612/OPN2, YM2151/OPM, OPL2/OPL3, and YM2413/OPLL. Macros can stay, but serious users need operator/algorithm/envelope access. | 9 | 8 | 7 | Operator-grid UI spike, sustained-note tests, and register/debug JSON for algorithm, level, feedback, and envelope state. |
| 6 | Improve NES/RP2A03 authenticity and usability around DMC playback, DMC loop-off terminal DAC hold, sweep edge cases, frame sequencing, nonlinear mixer tolerances, and test trace coverage. | 9 | 7 | 7 | Local development samples, test ROM references, emulator comparison renders, and golden metadata for clean-room behavior. |
...
- Current FM sustain/dropout issues are believed fixed by recent engine work, but sustained-note regression coverage should remain part of the FM test plan.
