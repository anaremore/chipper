...
3. **OPN2 FM operator editor slice.** Start with YM2612 because ymfm is already vendored and six lanes are visible. The first pass now makes the existing Operator EG surface role-aware, labeling resolved four-op slots as carrier or modulator while keeping multiplier, total-level, and envelope register bytes visible. The current slice moves the stable APVTS-backed Operator Tone and FM Level controls into Operator EG for OPN2/OPM, so those editable controls stay tied to MIDI CC, preset recall, renderer behavior, and held-tail checks. Next, graduate from macro-scope controls to deeper per-operator fields only where APVTS parameters, MIDI CCs, presets, renderer JSON, and held-tail checks can move together.
...
5. Continue the FM operator editor as contained vertical slices: OPN2/OPM now have editable Operator Tone and FM Level in Operator EG, so the next step should be per-operator controls with the full automation, preset, renderer, and held-tail contract kept green.
