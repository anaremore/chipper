...
Review status: synced on 2026-07-28 after refreshing ymfm, adding pinned YM2608-LLE ADPCM-A reference gates, and landing schema-9 OPN2 DAC rate/root/trim/tail state plus deterministic write-cadence evidence. This file owns source/licensing posture only; implementation status and verification claims belong in `docs/emulation-accuracy.md`, release-gate commands belong in `docs/release-builds.md`, and active execution order belongs in `docs/priority-roadmap.md`.
...
Latest planning sync: 2026-07-28. Latest upstream license/source spot-check: 2026-07-28 from upstream GitHub pages/license files. Do not treat a repository name, README claim, GitHub license badge, or tracker implementation as enough evidence to vendor code. Re-check the exact files and revision before reuse.
...
- ymfm's repository describes BSD-licensed Yamaha FM cores and its top-level `LICENSE` is BSD-3-Clause. A local file/header audit refreshed on 2026-07-28 at commit `81aec25ccbb98f4873a255f7551ac4dadac59b4a` found BSD-3-Clause headers on the core source files inspected (`src/ymfm*.h`, `src/ymfm*.cpp`, `src/ymfm_fm.ipp`) and README/GeneralInfo provenance notes that the cores began in MAME, use MAME-style design, and are cross-checked against Nemesis/Exodus and Nuked research while explicitly aiming for high audible accuracy rather than cycle accuracy. Chipper now vendors ymfm for verified-partial YM2612/OPN2 with first-pass native LFO Depth, DT1 detune, and generated/renderer-fed DAC byte playback, YM2203/OPN FM plus SSG tone/noise/envelope, YM2608/OPNA FM plus SSG tone/noise/envelope, generated/default or six-fixed-region ADPCM-A rhythm banks, and raw or WAV/AIFF-converted ADPCM-B memory, YM2610/OPNB FM plus SSG tone/noise/envelope, six logical page-packed ADPCM-A regions, and raw or WAV/AIFF-converted ADPCM-B memory, YM2610B/OPNB2 six-FM plus SSG tone/noise/envelope with the same logical ADPCM-A editor/packer, OPL2/OPL3 with rhythm mode, paired 18-channel high-bank layer mode, and shared editable `$104` 4-op pairs covering all four linked algorithms, and YM2151/OPM melodic adapters. Chipper's ADPCM-A codec/bank/state tests are now supplemented by pinned YM2608-LLE stereo fixtures for covered OPNA and OPNB ADPCM-A paths; hardware, broader ADPCM-B, and deeper OPL per-lane/flag/routing validation remain future work.
...
| --- | --- | --- |
| [ymfm](https://github.com/aaronsgiles/ymfm) | YM2612/OPN2 with first-pass native LFO Depth, DT1 detune, and generated/renderer-fed DAC byte playback, YM2203/OPN FM plus SSG tone/noise/envelope, YM2608/OPNA FM plus SSG tone/noise/envelope and generated/default or six-fixed-region ADPCM-A rhythm banks, YM2610/OPNB and YM2610B/OPNB2 with six logical page-packed ADPCM-A regions, raw or WAV/AIFF-converted shared ADPCM-B memory, OPL2/OPL3 rhythm plus paired 18-channel high-bank layer mode and shared editable `$104` 4-op pairs with all four algorithms, and YM2151/OPM now; pinned YM2608-LLE fixtures cover bounded OPNA/OPNB ADPCM-A traces; hardware, ADPCM-B, and deeper OPL3 per-lane/flag/routing validation remain future candidates | BSD-3-Clause; vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a`; core source headers inspected carry BSD-3-Clause, README notes MAME origins/design and high-audible-accuracy rather than cycle-accuracy goals |
...
| [ayumi](https://github.com/true-grue/ayumi) | AY-3-8910 / YM2149 | MIT test oracle pinned at `07c08b4874c359169e4a028edf73f046d8b763e2`; not vendored or linked |
...
| NES + VRC6 | Clean-room expansion on the internal NES core | Furnace, public VRC6 register docs, MAME/Game_Music_Emu as comparison targets only | Keep VRC6 host-attached to NES, preserve clean-room oscillators/register metadata, and add golden/reference comparisons before stronger accuracy claims |
| NES + FDS | Clean-room expansion on the internal NES core | Furnace, NESdev FDS audio docs, MAME/Game_Music_Emu as comparison targets only | Keep FDS host-attached to NES, preserve clean-room wave/modulation register metadata, and add golden/reference comparisons before stronger accuracy claims |
| NES + Sunsoft 5B | Clean-room expansion on the internal NES core | Furnace, public Sunsoft/FME-7 register docs, MAME/Game_Music_Emu as comparison targets only | Keep Sunsoft 5B host-attached to NES, preserve clean-room AY-style tone lanes and pseudo-register metadata, and add PSG noise/envelope plus golden/reference comparisons before stronger accuracy claims |
| NES + MMC5 | Clean-room expansion on the internal NES core | Furnace, public MMC5/NESdev register docs, MAME/Game_Music_Emu as comparison targets only | Keep MMC5 host-attached to NES, preserve clean-room pulse/PCM lanes and pseudo-register metadata, and add PCM-mode, mixer, golden/reference, and hardware comparisons before stronger accuracy claims |
| NES + VRC7 | emu2413 VRC7 patch table inside the internal NES core | Furnace, public VRC7/OPLL docs, NSFPlay, MAME/Game_Music_Emu as comparison targets only | Keep VRC7 host-attached to NES, preserve the MIT emu2413 attribution, expose only original Chipper presets, and add patch-set variants, mixer calibration, golden/reference, and hardware comparisons before stronger accuracy claims |
...
| YM2149 / AY | emu2149 vendored; Ayumi pinned as an independent test oracle | Game_Music_Emu, Furnace, MAME | MIT emu2149 backs the partial PSG core; preserve Chipper's register/macro adapter, keep the Ayumi tone-A reference gate green, and add broader traces plus hardware validation before increasing accuracy claims |
...
| Philips SAA1099 | Current clean-room partial PSG model | Furnace, SAASound, MAME, Philips/Mullard SAA1099 datasheet | Continue clean-room work; use SAASound/MAME/Furnace as comparison and audit targets only unless exact source/file licenses are recorded |
| PC Speaker | Current clean-room partial PIT beeper model | Furnace, DOSBox-style projects, MAME, PCem/86Box, public PIT/port `0x61` documentation | Continue clean-room work; use emulator projects as comparison and audit targets only, and keep the first slice scoped to one speaker lane plus gesture presets rather than a full DOS audio subsystem |
| ZX Spectrum Beeper | Current clean-room partial ULA port FE beeper model | Furnace, Fuse, MAME, SpecEmu-style projects, World of Spectrum port/ULA documentation | Continue clean-room work; use emulator projects as comparison and audit targets only, and keep the first slice scoped to one ULA beeper lane plus gesture presets rather than a full Spectrum machine or AY-128 subsystem |
| YM2612 / Genesis FM | ymfm vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a` | Nuked-OPN2, Furnace, MAME | ymfm backs the verified-partial six-channel melodic adapter with all six source lanes exposed, native LFO rate/AMS/PMS, Operator Tone DT1 writes, and channel-6 DAC playback. The VST and renderer accept raw bytes or rate-preserving WAV/AIFF, schema 9 retains root/trim/center-or-hold metadata through referenced, embedded, and missing-file states, and deterministic gates cover no-copy triggering, source-rate cadence, repeated-byte writes, tail behavior, host-rate invariance, and explicit DAC-only/FM-layered preset ownership. Add an independently generated Nuked-OPN2 or hardware trace, exact bus-cycle scheduling decisions, and analog ladder validation before increasing accuracy claims |
| YM2203 / OPN | ymfm vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a` | Furnace, MAME | ymfm now backs the verified-partial three-channel FM adapter plus embedded SSG tone/noise/envelope lanes with all six source lanes exposed; add timer/prescaler validation and golden references before increasing accuracy claims |
| YM2608 / OPNA | ymfm vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a` | Furnace, MAME | ymfm backs the verified-partial six-channel FM plus embedded SSG adapter. Drum/Hit uses generated or user-owned six-fixed-region ADPCM-A rhythm data; the VST converts per-role WAV/AIFF at the 7.9872 MHz `/432` or `/864` rates, accepts exact-window encoded bytes, previews decoded regions, preserves schema-v8 project fallback, and retains packed-ROM import. Codec/bank vectors follow inspected ymfm predictor/step rules, and a pinned YM2608-LLE stereo fixture independently gates covered BD/Tom decoding, both native rates, addressing, routing, gain, duration, and a required failing nibble mutation. Add broader traces, timer/prescaler validation, analog/hardware evidence, and separate ADPCM-B proof before increasing accuracy claims |
| YM2610 / OPNB | ymfm vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a` | Furnace, MAME | ymfm backs the verified-partial four-channel FM plus embedded SSG adapter. Drum/Hit uses six independently owned logical ADPCM-A regions that accept WAV/AIFF or 256-byte-page-aligned encoded data and pack consecutively into the 1 MiB window without losing hole identity; legacy packed-bank import and raw-or-converted shared ADPCM-B remain supported. A pinned YM2608-LLE YM2610 stereo fixture independently gates covered page-zero/page-two decoding, external addressing, routing, gain, duration, and a required failing misaddress mutation. Add fuller ADPCM-B editing, broader traces, timer/prescaler validation, analog/hardware evidence before increasing accuracy claims |
| YM2610B / OPNB2 | ymfm vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a` | Furnace, MAME | ymfm backs the verified-partial six-channel OPNB2 adapter through the YM2610 six-FM channel mask, with embedded SSG lanes and the same six-logical-region ADPCM-A editor/packer/state and raw-or-converted ADPCM-B workflow as OPNB. Add fuller ADPCM-B editing, timer/prescaler validation, external golden references, and hardware evidence before increasing accuracy claims |
| OPL2 / OPL3 / DOS FM | ymfm vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a` for YMF262/OPL3 | Nuked-OPL3, Furnace | ymfm now backs the verified-partial OPL2/OPL3 adapter with rhythm mode, explicit 18-channel Layer mode, and shared editable `$104` 4-op Pair mode covering all four linked algorithms and native OP1-OP4 level/multiplier/envelope fields. Add deeper OPL3 stereo/flag routing, dedicated 18-card/per-lane editing, golden validation, and hardware references before increasing OPL3 claims |
...
| YM2151 / arcade FM | ymfm vendored at `81aec25ccbb98f4873a255f7551ac4dadac59b4a` | Furnace, MAME | ymfm backs the verified-partial OPM adapter with eight exposed lanes, pan, direct native LFO waveform/PM/AM depth and sensitivity controls, per-operator DT1/DT2 packed with MULT/D2R, `$0F` channel-8 noise, and a dedicated shared four-operator editor; add independent per-channel patches, timer/CSM controls, exact noise validation, and golden comparisons before increasing accuracy claims |
| YM2413 / OPLL | emu2413 vendored; ymfm remains broader FM candidate | Nuked-OPLL, Furnace | emu2413 MIT core is used for the partial OPLL melodic-channel, source-card-labeled native rhythm, and editable user-patch slot 0 adapter; keep Nuked-OPLL reference-only unless licensing changes |
...
Furnace is valuable to Chipper as a broad comparison target because its public README lists direct overlap with nearly every requested Chipper family: NES/RP2A03, Game Boy, SID, AY/YM2149, SN76489, SAA1099, PC Speaker, ZX Spectrum beeper, YM2612, OPL2/OPL3, SNES, POKEY, Amiga, HuC6280, Namco WSG, YM2151, YM2413, and Konami SCC/SCC+. It is also a useful product reference for chip-specific macros, tracker workflows, per-chip export/render behavior, and practical preset vocabulary.
