...
Review status: synced on 2026-07-08 after YM2612/OPN2 added native DT1 detune and renderer-fed channel-6 DAC byte playback on top of the earlier ymfm LFO pass. This file owns source/licensing posture only; implementation status and verification claims belong in `docs/emulation-accuracy.md`, release-gate commands belong in `docs/release-builds.md`, and active execution order belongs in `docs/priority-roadmap.md`.
...
Latest planning sync: 2026-06-29. Latest upstream license/source spot-check: 2026-06-07 from upstream GitHub pages/license files. Do not treat a repository name, README claim, GitHub license badge, or tracker implementation as enough evidence to vendor code. Re-check the exact files and revision before reuse.
...
- ymfm's repository describes BSD-licensed Yamaha FM cores and its top-level `LICENSE` is BSD-3-Clause. A local file/header audit on 2026-06-07 at commit `17decfae857b92ab55fbb30ade2287ace095a381` found BSD-3-Clause headers on the core source files inspected (`src/ymfm*.h`, `src/ymfm*.cpp`, `src/ymfm_fm.ipp`) and README/GeneralInfo provenance notes that the cores began in MAME, use MAME-style design, and are cross-checked against Nemesis/Exodus and Nuked research while explicitly aiming for high audible accuracy rather than cycle accuracy. Chipper now vendors ymfm for verified-partial YM2612/OPN2 with first-pass native LFO Depth, DT1 detune, and generated/renderer-fed DAC byte playback, YM2203/OPN FM plus SSG tone/noise/envelope, YM2608/OPNA FM plus SSG tone/noise/envelope, generated/default or six-fixed-region ADPCM-A rhythm banks, and raw or WAV/AIFF-converted ADPCM-B memory, YM2610/OPNB FM plus SSG tone/noise/envelope, six logical page-packed ADPCM-A regions, and raw or WAV/AIFF-converted ADPCM-B memory, YM2610B/OPNB2 six-FM plus SSG tone/noise/envelope with the same logical ADPCM-A editor/packer, OPL2/OPL3 with rhythm mode, paired 18-channel high-bank layer mode, and shared editable `$104` 4-op pairs covering all four linked algorithms, and YM2151/OPM melodic adapters. Chipper's ADPCM-A codec/bank/state tests follow inspected ymfm predictor/step rules but are not external golden or hardware evidence; golden validation and deeper OPL per-lane/flag/routing validation remain future work.
...
| YM2612 / Genesis FM | ymfm vendored at `17decfae857b92ab55fbb30ade2287ace095a381` | Nuked-OPN2, Furnace, MAME | ymfm now backs the verified-partial six-channel melodic adapter with all six source lanes exposed, first-pass native LFO rate/AMS/PMS controls, native Operator Tone DT1 register writes, and generated or renderer-fed channel-6 DAC drum bytes; add a dedicated operator UI, VST DAC file/state workflow, exact DAC timing, and golden comparisons before increasing accuracy claims |
| YM2203 / OPN | ymfm vendored at `17decfae857b92ab55fbb30ade2287ace095a381` | Furnace, MAME | ymfm now backs the verified-partial three-channel FM adapter plus embedded SSG tone/noise/envelope lanes with all six source lanes exposed; add timer/prescaler validation and golden references before increasing accuracy claims |
| YM2608 / OPNA | ymfm vendored at `17decfae857b92ab55fbb30ade2287ace095a381` | Furnace, MAME | ymfm backs the verified-partial six-channel FM plus embedded SSG adapter. Drum/Hit uses generated or user-owned six-fixed-region ADPCM-A rhythm data; the VST converts per-role WAV/AIFF at the 7.9872 MHz `/432` or `/864` rates, accepts exact-window encoded bytes, previews decoded regions, preserves schema-v8 project fallback, and retains packed-ROM import. Codec/bank vectors follow inspected ymfm predictor/step rules. Add timer/prescaler validation, external golden references, and hardware evidence before increasing accuracy claims |
| YM2610 / OPNB | ymfm vendored at `17decfae857b92ab55fbb30ade2287ace095a381` | Furnace, MAME | ymfm backs the verified-partial four-channel FM plus embedded SSG adapter. Drum/Hit uses six independently owned logical ADPCM-A regions that accept WAV/AIFF or 256-byte-page-aligned encoded data and pack consecutively into the 1 MiB window without losing hole identity; legacy packed-bank import and raw-or-converted shared ADPCM-B remain supported. Add fuller ADPCM-B editing, timer/prescaler validation, external golden references, and hardware evidence before increasing accuracy claims |
| YM2610B / OPNB2 | ymfm vendored at `17decfae857b92ab55fbb30ade2287ace095a381` | Furnace, MAME | ymfm backs the verified-partial six-channel OPNB2 adapter through the YM2610 six-FM channel mask, with embedded SSG lanes and the same six-logical-region ADPCM-A editor/packer/state and raw-or-converted ADPCM-B workflow as OPNB. Add fuller ADPCM-B editing, timer/prescaler validation, external golden references, and hardware evidence before increasing accuracy claims |
| OPL2 / OPL3 / DOS FM | ymfm vendored at `17decfae857b92ab55fbb30ade2287ace095a381` for YMF262/OPL3 | Nuked-OPL3, Furnace | ymfm now backs the verified-partial OPL2/OPL3 adapter with rhythm mode, explicit 18-channel Layer mode, and shared editable `$104` 4-op Pair mode covering all four linked algorithms and native OP1-OP4 level/multiplier/envelope fields. Add deeper OPL3 stereo/flag routing, dedicated 18-card/per-lane editing, golden validation, and hardware references before increasing OPL3 claims |
...
| YM2151 / arcade FM | ymfm vendored at `17decfae857b92ab55fbb30ade2287ace095a381` | Furnace, MAME | ymfm backs the verified-partial OPM adapter with eight exposed lanes, pan, direct native LFO waveform/PM/AM depth and sensitivity controls, per-operator DT1/DT2 packed with MULT/D2R, `$0F` channel-8 noise, and a dedicated shared four-operator editor; add independent per-channel patches, timer/CSM controls, exact noise validation, and golden comparisons before increasing accuracy claims |
