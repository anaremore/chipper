...
`presetQualityTargets()` owns the shippable matrix. Each implemented chip must cover at least twelve chip-local factory presets, at least two presets in each core musical role `Bass`, `Lead`, `Arp`, `Keys / Pad`, `Drums`, and `SFX`, plus at least five chip-family reference tags such as `fm`, `sampler`, `wavetable`, `noise`, `tracker`, or `dmc`. These tags should include signature techniques from the chip family, for example triangle/DMC for NES, duty/wave/noise for DMG, PWM/sync/filter for SID, rhythm/operator feedback for FM, loop/echo for samplers, and LFO/wave/noise for wavetable chips. `chipper_render --list-presets` exports this matrix under `summary.qualityTargets`, including role and reference-tag counts that show where the next original preset pack should go, and `assert_preset_catalog_json.py` fails when the minimum preset count, any required role, any role-density floor, too few reference tags, a reported count mismatch, or any reference tag is missing.
