...
3. **FM Operator Editor**
   - Delivered foundation: YM2612/OPN2, YM2151/OPM, OPN-family FM, and OPL2/OPL3 share editable operator matrices with carrier/modulator roles, multiplier/level/envelope overrides, feedback, resolved register readouts, and chip-appropriate algorithm ownership. OPM also exposes native LFO Depth, waveform, PMS, AMS, DT1, and DT2 with exact register/state evidence. OPL 4-op topology maps both linked `$C0` bits to all four YMF262 algorithms and applies OP1-OP4 controls to the correct physical stages; its per-operator AM/VIB/KSR and KSL choices compose exact `$20/$40` bytes, and its shared Stereo Route writes tested `$F0/$50/$A0` output-select bits across active melodic and rhythm lanes.
   ...
   - Remaining scope: independent per-channel patches, operator enable/mute, native detune for additional FM families, independently addressable OPL A-D output buses, global `$BD` tremolo/vibrato depth, velocity response, dedicated OPL3 18-card editing, and stronger golden/hardware validation.
