...
Wavetable-family source cards follow the same rule. HuC6280, Namco WSG, and SCC cards keep compact labels focused on resolved channel/lane volume, while tooltips and preview scopes expose the native register framing: HuC6280 channel select/control and 32-sample 5-bit RAM, Namco WSG lane frequency/volume/enable register ranges and 32-sample 4-bit RAM, and SCC frequency, `$AA+$ch` volume, `$AF` key bits, and 32-byte wave RAM. The helper functions live in the descriptor layer so editor text, tests, and future dynamic UI metadata can share the same partial-core mapping instead of duplicating chip math in the view.
