...
SID keeps oscillator waveform menus in the three voice cards because waveform selection is a per-voice register decision. The SID Filter module should therefore prioritize filter mode and filter routing instead of repeating a stale voice-wave summary. SID ADSR is per voice, so overrides are displayed as a compact voice-by-column table with explicit Attack, Decay, Sustain, and Release headers, per-voice rows, and envelope previews. Future refinements may make that table roomier or more graphical, but should not collapse SID to one fake global ADSR. Output monitoring should reserve enough height for the final audio scope to be legible as a waveform, not a decorative trace.
