...
YM2612 is the first FM mode to get an algorithm-first UI component. Its Algorithm control uses a dropdown rather than a crowded segmented row because the legal set is Follow plus eight OPN2 algorithm values. The adjacent preview draws the four operators, modulation edges, and carrier outputs from the same `ym2612AlgorithmForPatch` helper used by the engine path, while the tooltip reports the resolved `$B0` algorithm and feedback bits. This is the intended first step toward FM layouts: expose the high-level signal flow immediately, then graduate to deeper per-operator grids for multiplier, total level, envelope rates, SSG-EG, LFO sensitivity, DAC, and chip-variant artifacts.

The other FM chips should not inherit the YM2612 layout blindly. OPL2/OPL3 needs an operator-pair layout that can morph between 2-operator melodic, OPL3 4-operator pair, and rhythm modes, with waveform grids for the OPL mathematical waveforms and per-operator vibrato/tremolo/KSR/KSL flags. YM2151/OPM should reuse the four-operator algorithm preview idea but add an arcade-specific LFO/noise hub, DT1/DT2 detune controls, eight-channel voice allocation, and channel-8 noise behavior. YM2413/OPLL should be simpler: a ROM-instrument/channel mixer first, then a single shared Voice 0 custom two-operator editor that makes it clear all channels using Custom share the same programmable patch slot.
