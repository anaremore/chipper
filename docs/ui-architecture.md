...
FM-family source cards should show useful native state even before the full operator editor lands. YM2612/OPN2 and YM2151/OPM cards expose resolved algorithm and feedback choices; OPL2 cards expose resolved waveform, connection, and feedback; YM2413/OPLL cards expose the resolved ROM instrument number and volume nibble. These readouts are not a substitute for the future operator-grid UI, but they make the current audible presets inspectable and keep the visible surface tied to the register writes sent into ymfm or emu2413.
