...
Wavetable and sampler chips also need distinct structural templates. HuC6280 should grow toward a 32-step by 5-bit micro-wavetable editor, six-channel mixer, channel 1/2 FM-LFO pairing, and channel 5/6 noise controls. Paula should use four hard-panned sample strips with fixed left/right clustering, period/volume registers, optional modern width, hardware LED/static-filter controls, and AM/FM channel-pair modulation. POKEY should use a four-channel deck centered on AUDC distortion/poly counters, 16-bit channel-pair switches, clock-rate choices, and high-pass pair routing rather than generic oscillator controls.

Wavetable-family source cards follow the same rule. HuC6280, Namco WSG, and SCC cards show the resolved wave-template family and first/last Wave RAM values directly in each card label, while tooltips and preview scopes expose the native register framing: HuC6280 channel select/control and 32-sample 5-bit RAM, Namco WSG lane frequency/volume/enable register ranges and 32-sample 4-bit RAM, and SCC frequency, `$AA+$ch` volume, `$AF` key bits, and 32-byte wave RAM. SPC700 and Paula sample cards use the same pattern: each card names the generated sample template and two sample checkpoints so a selected preset immediately communicates the sample shape without requiring a hover. The helper functions live in the descriptor layer so editor text, tests, and future dynamic UI metadata can share the same partial-core mapping instead of duplicating chip math in the view.

Some implemented chips currently have more native lanes than the four-card generic source surface can edit. SPC700-style exposes four of eight sample voices, HuC6280 four of six wavetable channels, Namco WSG four of eight lanes, YM2612 renders all six FM channels while directly exposing four source strips, YM2151 exposes four of eight FM channels, YM2413 four of nine melodic channels, and SCC four of five wavetable channels. The descriptor layer owns these visible/native counts, the Sources module summary and Performance readout disclose the mismatch, and tests pin the contract until those chips receive dedicated multi-lane layouts.
...
`chipper_render --list-descriptors --debug descriptors.json` exports the same chip descriptors, macro templates, parameter specs, visible/native source-lane counts, default MIDI CC map, and factory preset catalog without rendering audio. `chipper_render --describe-chip nes --debug nes.json` exports the same schema for one chip. This is the machine-readable contract for future dynamic UI work: chip-aware controls should be proven in descriptor metadata before the editor grows more chip-specific surfaces.
