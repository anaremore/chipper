...
NES, DMG, and YM2149 source cards are the first stable non-macro channel controls. They attach to universal `source1Enabled` through `source4Enabled` parameters and map to native source behavior: pulse 1, pulse 2, triangle/wave, and noise for APU-style chips; channel A, channel B, channel C, and shared noise for YM2149/AY. The YM cards drive register 7 mixer bits and source-aware volume writes, so muted channels cannot leak accidental constant output. Future chip source cards should preserve that user promise: active cards must represent audible/native sources, not decorative labels.
