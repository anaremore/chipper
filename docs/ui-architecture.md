...
SID Sustain uses `macroControl4` as the musical SR sustain source, SID ADSR Speed uses `envelopeDecay` for macro attack/decay/release templates, and each SID voice has its own Attack, Decay, Sustain, and Release 17-choice menus for `Follow` plus exact 0-15 AD/SR nibble overrides. Each Sustain menu's `Follow` choice uses the `macroControl4` Sustain slider, while explicit values write that voice's SR high nibble directly. Readouts show the resolved V1/V2/V3 AD/SR register bytes and A/D/S/R nibbles so both the shared musical control and exact per-voice overrides feel chip-aware. Future SID envelope UI should move these per-voice ADSR controls under each voice as vertical stepped sliders, but the current compact menus preserve exact register control without overcrowding the voice cards.
