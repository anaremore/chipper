...
2. **BRR Playback Path**
   ...
   - Done first pass: the command-line renderer can map a MIDI note from a root key to an inline BRR bank slot, matching the plugin's note-map workflow for regression tests.
