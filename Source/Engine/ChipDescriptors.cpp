...
std::vector<ChipParameterSpec> huc6280ParameterSpecs()
{
    return {
        ...
        envelopeSpec("huc6280.decay", "Global Amp Env", "Applies one shared Chipper volume-gate helper over HuC6280 channel volume; native per-channel 5-bit volume state remains visible in the voice cards and debug output."),
        segmentedSpec(ChipParameterRole::dmgStereoRoute,
                      "huc6280.lfoPair",
                      "Ch 1/2 LFO",
                      "Motion",
...
std::vector<ChipParameterSpec> sccParameterSpecs()
{
    return {
        ...
        envelopeSpec("scc.decay", "Global Amp Env", "Applies one shared Chipper volume-gate helper over SCC channel volume; native per-channel key and 4-bit volume registers remain visible in the voice cards and debug output."),
...
std::vector<ChipParameterSpec> namcoWsgParameterSpecs()
{
    return {
        ...
        envelopeSpec("namcoWsg.decay", "Global Amp Env", "Applies one shared Chipper volume-gate helper over Namco WSG lane volume; native per-lane 4-bit volume and enable state remain visible in the lane cards and debug output."),
    ...
    static const std::vector<ChipDescriptor> items {
        ...
        },
        {
            ...
            },
            {
                ...
                makeModule("envelope", "Global Amp Env", "Shared musical helper over HuC6280 channel volume; not native ADSR.", { "5-bit volume", "Per-channel trims", "Gate helper", "Register readout" }),
