            ...
            verifiedPartial(
                {
                    ...
                    "Source enables, source levels, sample-shape choices, envelope-shape choices, BRR block/end/loop debug metadata, BRR loop-start debug metadata, plugin sample slot selection/state recall, note-mapped sample bank selection, noise source mode/clock/mask metadata, pitch-motion, musical echo-helper debug metadata, envelope-release debug metadata, interpolation metadata, chip-poly allocation across all eight exposed voices, presets, and debug JSON are covered by automated tests.",
                    "No third-party SPC700, S-DSP, BRR, SNES, or tracker-player source code is vendored in this clean-room partial model."
                },
                {
                    "Source directory/sample memory addressing, exact S-DSP source-directory loop address behavior, exact S-DSP noise clocking/NON edge behavior, hardware pitch modulation, exact S-DSP Gaussian interpolation table behavior, FIR echo, exact ADSR/gain envelope timing, and SPC700 CPU timing are not implemented.",
