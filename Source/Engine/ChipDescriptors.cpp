...
std::vector<ChipParameterSpec> nesParameterSpecs()
{
    return {
        ...
        sliderSpec(ChipParameterRole::nesDmcDirectLevel,
                   ...
                   "DMC Direct Level",
                   "DMC",
                   "Maps to the RP2A03 $4011 7-bit DMC direct DAC load. Renderer playback can also step external .dmc bytes; VST sample directory/import UI remains planned.",
                   ParameterKind::chipRegister),
        { ChipParameterRole::nesDmcSampleSlot,
          "nes.dmcSampleSlot",
          "DMC Sample Slot",
          "DMC",
          "Selects one preloaded .dmc sample from the current user-loaded DMC bank. MIDI CC changes switch the selected in-memory slot; WAV-to-DMC import is planned.",
    ...
    static const std::vector<ChipDescriptor> items {
        {
            ...
            verifiedPartial(
                ...
                },
                {
                    ...
                    "User-facing DMC sample directory/import UI remains planned; renderer tests use generated bytes and no bundled sample content."
                })
        },
        {
            ChipMode::dmg,
