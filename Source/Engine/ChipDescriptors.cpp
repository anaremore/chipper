...
std::vector<ChipParameterSpec> nesParameterSpecs()
{
    return {
        ...
        { ChipParameterRole::nesDmcLoop,
          ...
          "DMC Loop",
          "DMC",
          "Maps to the RP2A03 $4010 loop bit. Off steps the selected DMC sample once and leaves the DAC holding its final level; Loop repeats from the first byte.",
          ParameterKind::chipRegister,
...
std::vector<ChipParameterSpec> huc6280ParameterSpecs()
{
    return {
        ...
        envelopeSpec("huc6280.decay", "Shared Volume Gate", "Applies one shared Chipper volume-gate helper over HuC6280 channel volume; native per-channel 5-bit volume state remains visible in the voice cards and debug output."),
        segmentedSpec(ChipParameterRole::dmgStereoRoute,
                      "huc6280.lfoPair",
                      "Ch 1/2 LFO",
                      "Motion",
...
std::vector<ChipParameterSpec> sccParameterSpecs()
{
    return {
        ...
        envelopeSpec("scc.decay", "Shared Volume Gate", "Applies one shared Chipper volume-gate helper over SCC channel volume; native per-channel key and 4-bit volume registers remain visible in the voice cards and debug output."),
...
std::vector<ChipParameterSpec> namcoWsgParameterSpecs()
{
    return {
        ...
        envelopeSpec("namcoWsg.decay", "Shared Volume Gate", "Applies one shared Chipper volume-gate helper over Namco WSG lane volume; native per-lane 4-bit volume and enable state remain visible in the lane cards and debug output."),
    ...
    static const std::vector<ChipDescriptor> items {
        ...
        },
        {
            ...
            },
            {
                ...
                makeModule("envelope", "Shared Volume Gate", "Shared musical helper over HuC6280 channel volume; not native ADSR.", { "5-bit volume", "Per-channel trims", "Gate helper", "Register readout" }),
        ...
        },
        {
            ...
            },
            {
                ...
                makeModule("envelope", "Shared Volume Gate", "Shared musical helper over Namco WSG lane volume; not native ADSR.", { "4-bit lane volume", "Enable mask", "Gate helper", "Register readout" }),
        ...
        },
        {
            ...
            },
            {
                ...
                makeModule("wave", "Wave / Mixer", "Wave RAM plus frequency and volume behavior.", { "Wave shape", "Wave skew", "4-bit volume", "Pitch periods" }),
                makeModule("envelope", "Shared Volume Gate", "Shared musical helper over SCC channel volume and key state; not native ADSR.", { "4-bit channel volume", "Key mask", "Gate helper", "Register readout" }),
