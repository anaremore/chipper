...
std::array<ModuleDescriptor, 6> ym2612Modules()
{
    return std::array<ModuleDescriptor, 6> {
        ...
        makeModule("output", "Envelope, DAC + Routing", "Ch 6 DAC uses generated bytes or a VST/renderer-loaded WAV, AIFF, or raw unsigned 8-bit sample with portable path recall. Envelope, LFO, and pan remain native register paths.", { "Envelope shape", "Ch 6 FM / DAC", "$22 + AMS/PMS", "$B4 pan" })
    };
}
