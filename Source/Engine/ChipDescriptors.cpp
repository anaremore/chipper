...
std::vector<ChipParameterSpec> ym2413ParameterSpecs()
{
    return {
        ...
        sliderSpec(ChipParameterRole::macroControl2,
                   ...
                   "Tuning Offset",
                   "Pitch",
                   "Offsets every melodic OPLL channel from -6 to +6 semitones while preserving native f-number/block register writes."),
        sliderSpec(ChipParameterRole::macroControl3,
                   "ym2413.motion",
                   "Motion",
                   "Motion",
...
std::vector<ChipParameterSpec> zxSpectrumBeeperParameterSpecs()
{
    return {
        sliderSpec(ChipParameterRole::macroControl1,
                   ...
                   "Duty + Border",
                   "Port $FE bits 2-0",
                   "Biases Chipper's generated one-bit duty color and sets the low three ULA border bits reported in debug metadata; the Spectrum has no native duty register."),
        sliderSpec(ChipParameterRole::macroControl2,
...
std::array<ModuleDescriptor, 6> sn76489Modules()
{
    return std::array<ModuleDescriptor, 6> {
        ...
        makeModule("sources", "Channels", "Three compact square-tone generators plus a deeper noise path; Tone 3 can clock Noise.", { "Tone 1", "Tone 2", "Tone 3 / noise clock", "Noise register + attenuation" }),
...
std::array<ModuleDescriptor, 6> ym2612Modules()
{
    return std::array<ModuleDescriptor, 6> {
        ...
        makeModule("output", "DAC Lab + Routing", "Ch 6 DAC uses distinct generated recipes or a bounded VST/renderer-loaded WAV, AIFF, or raw unsigned 8-bit sample. The Lab reports rate, root, trim, center/hold tail state, waveform, portable path recall, and project fallback while envelope, LFO, and pan remain native register paths.", { "DAC source + metadata", "Ch 6 FM / DAC", "$22 + AMS/PMS", "$B4 pan" })
    };
}
...
std::array<ModuleDescriptor, 6> ym2610bModules()
{
    return {
        ...
        makeModule("sources", "Six FM + Three SSG Lanes", "Six OPNB2 FM channels and three embedded SSG lanes stay visible. In Drum/Hit, FM1-6 controls also own external ADPCM-A segments 1-6.", { "FM 1-6", "SSG A-C", "Nine-lane allocation", "ADPCM-A ownership" }),
...
const std::vector<ChipDescriptor>& descriptors()
{
    static const std::vector<ChipDescriptor> items {
        ...
        },
        {
            ChipMode::sn76489,
            "SN76489 / Sega PSG",
            "Three square-tone lanes plus one noise lane. Noise selects periodic/white feedback and an internal divider or Tone 3 clock; Chip Poly allocates only the three tone lanes.",
            {
                { "stack", "Tone Stack", "Channels", "Sets interval spread across the three tone channels." },
                { "motion", "Pitch Motion", "Pitch", "Scales macro pitch movement." },
                { "noise", "Preset Noise Bias", "Noise", "Biases the preset-resolved noise register; explicit Noise Mode choices disable it." },
                { "level", "Native Noise Level", "Noise", "Writes the noise channel's 4-bit attenuation value before the modern source trim." },
            },
            sn76489Modules(),
            sn76489Macros(),
            true,
        ...
        },
        {
            ...
            },
            {
                ...
                makeModule("sources", "One ULA Output Path", "The single source card is the complete audible ZX Spectrum beeper lane; no Chip Poly, AY hardware, or hidden oscillator bank is implied.", { "ULA port $FE", "EAR bit 4", "MIC bit 3", "Modern trim" }),
                makeModule("tone", "CPU Toggles -> ULA Port $FE -> EAR / MIC -> Beeper", "Beeper Mode chooses the generated EAR/MIC bit path; duty, motion, MIC grit, drive, and helper decay shape that same one-bit lane.", { "EAR bit 4 tone", "MIC bit 3 click", "Border bits 2-0", "One-bit speaker drive" }),
            ...
            verifiedPartial(
                {
                    ...
                    "Channel-6 DAC mode enables $2B and streams distinct generated recipes or bounded raw/WAV/AIFF unsigned bytes through $2A; WAV/AIFF source rate, incoming-note root pitch, trim, and center/hold tail state are explicit.",
                    "Descriptor, MIDI CC, both-width editor, schema-9 processor migration/state, renderer decode, source gating, LFO/DT1, Chip Poly, repeated DAC writes, source-rate/host-rate cadence, trim/tail checksums, and explicit Kick/Snare/Chord key-mask tests cover the bounded adapter."
                },
                {
                    "Independent Nuked-OPN2 or hardware DAC traces, exact bus-cycle scheduling, ladder/analog validation, richer interactive sample editing, deeper LFO waveform/modulation control, SSG-EG quirks, and timers are not complete.",
        ...
        },
        {
            ...
            },
            {
                ...
                makeModule("sources", "Nine OPLL Lanes", "All nine OPLL melodic channels are exposed as playable lanes; rhythm mode repurposes source cards 7-9 as BD, HH+SD, and TOM+CYM mixer slots.", { "Ch 1-6 melodic", "Ch 7 / BD", "Ch 8 / HH+SD", "Ch 9 / TOM+CYM", "Chip Poly" }),
                makeModule("instrument", "Instrument + Topology", "Choose a shared ROM instrument or User0 patch and decide whether lanes 7-9 remain melodic or become the native OPLL rhythm set.", { "ROM preset instruments", "User $00-$07", "$0E rhythm mode", "Key-on" }),
                makeModule("envelope", "Shared User0 Patch", "The single programmable OPLL patch slot is shared by every melodic lane using User0; Mod and Carrier expose ML/TL/feedback and AR/DR/SL/RR fields.", { "Shared User0", "Modulator", "Carrier", "$00-$07" }),
            ...
            verifiedPartial(
                {
                    ...
                    "A pinned YM2608-LLE stereo fixture independently gates covered OPNA ADPCM-A BD/Tom decoding, both native rates, addressing, routing, gain, duration, and a required failing nibble mutation.",
                    "Descriptor, MIDI CC, renderer smoke, source gating, and Chip Poly regression tests cover the nine-lane FM plus SSG adapter."
                },
                {
                    "Full ADPCM-B editing, broader ADPCM-A traces, timers, prescaler behavior, CSM, analog output, and hardware validation remain future work; the independent emulator gate is not a hardware-golden claim.",
            ...
            verifiedPartial(
                {
                    ...
                    "A pinned YM2608-LLE YM2610 stereo fixture independently gates covered ADPCM-A page-zero/page-two decoding, external addressing, routing, gain, duration, and a required failing misaddress mutation.",
        ...
        },
        {
            ...
            verifiedPartial(
                ...
                {
                    "The shared YM2610 ADPCM-A path has an independent emulator gate in the four-FM YM2610 configuration, but OPNB2 is not separately captured; full ADPCM-B editing, broader traces, timers, prescaler behavior, CSM, analog output, and hardware validation remain future work.",
                    "Prescaler controls, timers, CSM, LFO/AMS/PMS, golden emulator comparison, hardware capture comparison, and cycle accuracy are not complete."
                })
        }
    };
