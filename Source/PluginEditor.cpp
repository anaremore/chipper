...
constexpr int editorMaxHeight = editorSpc700Height;
...
int preferredEditorHeightForMode(chipper::ChipMode mode)
{
    ...
    if (mode == chipper::ChipMode::ym2608)
        return editorYm2608Height;
    if (mode == chipper::ChipMode::ym2610)
        return editorYm2610Height;
    if (mode == chipper::ChipMode::ym2610b)
        return editorYm2610bHeight;
    if (mode == chipper::ChipMode::namcoWsg)
        return editorNamcoWsgHeight;
    if (mode == chipper::ChipMode::scc)
        return editorSccHeight;
...
{
    static constexpr std::array<chipper::ChipParameterRole, 4> roles {
        chipper::ChipParameterRole::fmOperator1Multiplier,
        chipper::ChipParameterRole::fmOperator2Multiplier,
        chipper::ChipParameterRole::fmOperator3Multiplier,
        chipper::ChipParameterRole::fmOperator4Multiplier
    };
...
chipper::ChipParameterRole fmOperatorDecayRateRole(size_t index)
{
    static constexpr std::array<chipper::ChipParameterRole, 4> roles {
        chipper::ChipParameterRole::fmOperator1DecayRate,
        chipper::ChipParameterRole::fmOperator2DecayRate,
        chipper::ChipParameterRole::fmOperator3DecayRate,
        chipper::ChipParameterRole::fmOperator4DecayRate
    };

    return roles[std::min(index, roles.size() - 1u)];
}

const char* fmOperatorDecayRateParameterId(size_t index)
{
    static constexpr std::array<const char*, 4> ids {
...
public:
        ...
        : audioProcessor(processor), spc700Mode(editsSpc700Brr), paulaMode(editsPaulaSamples), onRefresh(std::move(refreshCallback))
    {
        ...
        helper.setText(paulaMode
                           ? "Checked WAV/AIFF/8SVX/MOD samples feed the 32-slot Paula-style sample bank and note map. Folder contents stay local."
                           : spc700Mode
      ...
      focusOutline(*this)
{
    ...
    dmcSampleFileButton.onClick = [this]
    {
        if (displayedMode == chipper::ChipMode::ym2612)
            chooseOpn2DacSampleFile();
        else if (displayedMode == chipper::ChipMode::ym2608)
...
void ChipperAudioProcessorEditor::applyChipTheme()
{
    ...
    for (auto* box : std::array<juce::ComboBox*, 14> {
             &presetFilterBox, &presetBox, &chipModeBox, &accuracyBox, &macroBox, &playModeBox, &dmcRateBox,
...
void ChipperAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    if (usesSourceChannelSurface(displayedMode))
    {
        for (size_t index = 0; index < sourceChannelBounds.size(); ++index)
        {
            ...
            const auto isEmbeddedSsgLane = (displayedMode == chipper::ChipMode::ym2203 && index >= 3u)
                || (displayedMode == chipper::ChipMode::ym2608 && index >= 6u);
            g.setColour((isEmbeddedSsgLane ? theme.primary : theme.accent).withAlpha(0.70f));
...
void ChipperAudioProcessorEditor::resized()
{
    ...
    const auto namcoWsgLayout = displayedMode == chipper::ChipMode::namcoWsg;
    const auto sccLayout = displayedMode == chipper::ChipMode::scc;
    const auto ym2612Layout = displayedMode == chipper::ChipMode::ym2612;
    ...
    const auto ym2608Layout = displayedMode == chipper::ChipMode::ym2608;
    const auto ym2610Layout = displayedMode == chipper::ChipMode::ym2610;
    const auto ym2610bLayout = displayedMode == chipper::ChipMode::ym2610b;
    const auto nineLaneOpnSsgLayout = ym2608Layout || ym2610bLayout;
    const auto opnaOpnbLayout = nineLaneOpnSsgLayout || ym2610Layout;
    ...
    if (sidLayout)
    {
        ...
        const auto voiceHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getHeight()) * 0.33)), 186, 190);
        const auto filterHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getHeight()) * 0.17)), 96, 100);
        const auto envelopeHeight = std::max(0, availableRowsHeight - voiceHeight - filterHeight);
        const auto topY = modules.getY();
        const auto filterY = topY + voiceHeight + gap;
        const auto envelopeY = filterY + filterHeight + gap;

        moduleBounds[0] = {};
        moduleBounds[1] = { modules.getX(), topY, modules.getWidth(), voiceHeight };
        moduleBounds[2] = { modules.getX(), filterY, modules.getWidth(), filterHeight };
    ...
    {
        const auto availableHeight = modules.getHeight();
        const auto topRowHeight = std::min(114, availableHeight);
        const auto remainingHeight = std::max(0, availableHeight - topRowHeight - (gap * 2));
        const auto sourceRowHeight = std::clamp(remainingHeight - 200, 238, 254);
        const auto sampleRowHeight = std::max(0, remainingHeight - sourceRowHeight);
    ...
    {
        const auto topRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getHeight()) * 0.61)), 245, 255);
        const auto bottomRowHeight = std::max(0, modules.getHeight() - topRowHeight - gap);
        const auto topY = modules.getY();
        const auto bottomY = topY + topRowHeight + gap;
    ...
    else if (ym2149Layout)
    {
        ...
        const auto middleRowHeight = std::max(0, availableHeight - sourceRowHeight - gap);
    ...
    {
        const auto availableHeight = modules.getHeight();
        const auto sourceRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.46)), 250, 254);
        const auto relationshipRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.24)), 130, 134);
        const auto textureRowHeight = std::max(0, availableHeight - sourceRowHeight - relationshipRowHeight - (gap * 2));
        const auto topY = modules.getY();
        const auto relationshipY = topY + sourceRowHeight + gap;
        const auto textureY = relationshipY + relationshipRowHeight + gap;

        moduleBounds[0] = {};
        moduleBounds[1] = { modules.getX(), topY, modules.getWidth(), sourceRowHeight };
        moduleBounds[2] = { modules.getX(), relationshipY, modules.getWidth(), relationshipRowHeight };
        moduleBounds[3] = { modules.getX(), textureY, modules.getWidth(), textureRowHeight };
        moduleBounds[4] = {};
        moduleBounds[5] = {};
    }
    ...
    {
        const auto availableHeight = modules.getHeight();
        const auto sourceRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.22)), 124, 132);
        const auto middleRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.49)), 274, 288);
        const auto bottomRowHeight = std::max(0, availableHeight - sourceRowHeight - middleRowHeight - (gap * 2));
        const auto patchWidth = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getWidth()) * 0.39)), 430, 520);
        const auto topY = modules.getY();
        const auto middleY = topY + sourceRowHeight + gap;
    ...
    else if (ym2151Layout)
    {
        ...
        moduleBounds[3] = { modules.getX() + patchWidth + gap, middleY, modules.getWidth() - patchWidth - gap, middleRowHeight };
        moduleBounds[4] = {};
        moduleBounds[5] = { modules.getX(), bottomY, modules.getWidth(), routingRowHeight };
    }
    else if (ym2413Layout)
    {
        const auto availableHeight = modules.getHeight();
        const auto sourceRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.50)), 276, 284);
        const auto lowerRowHeight = std::max(0, availableHeight - sourceRowHeight - gap);
        const auto instrumentWidth = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getWidth()) * 0.40)), 450, 510);
        const auto topY = modules.getY();
        const auto lowerY = topY + sourceRowHeight + gap;

        moduleBounds[0] = {};
        moduleBounds[1] = { modules.getX(), topY, modules.getWidth(), sourceRowHeight };
        moduleBounds[2] = { modules.getX(), lowerY, instrumentWidth, lowerRowHeight };
        moduleBounds[3] = { modules.getX() + instrumentWidth + gap, lowerY, modules.getWidth() - instrumentWidth - gap, lowerRowHeight };
        moduleBounds[4] = {};
        moduleBounds[5] = {};
    }
    ...
    else if (nineLaneOpnSsgLayout)
    {
        const auto availableHeight = modules.getHeight();
        const auto sourceRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.49)), 304, 308);
        const auto lowerRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.17)), 104, 108);
        const auto middleRowHeight = std::max(0, availableHeight - sourceRowHeight - lowerRowHeight - (gap * 2));
        const auto patchWidth = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getWidth()) * 0.40)), 452, 520);
        const auto ssgWidth = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getWidth()) * 0.43)), 488, 560);
        const auto topY = modules.getY();
        const auto middleY = topY + sourceRowHeight + gap;
        const auto bottomY = middleY + middleRowHeight + gap;
        ...
        moduleBounds[2] = { modules.getX(), middleY, patchWidth, middleRowHeight };
        moduleBounds[3] = { modules.getX() + patchWidth + gap, middleY, modules.getWidth() - patchWidth - gap, middleRowHeight };
        moduleBounds[4] = { modules.getX(), bottomY, ssgWidth, lowerRowHeight };
        moduleBounds[5] = { modules.getX() + ssgWidth + gap, bottomY, modules.getWidth() - ssgWidth - gap, lowerRowHeight };
    }
    else if (ym2610Layout)
    {
        const auto availableHeight = modules.getHeight();
        const auto sourceRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.38)), 236, 240);
        const auto lowerRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(availableHeight) * 0.18)), 110, 114);
        const auto middleRowHeight = std::max(0, availableHeight - sourceRowHeight - lowerRowHeight - (gap * 2));
        const auto patchWidth = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getWidth()) * 0.40)), 452, 520);
        const auto ssgWidth = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getWidth()) * 0.40)), 452, 520);
        const auto topY = modules.getY();
        const auto middleY = topY + sourceRowHeight + gap;
        const auto bottomY = middleY + middleRowHeight + gap;
    ...
    else if (wavetableLayout)
    {
        ...
        constexpr auto targetCardHeight = 106;
        const auto desiredSourceHeight = sourceHeaderReserve + (targetCardHeight * sourceRows) + (gap * (sourceRows - 1));
        const auto sourceMaximumHeight = std::min(264, std::max(252, availableHeight - bottomRowHeight - gap));
    ...
    const auto useOplVoiceGrid = opl3Layout && visibleSourceCards > 6u;
    const auto useOpmVoiceGrid = ym2151Layout && visibleSourceCards > 4u;
    const auto useOpllVoiceGrid = ym2413Layout && visibleSourceCards > 6u;
    ...
    const auto compactSourceGrid = useSpc700VoiceGrid || usePaulaVoiceGrid || useWavetableVoiceGrid || useNesExpansionVoiceGrid || useOplVoiceGrid || useOpmVoiceGrid || useOpllVoiceGrid || ym2203Layout || opnaOpnbLayout || saa1099Layout || pokeyLayout || oneBitHardwarePathLayout;
    ...
    if (spc700Layout)
    {
        ...
        for (const auto moduleIndex : { 2u, 3u })
            for (auto& itemLabel : moduleItemLabels[moduleIndex])
                itemLabel.setBounds({});
    }
    if (pokeyLayout)
    {
    ...
    constexpr int ymSharedNoiseCardHeight = 78;
    juce::Rectangle<int> ymSharedNoiseSourceBounds;
    if (ym2149Layout)
    {
        auto sharedNoisePanel = moduleBounds[2].reduced(12, 9);
        sharedNoisePanel.removeFromTop(20);
        if (moduleSummaryLabels[2].isVisible())
            sharedNoisePanel.removeFromTop(30);
        sharedNoisePanel.removeFromTop(moduleSummaryLabels[2].isVisible() ? 4 : 8);
        ymSharedNoiseSourceBounds = sharedNoisePanel.removeFromTop(std::min(ymSharedNoiseCardHeight, sharedNoisePanel.getHeight()));
    }
    ...
    const auto sourceColumns = ym2610Layout ? 4 : (pokeyLayout ? 2 : ((ym2149Layout || ym2203Layout || nineLaneOpnSsgLayout || saa1099Layout || useOplVoiceGrid || useOpllVoiceGrid) ? 3 : ((useSpc700VoiceGrid || useOpmVoiceGrid) ? 4 : (usePaulaVoiceGrid ? paulaColumns : (useNesExpansionVoiceGrid ? nesExpansionColumns : (useWavetableVoiceGrid ? wavetableColumns : static_cast<int>(visibleSourceCards)))))));
    const auto sourceRows = (useSpc700VoiceGrid || usePaulaVoiceGrid || useWavetableVoiceGrid || useNesExpansionVoiceGrid || useOplVoiceGrid || useOpmVoiceGrid || useOpllVoiceGrid || ym2203Layout || opnaOpnbLayout || saa1099Layout || pokeyLayout)
    ...
    const auto sourceCardHeight = displayedMode == chipper::ChipMode::sid
        ...
        : ((huc6280Layout || namcoWsgLayout || sccLayout)
    ...
    for (size_t i = 0; i < sourceChannelBounds.size(); ++i)
    {
        ...
        const auto sourceColumn = (useSpc700VoiceGrid || usePaulaVoiceGrid || useWavetableVoiceGrid || useNesExpansionVoiceGrid || useOplVoiceGrid || useOpmVoiceGrid || useOpllVoiceGrid || ym2203Layout || opnaOpnbLayout || saa1099Layout || pokeyLayout) ? static_cast<int>(i % static_cast<size_t>(sourceColumns)) : static_cast<int>(i);
        const auto sourceRow = (useSpc700VoiceGrid || usePaulaVoiceGrid || useWavetableVoiceGrid || useNesExpansionVoiceGrid || useOplVoiceGrid || useOpmVoiceGrid || useOpllVoiceGrid || ym2203Layout || opnaOpnbLayout || saa1099Layout || pokeyLayout) ? static_cast<int>(i / static_cast<size_t>(sourceColumns)) : 0;
        const auto sourceGridOffsetX = ((sccLayout && visibleSourceCards == 5u) || (ym2610Layout && visibleSourceCards == 7u)) && sourceRow == 1
        ...
        const auto cardHeightForChannel = displayedMode == chipper::ChipMode::sn76489 && i < 3u
            ...
            : sourceCardHeight;
        sourceChannelBounds[i] = ym2149Layout && i == 3u
            ? ymSharedNoiseSourceBounds
            : juce::Rectangle<int> {
                sourcePanel.getX() + sourceGridOffsetX + (sourceColumn * (sourceCardWidth + sourceGap)),
                sourcePanel.getY() + (sourceRow * (sourceCardHeight + sourceGap)),
                sourceCardWidth,
                cardHeightForChannel
            };
        ...
        const auto isEmbeddedSsgSourceCard = isYm2149ToneSourceCard || isYm2203SsgSourceCard || isYm2608SsgSourceCard || isYm2610SsgSourceCard;
        ...
        const auto previewHeight = (useOplVoiceGrid || useOpmVoiceGrid || useOpllVoiceGrid)
            ...
            ? ((isYm2608SsgSourceCard || isYm2610SsgSourceCard)
        ...
        const auto placeEmbeddedLevelInArea = [this, i](juce::Rectangle<int> levelArea, int labelWidth = 52)
        {
            ...
            const auto needsAllRemainingHeight = compact && levelArea.getHeight() <= minimumSliderHeight;
            if (! needsAllRemainingHeight)
        ...
        if (isSidSourceCard && i < sidVoiceWaveCount)
        {
            ...
            auto pulseWidthRow = sourceCard.removeFromTop(std::min(15, sourceCard.getHeight()));
        ...
        else if (isSnSourceCard && i == 3)
        {
            ...
            const auto placeNoiseRegisterSlider = [&](size_t controlIndex)
            {
                nativeGroupLabels[controlIndex].setBounds({});
                auto header = sourceCard.removeFromTop(std::min(14, sourceCard.getHeight()));
                nativeLabels[controlIndex].setJustificationType(juce::Justification::centredLeft);
                nativeLabels[controlIndex].setMinimumHorizontalScale(0.70f);
                nativeLabels[controlIndex].setBounds(header.removeFromLeft(std::min(116, header.getWidth())));
                controlValueLabels[controlIndex].setJustificationType(juce::Justification::centredRight);
                controlValueLabels[controlIndex].setMinimumHorizontalScale(0.62f);
                controlValueLabels[controlIndex].setBounds(header);
                nativeSliders[controlIndex].setBounds(sourceCard.removeFromTop(std::min(20, sourceCard.getHeight())));
                sourceCard.removeFromTop(std::min(3, sourceCard.getHeight()));
            };

            placeNoiseRegisterSlider(2);
            placeNoiseRegisterSlider(3);
        }
        else if (isOpmSourceCard && i == 7u)
        {
            auto noiseArea = sourceCard.removeFromTop(std::min(36, sourceCard.getHeight()));
        ...
        else if (isWavetableSourceCard && i < hucVoiceWaveBoxes.size())
        {
            ...
            sourceCard.removeFromTop(std::min(3, sourceCard.getHeight()));
            auto levelArea = (huc6280Layout || namcoWsgLayout || sccLayout)
    ...
    {
        auto toneGroup = juce::Rectangle<int> {
            sourceChannelBounds[0].getX(),
            sourceChannelBounds[0].getBottom() + 8,
            sourceChannelBounds[2].getRight() - sourceChannelBounds[0].getX(),
            std::max(0, sourcePanel.getBottom() - sourceChannelBounds[0].getBottom() - 8)
        };
        auto groupHeader = toneGroup.removeFromTop(std::min(14, toneGroup.getHeight()));
        nativeGroupLabels[0].setText("Tone Group", juce::dontSendNotification);
        nativeGroupLabels[0].setBounds(groupHeader);
        nativeGroupLabels[1].setBounds({});
        toneGroup.removeFromTop(std::min(2, toneGroup.getHeight()));

        constexpr int toneGroupGap = 10;
        const auto toneControlWidth = std::max(0, (toneGroup.getWidth() - toneGroupGap) / 2);
        auto stackControl = toneGroup.removeFromLeft(std::min(toneControlWidth, toneGroup.getWidth()));
        toneGroup.removeFromLeft(std::min(toneGroupGap, toneGroup.getWidth()));
        placeLabeledSliderWithReadout(nativeSliders[0], nativeLabels[0], controlValueLabels[0], stackControl);
        placeLabeledSliderWithReadout(nativeSliders[1], nativeLabels[1], controlValueLabels[1], toneGroup);
    }
    else if (displayedMode == chipper::ChipMode::sid)
    {
        auto interactionGroup = juce::Rectangle<int> {
            sourceChannelBounds[0].getX(),
            sourceChannelBounds[0].getBottom() + 6,
            sourceChannelBounds[2].getRight() - sourceChannelBounds[0].getX(),
            std::max(0, sourcePanel.getBottom() - sourceChannelBounds[0].getBottom() - 6)
        };
        placeSnNoiseModeSegment(interactionGroup);
    }
    ...
    const auto usesFmOperatorRegisterSurface = displayedMode == chipper::ChipMode::ym2612
        ...
        || displayedMode == chipper::ChipMode::ym2608
        || isOpnbMode(displayedMode);
    const auto usesSampleToneStack = displayedMode == chipper::ChipMode::paula
        && usesSnNoiseModeSegment(displayedMode);
    if (displayedMode == chipper::ChipMode::ym2149)
        tonePanel.removeFromTop(std::min(ymSharedNoiseCardHeight + 8, tonePanel.getHeight()));
    ...
    else if (usesFmToneStack && ! ym2612Layout && ! opl3Layout && ! ym2151Layout && ! ym2413Layout && ! ym2203Layout && ! opnaOpnbLayout)
    {
    ...
    if (opl3Layout)
    {
        constexpr int oplPatchGap = 4;
        auto topologyArea = tonePanel.removeFromTop(std::min(48, tonePanel.getHeight()));
        placeYmEnvelopeShapeSegment(topologyArea);
        tonePanel.removeFromTop(std::min(oplPatchGap, tonePanel.getHeight()));

        auto waveformArea = tonePanel.removeFromTop(std::min(56, tonePanel.getHeight()));
        placeOplWaveformControl(waveformArea);
        tonePanel.removeFromTop(std::min(oplPatchGap, tonePanel.getHeight()));
        ...
        for (size_t control = 0; control < macroAreas.size(); ++control)
        {
            placeGroupedSlider(nativeSliders[control],
                               nativeGroupLabels[control],
                               nativeLabels[control],
                               controlValueLabels[control],
                               macroAreas[control]);
            nativeGroupLabels[control].setBounds({});
        }
    }
    else if (ym2612Layout || ym2151Layout)
    {
        constexpr int patchGap = 6;
        auto algorithmArea = tonePanel.removeFromTop(std::min(74, tonePanel.getHeight()));
        placeFmAlgorithmControl(algorithmArea);
        tonePanel.removeFromTop(std::min(patchGap, tonePanel.getHeight()));

        const auto patchRowHeight = std::max(0, (tonePanel.getHeight() - patchGap) / 2);
        auto firstRow = tonePanel.removeFromTop(std::min(patchRowHeight, tonePanel.getHeight()));
        tonePanel.removeFromTop(std::min(patchGap, tonePanel.getHeight()));
        auto secondRow = tonePanel;
        const auto patchColumnWidth = std::max(0, (firstRow.getWidth() - patchGap) / 2);

        auto algorithmBiasArea = firstRow.removeFromLeft(std::min(patchColumnWidth, firstRow.getWidth()));
        firstRow.removeFromLeft(std::min(patchGap, firstRow.getWidth()));
        auto feedbackArea = firstRow;
        auto operatorToneArea = secondRow.removeFromLeft(std::min(patchColumnWidth, secondRow.getWidth()));
        secondRow.removeFromLeft(std::min(patchGap, secondRow.getWidth()));
        auto fmLevelArea = secondRow;

        placeGroupedSlider(nativeSliders[0], nativeGroupLabels[0], nativeLabels[0], controlValueLabels[0], algorithmBiasArea);
        placeFmFeedbackControl(feedbackArea);
        placeGroupedSlider(nativeSliders[2], nativeGroupLabels[2], nativeLabels[2], controlValueLabels[2], operatorToneArea);
        placeGroupedSlider(nativeSliders[3], nativeGroupLabels[3], nativeLabels[3], controlValueLabels[3], fmLevelArea);
    }
    else if (spc700Layout)
    {
        constexpr int sourceTypeGap = 10;
        const auto sourceTypeWidth = (tonePanel.getWidth() - sourceTypeGap) / 2;
        auto generatedShapePanel = tonePanel.removeFromLeft(sourceTypeWidth);
        tonePanel.removeFromLeft(sourceTypeGap);
        placeWaveShapeSegment(generatedShapePanel);
    ...
    else if (displayedMode == chipper::ChipMode::sid)
    {
        ...
        const auto controlWidth = (tonePanel.getWidth() - (columnGap * 4)) / 5;
        auto cutoffPanel = tonePanel.removeFromLeft(controlWidth);
        tonePanel.removeFromLeft(columnGap);
        auto resonancePanel = tonePanel.removeFromLeft(controlWidth);
        tonePanel.removeFromLeft(columnGap);
        auto modelPanel = tonePanel.removeFromLeft(controlWidth);
        tonePanel.removeFromLeft(columnGap);
        auto modePanel = tonePanel.removeFromLeft(controlWidth);
        tonePanel.removeFromLeft(columnGap);
        auto routingPanel = tonePanel;
        placeFilterSlider(nativeSliders[2], nativeLabels[2], controlValueLabels[2], cutoffPanel);
        placeFilterSlider(stereoSpreadSlider, stereoSpreadLabel, stereoSpreadValueLabel, resonancePanel);
        placeDmgStereoRouteSegment(modelPanel);
        placeYmEnvelopeShapeSegment(modePanel);
        placeSidFilterRoutingControl(routingPanel);
        ymEnvelopeShapeValueLabel.setBounds({});
        sidFilterRoutingValueLabel.setBounds({});
    }
    else if (displayedMode == chipper::ChipMode::ym2149)
    {
        ymChannelMixLabel.setBounds({});
        ymChannelMixValueLabel.setBounds({});
        nativeLabels[3].setText("Default A/B/C Routing", juce::dontSendNotification);
    ...
    {
        constexpr int relationshipGap = 14;
        const auto relationshipWidth = (tonePanel.getWidth() - relationshipGap) / 2;
        auto pairingPanel = tonePanel.removeFromLeft(relationshipWidth);
        tonePanel.removeFromLeft(relationshipGap);
        auto filterPanel = tonePanel;
    ...
    else if (opnaOpnbLayout)
    {
        constexpr int fmPatchGap = 5;
        auto algorithmArea = tonePanel.removeFromTop(std::min(56, tonePanel.getHeight()));
        placeFmAlgorithmControl(algorithmArea);
        tonePanel.removeFromTop(std::min(fmPatchGap, tonePanel.getHeight()));

        auto envelopeArea = tonePanel.removeFromTop(std::min(42, tonePanel.getHeight()));
        placeYmEnvelopeShapeSegment(envelopeArea);
        tonePanel.removeFromTop(std::min(fmPatchGap, tonePanel.getHeight()));
        ...
        placeOpllInstrumentControl(tonePanel);
    else if (isNesFamily(displayedMode))
    {
    ...
    else if (! spc700Layout && ! pokeyLayout && ! oneBitHardwarePathLayout)
    ...
    motionPanel.removeFromTop(opnaOpnbLayout ? 2 : 4);
    if (opnaOpnbLayout)
    {
        constexpr int ssgGeneratorGap = 12;
        const auto availableGeneratorWidth = std::max(0, motionPanel.getWidth() - ssgGeneratorGap);
        const auto generatorWidth = ym2610Layout
            ? static_cast<int>(std::round(static_cast<double>(availableGeneratorWidth) * 0.62))
            : static_cast<int>(std::round(static_cast<double>(availableGeneratorWidth) * 0.58));
        auto envelopeShapeArea = motionPanel.removeFromLeft(std::min(generatorWidth, motionPanel.getWidth()));
        motionPanel.removeFromLeft(std::min(ssgGeneratorGap, motionPanel.getWidth()));
        placeSnNoiseModeSegment(envelopeShapeArea);
        placeLabeledSliderWithReadout(envelopeDecaySlider, envelopeDecayLabel, envelopeDecayValueLabel, motionPanel);
    }
    if (usesSnNoiseModeSegment(displayedMode)
        ...
        && displayedMode != chipper::ChipMode::saa1099
        && displayedMode != chipper::ChipMode::spc700
    ...
    {
        constexpr int textureGap = 14;
        const auto textureWidth = (envelopeDecayPanel.getWidth() - (textureGap * 2)) / 3;
        auto distortionPanel = envelopeDecayPanel.removeFromLeft(textureWidth);
        envelopeDecayPanel.removeFromLeft(textureGap);
        auto biasPanel = envelopeDecayPanel.removeFromLeft(textureWidth);
        envelopeDecayPanel.removeFromLeft(textureGap);
        auto gatePanel = envelopeDecayPanel;

        placeWaveShapeSegment(distortionPanel);
    ...
    {
        constexpr int shapingGap = 10;
        const auto shapeWidth = std::max(260, envelopeDecayPanel.getWidth() / 2);
        auto shapeArea = envelopeDecayPanel.removeFromLeft(std::min(shapeWidth, envelopeDecayPanel.getWidth()));
        envelopeDecayPanel.removeFromLeft(std::min(shapingGap, envelopeDecayPanel.getWidth()));
        const auto sliderWidth = std::max(0, (envelopeDecayPanel.getWidth() - shapingGap) / 2);
        auto pitchArea = envelopeDecayPanel.removeFromLeft(std::min(sliderWidth, envelopeDecayPanel.getWidth()));
        envelopeDecayPanel.removeFromLeft(std::min(shapingGap, envelopeDecayPanel.getWidth()));

        placeYmEnvelopeShapeSegment(shapeArea);
        placeGroupedSlider(nativeSliders[1], nativeGroupLabels[1], nativeLabels[1], controlValueLabels[1], pitchArea);
    ...
    else if (ym2203Layout || opnaOpnbLayout)
    {
    ...
    juce::Rectangle<int> opn2DacSampleCell;
    if (ym2612Layout)
    {
        auto sampleStrip = moduleBounds[5].reduced(12, 9);
        sampleStrip.removeFromTop(std::min(20, sampleStrip.getHeight()));
        opn2DacSampleCell = sampleStrip.removeFromTop(std::min(30, sampleStrip.getHeight()));
        moduleSummaryLabels[5].setBounds({});
    }
    juce::Rectangle<int> opnaRhythmRomCell;
    ...
    if (opnaOpnbLayout)
    {
        opnaRhythmRomCell = outputPanel;
        outputPanel = {};
    }
    else if (isOpnbMode(displayedMode) && outputPanel.getWidth() >= 620)
    {
        ...
        outputPanel.removeFromRight(std::min(12, outputPanel.getWidth()));
    }
    ...
    const auto splitOutputCard = displayedMode != chipper::ChipMode::sid
        ...
        && usesDmgStereoRouteSegment(displayedMode);
    if (splitOutputCard && ! ym2612Layout && ! opl3Layout && ! ym2151Layout && ! ym2413Layout)
    {
    ...
    if (ym2612Layout)
    {
        constexpr int ym2612OutputGap = 10;
        const auto outputControlWidth = std::max(0, (outputPanel.getWidth() - (ym2612OutputGap * 3)) / 4);
        std::array<juce::Rectangle<int>, 4> outputControls {};
        for (size_t control = 0; control < outputControls.size(); ++control)
        {
            outputControls[control] = {
                outputPanel.getX() + (static_cast<int>(control) * (outputControlWidth + ym2612OutputGap)),
                outputPanel.getY(),
                outputControlWidth,
                outputPanel.getHeight()
            };
        }

        placeYmEnvelopeShapeSegment(outputControls[0]);
        placeSnNoiseModeSegment(outputControls[1]);
        placeLabeledSliderWithReadout(stereoSpreadSlider, stereoSpreadLabel, stereoSpreadValueLabel, outputControls[2]);
        placeDmgStereoRouteSegment(outputControls[3]);
    }
    else if (opl3Layout)
    {
    ...
    else if (displayedMode != chipper::ChipMode::sid && ! pokeyLayout && ! huc6280Layout && ! ym2612Layout && ! opl3Layout && ! ym2151Layout && ! ym2413Layout && ! ym2203Layout && ! opnaOpnbLayout)
    {
    ...
    globalStripLabel.setBounds(stripHeader.removeFromLeft(paulaLayout ? 210 : (opnaOpnbLayout ? 230 : (huc6280Layout ? 205 : ((namcoWsgLayout || sccLayout) ? 195 : 150)))));
    ...
    std::array<juce::Rectangle<int>, 7> wavetableControlCells {};
    ...
    if (displayedMode == chipper::ChipMode::sid)
    {
        constexpr int sidControlGap = 10;
        constexpr std::array<size_t, 4> sidCellIndices { 1u, 2u, 3u, 4u };
        const auto sidControlWidth = (strip.getWidth() - (sidControlGap * 3)) / 4;
        controlCells.fill({});
        for (size_t column = 0; column < sidCellIndices.size(); ++column)
            controlCells[sidCellIndices[column]] = {
                strip.getX() + (static_cast<int>(column) * (sidControlWidth + sidControlGap)),
                strip.getY(),
                sidControlWidth,
                strip.getHeight()
            };
    ...
    else if (displayedMode == chipper::ChipMode::ym2149)
    {
        ...
        constexpr std::array<size_t, 5> ymCellIndices { 0u, 1u, 2u, 4u, 5u };
        const auto ymControlWidth = (strip.getWidth() - (ymControlGap * 4)) / 5;
        controlCells.fill({});
        for (size_t column = 0; column < ymCellIndices.size(); ++column)
            controlCells[ymCellIndices[column]] = {
                strip.getX() + (static_cast<int>(column) * (ymControlWidth + ymControlGap)),
                strip.getY(),
                ymControlWidth,
                strip.getHeight()
            };
    ...
    {
        constexpr int pokeyControlGap = 8;
        const auto pokeyControlWidth = (strip.getWidth() - (pokeyControlGap * 5)) / 6;
        for (size_t i = 0; i < controlCells.size(); ++i)
            controlCells[i] = {
                strip.getX() + (static_cast<int>(i) * (pokeyControlWidth + pokeyControlGap)),
                strip.getY(),
                pokeyControlWidth,
                strip.getHeight()
            };
    ...
    else if (displayedMode == chipper::ChipMode::sn76489)
    {
        constexpr int snControlGap = 12;
        const auto controlWidth = (strip.getWidth() - snControlGap) / 2;
        controlCells.fill({});
        controlCells[4] = { strip.getX(), strip.getY(), controlWidth, strip.getHeight() };
        controlCells[5] = { strip.getX() + controlWidth + snControlGap, strip.getY(), controlWidth, strip.getHeight() };
    }
    else if (ym2612Layout || opl3Layout || ym2151Layout)
    {
        constexpr int ym2612ControlGap = 12;
        const auto controlWidth = (strip.getWidth() - ym2612ControlGap) / 2;
        controlCells.fill({});
        controlCells[4] = { strip.getX(), strip.getY(), controlWidth, strip.getHeight() };
    ...
    {
        constexpr int opnaControlGap = 8;
        const auto opnaControlWidth = (strip.getWidth() - (opnaControlGap * 5)) / 6;
        for (size_t column = 0; column < controlCells.size(); ++column)
            controlCells[column] = {
                strip.getX() + (static_cast<int>(column) * (opnaControlWidth + opnaControlGap)),
                strip.getY(),
                opnaControlWidth,
                strip.getHeight()
            };
    ...
    {
        // OPLL keeps its four shared performance controls beside clock/output
        // in a compact two-row footer. Instrument, topology, and User0 live above.
    }
    else if (paulaLayout)
    {
    ...
    else if (namcoWsgLayout || sccLayout)
    {
        constexpr int wavetableRowGap = 10;
        constexpr int wavetableCellGap = 9;
        auto topRow = strip.removeFromTop(std::max(0, (strip.getHeight() - wavetableRowGap) / 2));
        strip.removeFromTop(std::min(wavetableRowGap, strip.getHeight()));
        auto bottomRow = strip;
        const auto topWidth = (topRow.getWidth() - (wavetableCellGap * 3)) / 4;
        const auto bottomWidth = (bottomRow.getWidth() - (wavetableCellGap * 2)) / 3;

        for (size_t column = 0; column < 4u; ++column)
            wavetableControlCells[column] = {
                topRow.getX() + (static_cast<int>(column) * (topWidth + wavetableCellGap)),
                topRow.getY(),
                topWidth,
                topRow.getHeight()
            };
        for (size_t column = 0; column < 3u; ++column)
            wavetableControlCells[column + 4u] = {
                bottomRow.getX() + (static_cast<int>(column) * (bottomWidth + wavetableCellGap)),
                bottomRow.getY(),
                bottomWidth,
                bottomRow.getHeight()
            };
    ...
    else if (spc700Layout)
    {
        ...
        constexpr std::array<size_t, 5> spcCellIndices { 0u, 1u, 2u, 3u, 5u };
        const auto spcControlWidth = (strip.getWidth() - (spcControlGap * 4)) / 5;
        controlCells.fill({});
        for (size_t column = 0; column < spcCellIndices.size(); ++column)
            controlCells[spcCellIndices[column]] = {
                strip.getX() + (static_cast<int>(column) * (spcControlWidth + spcControlGap)),
                strip.getY(),
                spcControlWidth,
                strip.getHeight()
            };
    ...
    {
        // Tone Stack and Pitch Motion are spatially owned by the three tone
        // cards above; the global strip contains only clock and output.
    }
    else if (ym2612Layout || opl3Layout || ym2151Layout)
    {
    ...
    {
        placeGroupedSlider(nativeSliders[2], nativeGroupLabels[2], nativeLabels[2], controlValueLabels[2], controlCells[0]);
        placeGroupedSlider(nativeSliders[3], nativeGroupLabels[3], nativeLabels[3], controlValueLabels[3], controlCells[1]);
        placeDmgStereoRouteSegment(controlCells[2]);
    ...
    {
        placeGroupedSlider(nativeSliders[0], nativeGroupLabels[0], nativeLabels[0], controlValueLabels[0], wavetableControlCells[0]);
        placeGroupedSlider(nativeSliders[1], nativeGroupLabels[1], nativeLabels[1], controlValueLabels[1], wavetableControlCells[1]);
        placeGroupedSlider(nativeSliders[2], nativeGroupLabels[2], nativeLabels[2], controlValueLabels[2], wavetableControlCells[2]);
        placeGroupedSlider(nativeSliders[3], nativeGroupLabels[3], nativeLabels[3], controlValueLabels[3], wavetableControlCells[3]);
        placeLabeledSliderWithReadout(envelopeDecaySlider, envelopeDecayLabel, envelopeDecayValueLabel, wavetableControlCells[4]);
        placeLabeledSliderWithReadout(stereoSpreadSlider, stereoSpreadLabel, stereoSpreadValueLabel, wavetableControlCells[5]);
    }
    else if (sampleLayout)
    {
        placeGroupedSlider(nativeSliders[0], nativeGroupLabels[0], nativeLabels[0], controlValueLabels[0], controlCells[0]);
    ...
    else
    {
        ...
        else if (displayedMode == chipper::ChipMode::ym2149)
        {
            ...
            placeLabeledSliderWithReadout(stereoSpreadSlider, stereoSpreadLabel, stereoSpreadValueLabel, controlCells[2]);
        }
    ...
    else if (displayedMode == chipper::ChipMode::spc700 || displayedMode == chipper::ChipMode::paula)
    {
        ...
        const auto minimumTwoColumnSampleBankHeight = displayedMode == chipper::ChipMode::spc700 ? 104 : 126;
        ...
        {
            const auto columnGap = 12;
            const auto controlWidth = std::clamp(static_cast<int>(std::round(static_cast<double>(sampleCell.getWidth()) * 0.42)), 420, 520);
        ...
        auto sampleHeader = controlColumn.removeFromTop(std::min(compactSampleBank ? 14 : 18, controlColumn.getHeight()));
        ...
        controlColumn.removeFromTop(std::min(compactSampleBank ? 3 : (twoColumnSampleBank ? 4 : 5), controlColumn.getHeight()));
        ...
        auto rootCell = sampleRow.removeFromRight(std::min(twoColumnSampleBank ? 124 : 116, sampleRow.getWidth()));
        sampleRow.removeFromRight(std::min(8, sampleRow.getWidth()));
        ...
        if (displayedMode == chipper::ChipMode::spc700 && ! compactSampleBank && ! twoColumnSampleBank)
        {
        ...
        else if (! (displayedMode == chipper::ChipMode::spc700 && twoColumnSampleBank))
        {
            sampleLoopStartLabel.setBounds({});
            sampleLoopEndLabel.setBounds({});
            sampleLoopStartValueLabel.setBounds({});
        ...
        if (twoColumnSampleBank)
        {
            controlColumn.removeFromTop(std::min(compactSampleBank ? 3 : 6, controlColumn.getHeight()));
            const auto statusHeight = compactSampleBank ? 12 : (displayedMode == chipper::ChipMode::paula ? 26 : 40);
        ...
        if (displayedMode == chipper::ChipMode::spc700 && twoColumnSampleBank)
        {
            auto loopControls = waveformBounds.removeFromBottom(std::min(38, waveformBounds.getHeight()));
            waveformBounds.removeFromBottom(std::min(4, waveformBounds.getHeight()));
            constexpr int loopControlGap = 10;
            const auto loopControlWidth = (loopControls.getWidth() - loopControlGap) / 2;

            auto placeLoopControl = [](juce::Label& label,
                                       juce::Label& valueLabel,
                                       juce::Slider& slider,
                                       juce::Rectangle<int> bounds)
            {
                auto header = bounds.removeFromTop(std::min(14, bounds.getHeight()));
                valueLabel.setBounds(header.removeFromRight(std::min(54, header.getWidth())));
                label.setBounds(header);
                slider.setBounds(bounds.reduced(0, 2));
            };

            auto loopStart = loopControls.removeFromLeft(loopControlWidth);
            loopControls.removeFromLeft(loopControlGap);
            placeLoopControl(sampleLoopStartLabel, sampleLoopStartValueLabel, sampleLoopStartSlider, loopStart);
            placeLoopControl(sampleLoopEndLabel, sampleLoopEndValueLabel, sampleLoopEndSlider, loopControls);
        }
    ...
    else if (displayedMode == chipper::ChipMode::ym2612)
    {
        ...
        auto sampleCell = opn2DacSampleCell;
        dmcSampleLabel.setBounds(sampleCell.removeFromLeft(std::min(92, sampleCell.getWidth())));
        sampleCell.removeFromLeft(std::min(6, sampleCell.getWidth()));
        dmcSampleFileButton.setBounds(sampleCell.removeFromLeft(std::min(62, sampleCell.getWidth())).reduced(0, 1));
        sampleCell.removeFromLeft(std::min(8, sampleCell.getWidth()));
        const auto statusWidth = std::min(320, std::max(180, sampleCell.getWidth() / 3));
        dmcSampleStatusLabel.setBounds(sampleCell.removeFromLeft(std::min(statusWidth, sampleCell.getWidth())).reduced(0, 1));
        sampleCell.removeFromLeft(std::min(10, sampleCell.getWidth()));
        sampleWaveformPreview.setBounds(sampleCell.reduced(0, 1));
    }
    else if (displayedMode == chipper::ChipMode::ym2608 || isOpnbMode(displayedMode))
    {
        ...
        auto romCell = opnaRhythmRomCell.isEmpty() ? utilityCell : opnaRhythmRomCell;
        if (romCell.getHeight() <= 0 || romCell.getWidth() <= 0)
        {
        ...
        else
        {
            ...
            if (opnaOpnbLayout && romCell.getWidth() >= 460)
            {
                const auto controlWidth = std::clamp(romCell.getWidth() / 2, 260, 330);
                ...
                controlCell.removeFromTop(std::min(5, controlCell.getHeight()));
                dmcSampleStatusLabel.setBounds(controlCell.removeFromTop(std::min(28, controlCell.getHeight())).reduced(0, 1));
                sampleWaveformPreview.setBounds(romCell.reduced(0, 1));
            }
            else
            {
                ...
                dmcSampleStatusLabel.setBounds(romCell.removeFromTop(std::min(18, romCell.getHeight())).reduced(0, 1));
                romCell.removeFromTop(std::min(6, romCell.getHeight()));
                sampleWaveformPreview.setBounds(romCell.reduced(0, 1));
            }
        }
    ...
    auto outputCell = paulaLayout
        ...
        : (huc6280Layout
               ...
               : ((namcoWsgLayout || sccLayout)
                      ? wavetableControlCells[6]
...
void ChipperAudioProcessorEditor::placeGroupedSlider(juce::Slider& slider,
                                                     ...
                                                     juce::Rectangle<int> bounds)
{
    constexpr auto groupHeight = 12;
    constexpr auto labelHeight = 16;
    constexpr auto sliderHeight = 23;
    constexpr auto readoutHeight = 16;
    ...
    const auto canShowReadout = canShowFullStack
        && originalBounds.getWidth() >= (displayedMode == chipper::ChipMode::ym2612 ? 190 : 280)
...
void ChipperAudioProcessorEditor::placeOpllInstrumentControl(juce::Rectangle<int> bounds)
{
    ...
    ymEnvelopeShapeValueLabel.setBounds(useColumns ? juce::Rectangle<int> {} : rhythmHeader);
    ...
    if (useColumns)
    {
        rhythmPanel.removeFromTop(std::min(3, rhythmPanel.getHeight()));
        ymEnvelopeShapeValueLabel.setJustificationType(juce::Justification::centredLeft);
        ymEnvelopeShapeValueLabel.setBounds(rhythmPanel.removeFromTop(std::min(readoutHeight, rhythmPanel.getHeight())));
    }
    else
    {
        ymEnvelopeShapeValueLabel.setJustificationType(juce::Justification::centredLeft);
...
void ChipperAudioProcessorEditor::placeSnNoiseModeSegment(juce::Rectangle<int> bounds)
{
    ...
    if (displayedMode == chipper::ChipMode::sid && bounds.getHeight() < 38)
    {
        snNoiseModeLabel.setBounds({});
        snNoiseModeValueLabel.setBounds({});
        snNoiseModeBox.setBounds({});
        snNoiseModeSegmentBounds = bounds.reduced(0, 1);
...
void ChipperAudioProcessorEditor::updatePresetChoices(chipper::ChipMode mode)
{
    ...
    allDisplayedPresets = chipper::presetBrowserCatalog(mode);
    reloadUserPresetFiles(mode);
    updatePresetFilterChoices(mode);
                                                                 ...
                                                                 float stereoSpread) const
{
    ...
    auto patch = chipper::makePatchConfig(
        ...
        {
            static_cast<int>(std::round(parameterValue(chipper::parameters::id::fmOperator1Multiplier))),
        ...
        {
            static_cast<int>(std::round(parameterValue(chipper::parameters::id::fmOperator1DecayRate))),
...
juce::String ChipperAudioProcessorEditor::performanceMacroDestination(chipper::ChipMode mode, size_t index) const
{
    switch (mode)
    {
        ...
        case chipper::ChipMode::namcoWsg:
        {
            static constexpr std::array<const char*, 4> labels { "Lane spread", "Pitch motion", "Pulse width", "4-bit volume" };
            return labels[std::min(index, labels.size() - 1u)];
        }

        case chipper::ChipMode::scc:
        {
            static constexpr std::array<const char*, 4> labels { "Stack intervals", "Zap/Jump ch1", "Pulse bytes", "$AA-$AE volume" };
            return labels[std::min(index, labels.size() - 1u)];
        }
        ...
        case chipper::ChipMode::ym2413:
        {
            static constexpr std::array<const char*, 4> labels { "ROM instrument", "Tuning offset", "Motion", "Volume nibble" };
            return labels[std::min(index, labels.size() - 1u)];
        }
    }
...
juce::String ChipperAudioProcessorEditor::pokeySourceRegisterReadout(const chipper::PatchConfig& patch, size_t index) const
{
    ...
    const auto pairedLow = (index == 0u && (audctl & 0x10u) != 0) || (index == 2u && (audctl & 0x08u) != 0);
    if (pairedHigh)
        return "AUDF high byte for the preceding 16-bit pair; this channel has no independent AUDC output.";
    if (pairedLow)
        return "16-bit AUDF pair | AUDC $" + byteHex(audc) + " | AUDV " + juce::String(audv) + "/15";
...
juce::String ChipperAudioProcessorEditor::pokeySourceLevelReadout(const chipper::PatchConfig& patch, size_t index) const
{
    ...
    const auto audctl = chipper::pokeyAudctlForPatch(patch);
    const auto pairedHigh = (index == 1u && (audctl & 0x10u) != 0) || (index == 3u && (audctl & 0x08u) != 0);
    const auto safeIndex = std::min(index, patch.sourceLevels.size() - 1u);
    const auto trim = std::clamp(static_cast<int>(std::round(patch.sourceLevels[safeIndex] * 100.0f)), 0, 100);
    if (pairedHigh)
        return "Pair high byte | trim parked " + juce::String(trim) + "%";
    return "AUDV " + juce::String(audv) + "/15 | trim " + juce::String(trim) + "%";
}
...
juce::String ChipperAudioProcessorEditor::wavetableChipReadout(chipper::ChipMode mode, const chipper::PatchConfig& patch) const
{
    ...
    if (mode == chipper::ChipMode::namcoWsg)
    {
        const auto volume = std::clamp(static_cast<int>(std::round(std::clamp(patch.control4, 0.0f, 1.0f) * 15.0f)), 1, 15);
        const auto highSamples = std::clamp(static_cast<int>(std::round(4.0f + std::clamp(patch.control3, 0.0f, 1.0f) * 24.0f)), 4, 28);
        return "32 x 4-bit Wave RAM | pulse high " + juce::String(highSamples) + "/32 | volume " + juce::String(volume) + "/15";
    }

    if (mode == chipper::ChipMode::scc)
    {
        const auto volume = std::clamp(static_cast<int>(std::round(std::clamp(patch.control4, 0.0f, 1.0f) * 15.0f)), 1, 15);
        const auto highBytes = std::clamp(static_cast<int>(std::round(4.0f + std::clamp(patch.control3, 0.0f, 1.0f) * 24.0f)), 4, 28);
        return "5 x 32-byte 8-bit Wave RAM | pulse high " + juce::String(highBytes) + "/32 | volume " + juce::String(volume) + "/15 | enhanced core";
    }
                                                                   ...
                                                                   size_t index) const
{
    ...
    if (mode == chipper::ChipMode::namcoWsg)
    {
        auto text = "Lane " + number
            + " | 4-bit Wave RAM | V" + juce::String(static_cast<int>(chipper::namcoWsgVolumeForPatch(patch, index))) + "/15";
        if (patch.playMode == chipper::PlayMode::chipPoly)
            text += " | Note " + number;
        return text;
    }
    ...
    {
        auto text = "Ch " + number
            + " | 32-byte Wave RAM | V" + juce::String(static_cast<int>(chipper::sccVolumeForPatch(patch, index))) + "/15";
        if (patch.playMode == chipper::PlayMode::chipPoly)
            text += " | Note " + number;
        return text;
    }
...
juce::String ChipperAudioProcessorEditor::fmFeedbackReadout(chipper::ChipMode mode, const chipper::PatchConfig& patch) const
{
    ...
    if (mode == chipper::ChipMode::ym2413)
    {
        const auto offset = static_cast<int>(std::round((std::clamp(patch.control2, 0.0f, 1.0f) - 0.5f) * 12.0f));
        return "Tuning " + juce::String(offset >= 0 ? "+" : "") + juce::String(offset) + " st";
    }
                                                                    ...
                                                                    size_t op) const
{
    ...
    if (isOpllOperatorEditMode(mode))
    {
        ...
        const auto family = mode == chipper::ChipMode::nesVrc7 ? juce::String("VRC7") : juce::String("YM2413");
        if (mode == chipper::ChipMode::ym2413 && patch.waveShape > 0)
            return "ROM I" + juce::String(static_cast<int>(chipper::ym2413InstrumentForPatch(patch))) + " owns EG | User0 editor inactive";
...
juce::String ChipperAudioProcessorEditor::fmSourceRegisterReadout(chipper::ChipMode mode, const chipper::PatchConfig& patch, size_t index) const
{
    ...
    auto ymPort = (mode == chipper::ChipMode::ym2203) ? 0 : (index >= 3u ? 1 : 0);
    auto ymSlot = static_cast<uint8_t>(std::min(index % 3u, size_t { 2u }));
    if (mode == chipper::ChipMode::ym2610)
    {
        static constexpr std::array<uint8_t, 4> opnbNativeChannels { 1u, 2u, 4u, 5u };
        const auto nativeChannel = opnbNativeChannels[std::min(index, opnbNativeChannels.size() - 1u)];
        ymPort = nativeChannel >= 3u ? 1 : 0;
        ymSlot = static_cast<uint8_t>(nativeChannel % 3u);
    }
                                                               ...
                                                               juce::String fallback) const
{
    ...
    {
        const auto audctl = chipper::pokeyAudctlForPatch(patch);
        const auto pair12 = (audctl & 0x10u) != 0;
        const auto pair34 = (audctl & 0x08u) != 0;
        const auto pairedHigh = (index == 1u && pair12) || (index == 3u && pair34);
        const auto pairedLow = (index == 0u && pair12) || (index == 2u && pair34);
        if (pairedHigh)
            return "Ch " + number + " | pair high byte · no note lane";

        auto role = juce::String();
        switch (index)
        {
            case 0: role = "HP target <- Ch3"; break;
            case 1: role = "HP target <- Ch4"; break;
            case 2: role = "HP clock -> Ch1"; break;
            case 3: role = "HP clock -> Ch2"; break;
            default: break;
        }
        ...
        {
            auto noteLane = 0;
            for (size_t channel = 0; channel <= index; ++channel)
            {
                const auto highByte = (channel == 1u && pair12) || (channel == 3u && pair34);
                if (! highByte)
                    ++noteLane;
            }
            return pairedLow
                ? "Ch " + number + "+" + juce::String(static_cast<int>(index + 2u)) + " | 16-bit note " + juce::String(noteLane)
                : "Ch " + number + " | 8-bit note " + juce::String(noteLane) + " · " + role;
        }

        return pairedLow
            ? "Ch " + number + "+" + juce::String(static_cast<int>(index + 2u)) + " | 16-bit voice"
            : "Ch " + number + " | 8-bit · " + role;
    }
    ...
    if (mode == chipper::ChipMode::ym2149)
    {
        ...
        return "Shared " + ym2149ResolvedChannelMixLabel(patch, index);
    }
    ...
    if (mode == chipper::ChipMode::ym2413)
    {
        const auto rhythm = chipper::ym2413RhythmModeForPatch(patch) == 2u;
        const auto rhythmOnly = rhythm
            && (patch.playMode == chipper::PlayMode::chipPoly
                || patch.macro == chipper::MacroKind::drum
                || patch.macro == chipper::MacroKind::hit);
        if (rhythm && index >= 6u && index <= 8u)
        {
            if (index == 6u)
                return "BD | $36 V" + juce::String(static_cast<int>(chipper::ym2413RhythmBassDrumVolumeForPatch(patch))) + " | rhythm trigger";
            if (index == 7u)
                return "HH+SD | $37 V"
                    + juce::String(static_cast<int>(chipper::ym2413RhythmHatVolumeForPatch(patch)))
                    + "/" + juce::String(static_cast<int>(chipper::ym2413RhythmSnareVolumeForPatch(patch))) + " | rhythm trigger";
            return "TOM+CYM | $38 V"
                + juce::String(static_cast<int>(chipper::ym2413RhythmTomVolumeForPatch(patch)))
                + "/" + juce::String(static_cast<int>(chipper::ym2413RhythmCymVolumeForPatch(patch))) + " | rhythm trigger";
        }

        if (rhythmOnly && index < 6u)
            return "OPLL " + number + " | idle | rhythm owns notes";
        ...
        const auto allocation = patch.playMode == chipper::PlayMode::chipPoly
            ? juce::String("note ") + number
            : (rhythm ? juce::String("melodic + rhythm") : juce::String("melodic layer"));
    ...
    if (mode == chipper::ChipMode::ym2612 || mode == chipper::ChipMode::opl3 || mode == chipper::ChipMode::ym2151 || mode == chipper::ChipMode::ym2203 || mode == chipper::ChipMode::ym2608 || isOpnbMode(mode))
    {
        ...
        if (mode == chipper::ChipMode::ym2608)
        {
            const auto allocation = patch.playMode == chipper::PlayMode::chipPoly
                ? juce::String("note ") + number
                : juce::String("stack note ") + number;
            const auto rhythmLayer = patch.macro == chipper::MacroKind::drum || patch.macro == chipper::MacroKind::hit;
            if (index < 6u)
            {
                static constexpr std::array<const char*, 6> rhythmNames { "BD", "SD", "Top Cym", "HH", "Tom", "Rim" };
                auto label = juce::String("FM ") + number;
                if (rhythmLayer)
                    label += " + " + juce::String(rhythmNames[index]);
                return label
                    + " | " + allocation
                    + " | A" + juce::String(algorithm)
                    + " TL" + juce::String(static_cast<int>(chipper::fmOperatorTotalLevelForPatch(mode, patch, 3)));
        ...
        if (mode == chipper::ChipMode::ym2610b)
        {
            const auto allocation = patch.playMode == chipper::PlayMode::chipPoly
                ? juce::String("note ") + number
                : juce::String("stack note ") + number;
            const auto adpcmLayer = patch.macro == chipper::MacroKind::drum || patch.macro == chipper::MacroKind::hit;
            if (index < 6u)
            {
                auto label = juce::String("FM ") + number;
                if (adpcmLayer)
                    label += " + ADPCM-A" + juce::String(static_cast<int>(index + 1u));
                return label
                    + " | " + allocation
                    + " | A" + juce::String(algorithm)
                    + " TL" + juce::String(static_cast<int>(chipper::fmOperatorTotalLevelForPatch(mode, patch, 3)));
        ...
        if (mode == chipper::ChipMode::ym2610)
        {
            ...
            if (chipper::opnSsgEnvelopeEnabledForPatch(patch))
                modeText += " Env";
            auto label = juce::String("SSG ") + ssgNames[ssgIndex];
            if (adpcmLayer && ssgIndex < 2u)
                label += " + ADPCM-A" + juce::String(static_cast<int>(ssgIndex + 5u));
            return label
                + " | " + allocation
                + " | " + modeText
                + " V" + juce::String(static_cast<int>(std::round(std::clamp(patch.control4, 0.0f, 1.0f) * 15.0f)));
        ...
        if (mode == chipper::ChipMode::ym2151)
        {
            const auto panBits = chipper::ym2151PanBitsForPatch(patch, index);
            const auto panText = panBits == 0xc0u ? juce::String("L+R") : (panBits == 0x80u ? juce::String("L") : juce::String("R"));
            auto label = juce::String("OPM ") + number
                + " | A" + juce::String(algorithm)
                + " | " + panText;
            if (index == 7u)
                label += (chipper::ym2151NoiseRegisterForPatch(patch) & 0x80u) != 0u ? juce::String(" | Noise") : juce::String(" | Sine");

            const auto allocation = fallback.fromFirstOccurrenceOf("| ", false, false).trim();
...
juce::String ChipperAudioProcessorEditor::ymEnvelopeShapeReadout(int choice) const
{
    ...
    if (displayedMode == chipper::ChipMode::ym2413)
    {
        ...
        switch (clamped)
        {
            case 1: return "Melodic: all nine channels";
            case 2: return "Rhythm: 7-9 = BD / HH+SD / TOM+CYM";
            case 0:
            default:
                return "Preset: Drum/Hit = Rhythm";
        }
    }
...
juce::String ChipperAudioProcessorEditor::stereoSpreadReadout(chipper::ChipMode mode, float value) const
{
    ...
    if (mode == chipper::ChipMode::namcoWsg)
    {
        ...
        if (spread <= 0.01f)
            return "Centered mono: modern width off";
        return juce::String(percent) + "%: modern eight-lane spread; native pan not modeled";
    }
    ...
    if (mode == chipper::ChipMode::scc)
    {
        ...
        if (spread <= 0.01f)
            return "Centered mono: modern width off";
        return juce::String(percent) + "%: modern five-channel spread; native SCC pan not modeled";
    }
...
juce::String ChipperAudioProcessorEditor::snSourceCardLabel(const chipper::PatchConfig& patch, size_t index) const
{
    ...
    {
        if (index < 3u)
            return "Tone " + juce::String(static_cast<int>(index + 1u)) + " | note " + juce::String(static_cast<int>(index + 1u));
        return "Noise | SFX, not note-allocated";
    }
...
void ChipperAudioProcessorEditor::setWaveShapeSegmentVisible(chipper::ChipMode mode, bool shouldBeVisible)
{
    ...
    const auto hidesGlobalWaveShape = usesPerLaneWaves && mode != chipper::ChipMode::spc700;
...
void ChipperAudioProcessorEditor::updateSourceChannelButtons(chipper::ChipMode mode)
{
    ...
    for (size_t i = 0; i < sourceChannelButtons.size(); ++i)
    {
        ...
        const auto pokeyAudctl = mode == chipper::ChipMode::pokey ? chipper::pokeyAudctlForPatch(patch) : uint8_t { 0u };
        const auto pokeyPairedHighByte = mode == chipper::ChipMode::pokey
            && ((i == 1u && (pokeyAudctl & 0x10u) != 0) || (i == 3u && (pokeyAudctl & 0x08u) != 0));
        const auto oplPairedOperatorLane = mode == chipper::ChipMode::opl3
            && chipper::oplFourOperatorPairForPatch(patch)
            && i >= 3u
            && i <= 5u;
        const auto opllRhythmOnlyMelodicLane = mode == chipper::ChipMode::ym2413
            && chipper::ym2413RhythmModeForPatch(patch) == 2u
            && i < 6u
            && (patch.playMode == chipper::PlayMode::chipPoly
                || patch.macro == chipper::MacroKind::drum
                || patch.macro == chipper::MacroKind::hit);
        ...
        sourceChannelButtons[i].setEnabled(spec != nullptr && ! pokeyPairedHighByte && ! oplPairedOperatorLane && ! opllRhythmOnlyMelodicLane);
        if (isSidVoice)
        {
        ...
        const auto sourceControlAlpha = (pokeyPairedHighByte || opllRhythmOnlyMelodicLane) ? 0.45f : 1.0f;
        sourceLevelLabels[i].setAlpha(sourceControlAlpha);
        sourceLevelSliders[i].setAlpha(sourceControlAlpha);
        sourceLevelValueLabels[i].setAlpha(sourceControlAlpha);
        if (! isNesDmcLane)
...
void ChipperAudioProcessorEditor::updateSourcePreviewScope(chipper::ChipMode mode, const chipper::PatchConfig& patch, size_t index, bool shouldBeVisible)
{
    ...
    else if (mode == chipper::ChipMode::spc700)
    {
        ...
        tooltip = juce::String("SPC700-style sample voice ") + juce::String(static_cast<int>(index + 1u))
            ...
            + "\nGenerated Shape: " + waveShapeReadout(mode, patch.waveShape)
    ...
    else if (mode == chipper::ChipMode::ym2612 || mode == chipper::ChipMode::opl3 || mode == chipper::ChipMode::ym2151 || mode == chipper::ChipMode::ym2413 || mode == chipper::ChipMode::ym2203 || mode == chipper::ChipMode::ym2608 || isOpnbMode(mode))
    {
        ...
        {
            const auto oplRhythmLane = mode == chipper::ChipMode::opl3
                && chipper::oplRhythmModeForPatch(patch) == 2u
                && index >= 6u;
            const auto oplFourOperatorLane = mode == chipper::ChipMode::opl3
                && chipper::oplFourOperatorPairForPatch(patch)
                && index < 6u;
            const auto oplLayerLane = mode == chipper::ChipMode::opl3
                && chipper::opl18ChannelLayerForPatch(patch);
            const auto opn2DacActive = mode == chipper::ChipMode::ym2612
                && index == 5u
                && chipper::ym2612DacModeForPatch(patch) == 2u;
            ...
            if (oplRhythmLane)
            {
                shape = index == 6u ? ChipWaveformPreviewShape::stepped : ChipWaveformPreviewShape::noise;
            }
            else if (oplFourOperatorLane || oplLayerLane)
            {
                shape = ChipWaveformPreviewShape::combined;
            }
            else if (opn2DacActive)
            {
                shape = ChipWaveformPreviewShape::stepped;
            }
            else if (opmNoiseActive)
            {
                shape = ChipWaveformPreviewShape::noise;
            }
            ...
            tooltip = opn2DacActive
                ? juce::String("YM2612 channel 6 native 8-bit DAC path. Chipper streams generated bytes or the user-loaded WAV/AIFF/raw unsigned 8-bit sample.\n")
                    + fmSourceRegisterReadout(mode, patch, index)
                : juce::String(chipper::toString(mode)) + " exposed FM lane "
                                                                  ...
                                                                  bool shouldBeVisible)
{
    ...
    fmEditor.setEnabled(userPatchEditable);
...
void ChipperAudioProcessorEditor::updateWaveShapeButtons(int choice, bool shouldBeVisible)
{
    ...
    if (mode == chipper::ChipMode::spc700)
        waveShapeLabel.setText("Generated Shape", juce::dontSendNotification);
        ...
        hucVoiceWaveLabels[i].setText(isSpc700 ? juce::String("Sample")
                                                : (isPaula ? juce::String("Shape")
                                                           : (mode == chipper::ChipMode::huc6280
                                                                  ...
                                                                  : ((isNamco || isScc) ? juce::String("Wave") : laneName + " " + juce::String(static_cast<int>(i + 1u))))),
                                      juce::dontSendNotification);
...
void ChipperAudioProcessorEditor::updateDmcSampleControls()
{
    ...
    {
        auto runState = juce::String(playbackInfo.loopEnabled ? "Looping sample" : "One-shot, no loop; DAC holds");
...
void ChipperAudioProcessorEditor::updateSpc700BrrSampleControls()
{
    ...
    dmcSampleSlotBox.setTooltip(withMidiCcForRole(
        sampleCount > 0
            ...
            : "No external SPC700 bank is loaded. Chipper is playing the fallback selected by Generated Shape.",
...
void ChipperAudioProcessorEditor::updatePaulaSampleControls()
{
    ...
    auto tooltip = info.statusLine
        + "\nWAV/AIFF, uncompressed IFF/8SVX, and ProTracker MOD sample instruments are imported into Paula-style 8-bit sample memory. Folder loads keep up to 32 user-provided slots addressable by the dropdown and CC117.";
    if (playbackMode == 0)
        tooltip += "\nSample Playback is Manual Slot: each MIDI note uses the selected dropdown sample.";
    else if (playbackMode == 2)
    ...
    if (info.loaded)
        tooltip += "\nPath: " + info.path;
    tooltip += "\nWAV smpl, AIFF MARK/INST, uncompressed 8SVX, and ProTracker MOD sample loop metadata are imported when present. Compressed 8SVX, full MOD playback/effects, and exact Paula DMA timing remain planned.";
    dmcSampleStatusLabel.setTooltip(withMidiCcForRole(tooltip, chipper::ChipParameterRole::nesDmcSampleSlot));
...
void ChipperAudioProcessorEditor::updateOpnbAdpcmSampleControls()
{
    ...
    if (adpcmAInfo.loaded)
    {
        ...
        if (adpcmAInfo.truncated)
            tooltip += "\nOnly the first 1048576 encoded bytes are used by the YM2610 ADPCM-A memory window.";
    }
    if (adpcmBInfo.loaded)
    {
        tooltip += "\nADPCM-B sample path: " + adpcmBInfo.path;
...
void ChipperAudioProcessorEditor::chooseOpn2DacSampleFile()
{
    dmcSampleChooser = std::make_unique<juce::FileChooser>("Choose a YM2612 DAC sample",
                                                           juce::File {},
                                                           "*.wav;*.aif;*.aiff;*.bin;*.raw;*.pcm;*.dat");
    dmcSampleChooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles,
                                  [this](const juce::FileChooser& chooser)
                                  {
                                      const auto file = chooser.getResult();
                                      ...
                                      handleDmcSampleLoadResult(audioProcessor.loadOpn2DacSampleFile(file));
                                  });
}

void ChipperAudioProcessorEditor::chooseOpnaRhythmRomFile()
{
    dmcSampleChooser = std::make_unique<juce::FileChooser>("Choose an OPNA ADPCM-A rhythm ROM",
                                                           juce::File {},
                                                           "*.bin;*.rom;*.dat;*.adpcm;*.*");
    dmcSampleChooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles,
                                  [this](const juce::FileChooser& chooser)
                                  {
                                      ...
                                      handleDmcSampleLoadResult(audioProcessor.loadOpnaRhythmRomFile(file));
                                  });
}
...
void ChipperAudioProcessorEditor::chooseOpnbAdpcmASampleFile()
{
    ...
    dmcSampleChooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles,
                                  [this](const juce::FileChooser& chooser)
                                  {
                                      ...
                                      handleDmcSampleLoadResult(audioProcessor.loadOpnbAdpcmASampleFile(file));
                                  });
}
...
void ChipperAudioProcessorEditor::chooseOpnbAdpcmBSampleFile()
{
    ...
    dmcSampleChooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles,
                                  [this](const juce::FileChooser& chooser)
                                  {
                                      ...
                                      handleDmcSampleLoadResult(audioProcessor.loadOpnbAdpcmBSampleFile(file));
                                  });
}
...
void ChipperAudioProcessorEditor::updateDescriptorText()
{
    ...
    chipSummaryLabel.setText(mode == chipper::ChipMode::ym2608
                                 ? "Six FM + three SSG lanes; Drum/Hit adds six ADPCM-A rhythm voices and optional ADPCM-B."
    ...
    if (! hasLiveCore)
        ...
        globalStripText = "Shared FM/SSG + Stereo Output";
    else if (mode == chipper::ChipMode::spc700)
    ...
    for (size_t i = 0; i < moduleTitleLabels.size(); ++i)
    {
        ...
        if (i == 1 && hasLiveCore && usesSourceChannelSurface(mode))
        {
            ...
            if (mode == chipper::ChipMode::opl3)
            {
                summary = "Nine cards stay visible; topology decides whether they are 2-op voices, rhythm slots, paired layers, or linked 4-op operators.";
            }
            else if (nativeCount > visibleCount)
            {
    ...
    if (mode == chipper::ChipMode::ym2203 || mode == chipper::ChipMode::ym2608 || isOpnbMode(mode))
    {
    ...
    else if (mode == chipper::ChipMode::ym2413)
    {
        nativeSliders[0].textFromValueFunction = [](double value)
        {
            return "I" + juce::String(std::clamp(static_cast<int>(std::round(1.0 + value * 14.0)), 1, 15));
        };
        nativeSliders[0].valueFromTextFunction = [](const juce::String& text)
        {
            const auto instrument = std::clamp(text.retainCharacters("0123456789").getIntValue(), 1, 15);
            return static_cast<double>(instrument - 1) / 14.0;
        };
        nativeSliders[1].textFromValueFunction = [](double value)
        {
            const auto offset = static_cast<int>(std::round((std::clamp(value, 0.0, 1.0) - 0.5) * 12.0));
            return juce::String(offset >= 0 ? "+" : "") + juce::String(offset) + " st";
        };
        nativeSliders[1].valueFromTextFunction = [](const juce::String& text)
        {
            const auto offset = std::clamp(text.getIntValue(), -6, 6);
            return 0.5 + static_cast<double>(offset) / 12.0;
        };
        nativeSliders[2].textFromValueFunction = [](double value)
        {
            return juce::String(static_cast<int>(std::round(std::clamp(value, 0.0, 1.0) * 100.0))) + "%";
        ...
        nativeSliders[3].valueFromTextFunction = [](const juce::String& text)
        {
            const auto nibble = text.upToFirstOccurrenceOf("/", false, false).retainCharacters("0123456789").getIntValue();
            return std::clamp(static_cast<double>(nibble) / 15.0, 0.0, 1.0);
        };
    }
    for (auto& slider : nativeSliders)
        slider.updateText();
    ...
    if ((mode == chipper::ChipMode::ym2608 || isOpnbMode(mode)) && ! dmgStereoRouteButtons.empty())
        dmgStereoRouteButtons[0].setButtonText("Auto");
    if (mode == chipper::ChipMode::ym2203 || mode == chipper::ChipMode::ym2608 || isOpnbMode(mode))
    {
    ...
    setWaveShapeSegmentVisible(mode, usesWaveShapeSegment(mode)
                                         && hasLiveCore
                                         && (! hasChannelLocalWaveDeck || mode == chipper::ChipMode::spc700));
    ...
    const auto hasFmOperatorRegisterSurface = mode == chipper::ChipMode::ym2612
        ...
        || mode == chipper::ChipMode::ym2608
        || isOpnbMode(mode);
    moduleSummaryLabels[1].setVisible(mode == chipper::ChipMode::scc
                                     || !(hasLiveCore && usesSourceChannelSurface(mode)));
    if (mode == chipper::ChipMode::opl3 || mode == chipper::ChipMode::ym2151)
        moduleSummaryLabels[2].setVisible(false);
    if (mode == chipper::ChipMode::ym2608 || isOpnbMode(mode))
    {
        moduleSummaryLabels[4].setVisible(false);
        moduleSummaryLabels[5].setVisible(false);
    }
    if (mode == chipper::ChipMode::ym2151)
        moduleSummaryLabels[5].setVisible(false);
    ...
    const auto hasEmbeddedSourceRegisterControls = isNesFamily(mode)
        || mode == chipper::ChipMode::dmg
        || (usesChannelLocalWaveDeck(mode) && mode != chipper::ChipMode::spc700);
...
void ChipperAudioProcessorEditor::updateLiveControlReadouts()
{
    ...
    const auto contextualBiasActive = mode == chipper::ChipMode::sn76489
        ? patch.snNoiseMode == 0
        : (mode == chipper::ChipMode::pokey ? patch.waveShape == 0 : true);
    nativeLabels[2].setEnabled(contextualBiasActive);
    nativeSliders[2].setEnabled(contextualBiasActive);
    controlValueLabels[2].setEnabled(contextualBiasActive);
    ...
    if (mode == chipper::ChipMode::ym2612 || mode == chipper::ChipMode::ym2151 || mode == chipper::ChipMode::ym2203 || mode == chipper::ChipMode::ym2608 || isOpnbMode(mode))
    {
        nativeLabels[0].setText(fmAlgorithmBiasActive ? "Algorithm Bias" : "Algorithm Bias (Manual + Preset only)",
                                 juce::dontSendNotification);
        nativeGroupLabels[0].setEnabled(fmAlgorithmBiasActive);
        nativeLabels[0].setEnabled(fmAlgorithmBiasActive);
        nativeSliders[0].setEnabled(fmAlgorithmBiasActive);
        controlValueLabels[0].setEnabled(fmAlgorithmBiasActive);
        const auto biasAlpha = fmAlgorithmBiasActive ? 1.0f : 0.45f;
        nativeGroupLabels[0].setAlpha(biasAlpha);
        nativeLabels[0].setAlpha(biasAlpha);
        nativeSliders[0].setAlpha(biasAlpha);
        controlValueLabels[0].setAlpha(biasAlpha);
    }
    if (mode == chipper::ChipMode::ym2413)
    {
        const auto customPatch = chipper::opllCustomPatchEnabledForPatch(patch);
        const auto instrumentBiasActive = patch.waveShape == 0
            && patch.macro == chipper::MacroKind::manual
            && ! customPatch;
        nativeLabels[0].setText(instrumentBiasActive ? "Instrument Bias" : "Instrument Bias (Manual + Preset only)",
                                juce::dontSendNotification);
        nativeGroupLabels[0].setEnabled(instrumentBiasActive);
        nativeLabels[0].setEnabled(instrumentBiasActive);
        nativeSliders[0].setEnabled(instrumentBiasActive);
        controlValueLabels[0].setEnabled(instrumentBiasActive);
        const auto instrumentAlpha = instrumentBiasActive ? 1.0f : 0.45f;
        nativeGroupLabels[0].setAlpha(instrumentAlpha);
        nativeLabels[0].setAlpha(instrumentAlpha);
        nativeSliders[0].setAlpha(instrumentAlpha);
        controlValueLabels[0].setAlpha(instrumentAlpha);

        const auto motionActive = patch.macro == chipper::MacroKind::laser
            || (patch.macro == chipper::MacroKind::manual && customPatch);
        nativeLabels[2].setText(motionActive ? "Motion" : "Motion (Laser or Manual User0)",
                                juce::dontSendNotification);
        nativeGroupLabels[2].setEnabled(motionActive);
        nativeLabels[2].setEnabled(motionActive);
        nativeSliders[2].setEnabled(motionActive);
        controlValueLabels[2].setEnabled(motionActive);
        const auto motionAlpha = motionActive ? 1.0f : 0.45f;
        nativeGroupLabels[2].setAlpha(motionAlpha);
        nativeLabels[2].setAlpha(motionAlpha);
        nativeSliders[2].setAlpha(motionAlpha);
        controlValueLabels[2].setAlpha(motionAlpha);
    }
    ...
    const auto hasWaveShapeSegment = usesWaveShapeSegment(mode)
        && chipper::descriptorFor(mode).implemented
        && (! hasChannelLocalWaveDeck || mode == chipper::ChipMode::spc700);
    ...
    else if (mode == chipper::ChipMode::namcoWsg)
    {
        ...
        const auto highSamples = std::clamp(static_cast<int>(std::round(4.0f + std::clamp(patch.control3, 0.0f, 1.0f) * 24.0f)), 4, 28);
        const auto volume = std::clamp(static_cast<int>(std::round(std::clamp(patch.control4, 0.0f, 1.0f) * 15.0f)), 1, 15);
        auto anyPulseLane = false;
        for (size_t lane = 0; lane < 8u; ++lane)
            anyPulseLane = anyPulseLane || chipper::wavetableWaveShapeForChannel(chipper::ChipMode::namcoWsg, patch, lane) == 3u;

        controlValueLabels[0].setText(macroReadout(0, "Spread " + juce::String(spread) + " st"), juce::dontSendNotification);
        controlValueLabels[1].setText(macroReadout(1, "Motion " + juce::String(motion) + "%"), juce::dontSendNotification);
        controlValueLabels[2].setText(macroReadout(2, anyPulseLane ? "Pulse high " + juce::String(highSamples) + "/32" : juce::String("Inactive: no Pulse lanes")), juce::dontSendNotification);
        controlValueLabels[3].setText(macroReadout(3, "Lane volume " + juce::String(volume) + "/15"), juce::dontSendNotification);
        nativeLabels[2].setText(anyPulseLane ? "Pulse Width" : "Pulse Width (select Pulse)", juce::dontSendNotification);
        nativeSliders[2].setEnabled(anyPulseLane);
        nativeGroupLabels[2].setEnabled(anyPulseLane);
        nativeLabels[2].setEnabled(anyPulseLane);
        controlValueLabels[2].setEnabled(anyPulseLane);
        const auto pulseAlpha = anyPulseLane ? 1.0f : 0.55f;
        nativeSliders[2].setAlpha(pulseAlpha);
        nativeGroupLabels[2].setAlpha(pulseAlpha);
        nativeLabels[2].setAlpha(pulseAlpha);
        controlValueLabels[2].setAlpha(pulseAlpha);
        updateSourceChannelButtons(mode);
    }
    else if (mode == chipper::ChipMode::scc)
    {
        const auto spread = static_cast<int>(std::round(std::clamp(patch.control1, 0.0f, 1.0f) * 16.0f));
        const auto gestureSemitones = static_cast<int>(std::round(std::clamp(patch.control2, 0.0f, 1.0f) * 12.0f));
        const auto highBytes = std::clamp(static_cast<int>(std::round(4.0f + std::clamp(patch.control3, 0.0f, 1.0f) * 24.0f)), 4, 28);
        const auto volume = std::clamp(static_cast<int>(std::round(std::clamp(patch.control4, 0.0f, 1.0f) * 15.0f)), 1, 15);
        const auto gestureActive = patch.macro == chipper::MacroKind::laser || patch.macro == chipper::MacroKind::jump;
        auto anyPulseChannel = false;
        for (size_t channel = 0; channel < 5u; ++channel)
            anyPulseChannel = anyPulseChannel || chipper::wavetableWaveShapeForChannel(chipper::ChipMode::scc, patch, channel) == 3u;

        controlValueLabels[0].setText(macroReadout(0, "Stack " + juce::String(spread) + " st"), juce::dontSendNotification);
        controlValueLabels[1].setText(macroReadout(1, gestureActive ? "Ch 1 +" + juce::String(gestureSemitones) + " st" : juce::String("Inactive: Zap / Jump only")), juce::dontSendNotification);
        controlValueLabels[2].setText(macroReadout(2, anyPulseChannel ? "Pulse high " + juce::String(highBytes) + "/32" : juce::String("Inactive: no Pulse channels")), juce::dontSendNotification);
        controlValueLabels[3].setText(macroReadout(3, "$AA-$AE volume " + juce::String(volume) + "/15"), juce::dontSendNotification);

        nativeLabels[1].setText(gestureActive ? "Gesture Pitch" : "Gesture Pitch (choose Zap / Jump)", juce::dontSendNotification);
        nativeSliders[1].setEnabled(gestureActive);
        nativeGroupLabels[1].setEnabled(gestureActive);
        nativeLabels[1].setEnabled(gestureActive);
        controlValueLabels[1].setEnabled(gestureActive);
        const auto gestureAlpha = gestureActive ? 1.0f : 0.55f;
        nativeSliders[1].setAlpha(gestureAlpha);
        nativeGroupLabels[1].setAlpha(gestureAlpha);
        nativeLabels[1].setAlpha(gestureAlpha);
        controlValueLabels[1].setAlpha(gestureAlpha);

        nativeLabels[2].setText(anyPulseChannel ? "Pulse Width" : "Pulse Width (select Pulse)", juce::dontSendNotification);
        nativeSliders[2].setEnabled(anyPulseChannel);
        nativeGroupLabels[2].setEnabled(anyPulseChannel);
        nativeLabels[2].setEnabled(anyPulseChannel);
        controlValueLabels[2].setEnabled(anyPulseChannel);
        const auto pulseAlpha = anyPulseChannel ? 1.0f : 0.55f;
        nativeSliders[2].setAlpha(pulseAlpha);
        nativeGroupLabels[2].setAlpha(pulseAlpha);
        nativeLabels[2].setAlpha(pulseAlpha);
        controlValueLabels[2].setAlpha(pulseAlpha);
    ...
    else if (mode == chipper::ChipMode::ym2612 || mode == chipper::ChipMode::opl3 || mode == chipper::ChipMode::ym2151 || mode == chipper::ChipMode::ym2413 || mode == chipper::ChipMode::ym2203 || mode == chipper::ChipMode::ym2608 || isOpnbMode(mode))
    {
        if (mode == chipper::ChipMode::opl3)
        {
            ...
            if (chipper::oplRhythmModeForPatch(patch) == 2u)
                signalPath = "Six melodic 2-op lanes + $BD percussion: cards 7-9 become BD, HH+SD, and TOM+CYM -> YMF262 buses -> stereo.";
            else if (chipper::opl18ChannelLayerForPatch(patch))
        ...
        else if (mode == chipper::ChipMode::ym2413)
        {
            const auto custom = chipper::opllCustomPatchEnabledForPatch(patch);
            const auto instrument = static_cast<int>(chipper::ym2413InstrumentForPatch(patch));
            const auto tuningOffset = static_cast<int>(std::round((std::clamp(patch.control2, 0.0f, 1.0f) - 0.5f) * 12.0f));
            const auto rhythm = chipper::ym2413RhythmModeForPatch(patch) == 2u;
            const auto instrumentBiasReadout = custom
                ? juce::String("Inactive: User0 owns instrument")
                : (patch.waveShape > 0
                       ? juce::String("Inactive: explicit I") + juce::String(instrument)
                       : (patch.macro != chipper::MacroKind::manual
                              ? juce::String("Recipe owns I") + juce::String(instrument)
                              : juce::String("Resolved I") + juce::String(instrument)));
            const auto motionReadout = patch.macro == chipper::MacroKind::laser
                ? juce::String("Laser pitch motion ") + juce::String(patch.control3, 2)
                : ((patch.macro == chipper::MacroKind::manual && custom)
                       ? juce::String("User0 Follow bias ") + juce::String(patch.control3, 2)
                       : juce::String("Inactive for current recipe"));

            controlValueLabels[0].setText(macroReadout(0, instrumentBiasReadout), juce::dontSendNotification);
            controlValueLabels[1].setText(macroReadout(1,
                                                       "Tuning " + juce::String(tuningOffset >= 0 ? "+" : "")
                                                           + juce::String(tuningOffset) + " st"),
                                          juce::dontSendNotification);
            controlValueLabels[2].setText(macroReadout(2, motionReadout), juce::dontSendNotification);
            controlValueLabels[3].setText(macroReadout(3,
                                                       juce::String(rhythm ? "$36-$38 rhythm volume " : "$30-$38 channel volume ")
                                                           + juce::String(static_cast<int>(std::round(patch.control4 * 15.0f))) + "/15"),
                                          juce::dontSendNotification);
        }
        else
        {
            ...
            const auto algorithmBiasReadout = (mode != chipper::ChipMode::ym2612
                ...
                : (patch.waveShape > 0
                       ...
                       : juce::String("Recipe owns Algorithm ") + juce::String(algorithm));
            controlValueLabels[0].setText(macroReadout(0, algorithmBiasReadout), juce::dontSendNotification);
            controlValueLabels[1].setText(macroReadout(1, fmFeedbackReadout(mode, patch)), juce::dontSendNotification);
            const auto operatorToneReadout = mode == chipper::ChipMode::ym2612
                ? juce::String("MULT ")
                    ...
                    + " | DT1 " + juce::String(static_cast<int>(chipper::ym2612OperatorDetuneForPatch(patch, 0)))
                    + "/" + juce::String(static_cast<int>(chipper::ym2612OperatorDetuneForPatch(patch, 3)))
                : (mode == chipper::ChipMode::ym2151
                       ? juce::String("MULT ")
                           + juce::String(static_cast<int>(chipper::fmOperatorMultipleForPatch(mode, patch, 0)))
    ...
    else if (mode == chipper::ChipMode::ym2612)
    {
        ...
        controlValueLabels[4].setText(clock <= 0.0f
                                      ...
                                      juce::dontSendNotification);
        controlValueLabels[4].setTooltip(withMidiCc("Optional YM2612 clock override. Zero uses the documented default.", chipper::parameters::id::clockHz));
        clockSlider.updateText();
        updateOpn2DacSampleControls();
    }
    else if (mode == chipper::ChipMode::ym2608)
    {
        ...
        controlValueLabels[4].setText(clock <= 0.0f
                                          ...
                                          : juce::String("Override ") + juce::String(static_cast<double>(clock) / 1000000.0, 2) + " MHz",
                                      juce::dontSendNotification);
        controlValueLabels[4].setTooltip(withMidiCc("Optional OPNA clock override. Zero uses the documented 7.99 MHz default.", chipper::parameters::id::clockHz));
        clockSlider.updateText();
        updateOpnaRhythmRomControls();
    }
    else if (isOpnbMode(mode))
    {
        ...
        controlValueLabels[4].setText(clock <= 0.0f
                                          ...
                                          : juce::String("Override ") + juce::String(static_cast<double>(clock) / 1000000.0, 2) + " MHz",
                                      juce::dontSendNotification);
        controlValueLabels[4].setTooltip(withMidiCc("Optional OPNB-family clock override. Zero uses the documented 8.00 MHz default.", chipper::parameters::id::clockHz));
        clockSlider.updateText();
        updateOpnbAdpcmSampleControls();
    }
    else
