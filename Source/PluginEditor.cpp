...
void ChipperAudioProcessorEditor::resized()
{
    ...
    const auto performanceStripHeight = sidLayout ? 260 : (paulaLayout ? 220 : (spc700Layout ? 176 : (sampleLayout ? 200 : (nesLayout ? 318 : (wavetableLayout ? 260 : 300)))));
    ...
    else if (spc700Layout)
    {
        ...
        const auto sourceRowHeight = std::clamp(desiredSourceHeight, std::min(300, maximumSourceRowHeight), std::min(360, maximumSourceRowHeight));
    ...
    {
        const auto middleRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getHeight()) * 0.09)), 76, 92);
        const auto minimumSourceRowHeight = 350;
        const auto maximumSampleRowHeight = std::max(230, modules.getHeight() - middleRowHeight - minimumSourceRowHeight - (gap * 2));
        const auto desiredSampleRowHeight = static_cast<int>(std::round(static_cast<double>(modules.getHeight()) * 0.36));
        const auto sampleRowHeight = std::clamp(desiredSampleRowHeight, std::min(280, maximumSampleRowHeight), std::min(330, maximumSampleRowHeight));
    ...
    else if (displayedMode == chipper::ChipMode::spc700 || displayedMode == chipper::ChipMode::paula)
    {
        ...
        if (displayedMode == chipper::ChipMode::spc700)
        {
            auto playbackLifeRow = controlColumn.removeFromTop(std::min(30, controlColumn.getHeight()));
            dmgStereoRouteLabel.setBounds({});
            spc700LoopModeButton.setBounds(playbackLifeRow.removeFromLeft(std::min(128, playbackLifeRow.getWidth())).reduced(0, 1));
            dmgStereoRouteBox.setBounds({});
            for (auto& button : dmgStereoRouteButtons)
                button.setBounds({});
            controlColumn.removeFromTop(std::min(twoColumnSampleBank ? 4 : 5, controlColumn.getHeight()));
