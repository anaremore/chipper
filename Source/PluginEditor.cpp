      ...
      audioProcessor(processor)
{
    ...
    dmcLoopButton.setTooltip(withMidiCcForRole("RP2A03 $4010 DMC loop bit. One Shot steps the selected sample once, stops at the final bit, and holds the final DMC DAC level until retriggered. Loop repeats from byte 0 at the sample end.", chipper::ChipParameterRole::nesDmcLoop));
...
void ChipperAudioProcessorEditor::resized()
{
    ...
    const auto performanceStripHeight = sidLayout ? 260 : (nesLayout ? 318 : (spc700Layout ? 176 : (paulaLayout ? 132 : (wavetableLayout ? 204 : 300))));
    const auto maxModulesHeight = sidLayout ? 620 : (nesLayout ? 650 : (spc700Layout ? 1184 : (paulaLayout ? 1160 : (wavetableLayout ? 528 : 492))));
    ...
    {
        const auto topRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getHeight()) * 0.19)), 176, 210);
        constexpr auto spc700SourceRows = 2;
        constexpr auto sourcePanelChrome = 68;
        constexpr auto targetSpc700CardHeight = 172;
        const auto targetSourceHeight = sourcePanelChrome + (targetSpc700CardHeight * spc700SourceRows) + gap;
        const auto minimumSampleRowHeight = 360;
    ...
    else if (wavetableLayout)
    {
        ...
        constexpr auto targetCardHeight = 116;
        const auto desiredSourceHeight = sourceHeaderReserve + (targetCardHeight * sourceRows) + (gap * (sourceRows - 1));
        const auto sourceMaximumHeight = std::min(304, std::max(252, availableHeight - minimumEnvelopeHeight - minimumOutputHeight - reservedGap));
    ...
    {
        const auto middleRowHeight = std::clamp(static_cast<int>(std::round(static_cast<double>(modules.getHeight()) * 0.05)), 58, 68);
        const auto sampleMinimumHeight = std::min(486, std::max(420, modules.getHeight() / 3));
    ...
    const auto sourceCardHeight = useWavetableVoiceGrid
        ...
        : (usePaulaVoiceGrid
               ? std::min(rawSourceCardHeight, 172)
               : (useSpc700VoiceGrid ? std::min(rawSourceCardHeight, 172) : rawSourceCardHeight));
    for (size_t i = 0; i < sourceChannelBounds.size(); ++i)
    {
        ...
        const auto buttonHeight = useSpc700VoiceGrid ? 22 : (isPaulaSourceCard ? 22 : (isSidSourceCard ? 17 : (isDenseSampleCard ? 20 : 18)));
        ...
        const auto previewHeight = isWavetableSourceCard
            ? std::clamp(sourceCard.getHeight() / 4, 20, 26)
        ...
        else if (isPaulaSourceCard && i < hucVoiceWaveBoxes.size())
        {
            if (i < paulaVoiceSampleBoxes.size())
            {
                auto levelArea = sourceCard.removeFromBottom(std::min(46, sourceCard.getHeight()));
    ...
    else if (displayedMode == chipper::ChipMode::spc700 || displayedMode == chipper::ChipMode::paula)
    {
        ...
        const auto minimumTwoColumnSampleBankHeight = displayedMode == chipper::ChipMode::spc700 ? 400 : 360;
                                                     ...
                                                     juce::Rectangle<int> bounds)
{
    ...
    valueLabel.setJustificationType(juce::Justification::centredLeft);
    groupLabel.setBounds(bounds.removeFromTop(std::min(groupHeight, bounds.getHeight())));
...
void ChipperAudioProcessorEditor::updateDmcSampleControls()
{
    ...
    {
        auto runState = juce::String(playbackInfo.loopEnabled ? "Looping sample" : "One-shot; DAC holds");
