...
bool expectEnvelopeModels()
{
                             ...
                             chipper::ChipMode::saa1099 })
    {
        ok &= expect(chipper::envelopeModelFor(mode) == chipper::EnvelopeModel::nativeNonAdsr,
                     "NES, DMG, YM/AY, and SAA1099 should be classified as native non-ADSR envelope chips");
    }
...
bool expectFmRegisterHelpers()
{
    ...
    auto opn2SustainReleaseOverride = opn2Lead;
    opn2SustainReleaseOverride.fmOperatorSustainRates = { 1, 0, 0, 32 };
    opn2SustainReleaseOverride.fmOperatorReleaseRates = { 1, 0, 0, 16 };
    const auto opn2SustainReleaseOp1 = chipper::ym2612EnvelopeRegistersForPatch(opn2SustainReleaseOverride, 0);
    const auto opn2SustainReleaseOp2 = chipper::ym2612EnvelopeRegistersForPatch(opn2SustainReleaseOverride, 1);
    const auto opn2SustainReleaseOp4 = chipper::ym2612EnvelopeRegistersForPatch(opn2SustainReleaseOverride, 3);
    ok &= expect(opn2SustainReleaseOp1.sustainRate == 0u, "YM2612 operator 1 sustain override should write native D2R 0");
    ok &= expect((opn2SustainReleaseOp1.sustainRelease & 0x0fu) == 0u, "YM2612 operator 1 release override should write native RR 0");
    ok &= expect(opn2SustainReleaseOp2.sustainRate == chipper::ym2612EnvelopeRegistersForPatch(opn2Lead, 1).sustainRate,
                 "YM2612 operator 2 sustain should follow Envelope Shape when not overridden");
    ok &= expect((opn2SustainReleaseOp2.sustainRelease & 0x0fu) == (chipper::ym2612EnvelopeRegistersForPatch(opn2Lead, 1).sustainRelease & 0x0fu),
                 "YM2612 operator 2 release should follow Envelope Shape when not overridden");
    ok &= expect(opn2SustainReleaseOp4.sustainRate == 31u, "YM2612 operator 4 sustain override should write native D2R 31");
    ok &= expect((opn2SustainReleaseOp4.sustainRelease & 0x0fu) == 15u, "YM2612 operator 4 release override should write native RR 15");
    ...
    const auto opnLead = chipper::makePatchConfig(chipper::ChipMode::ym2203,
                                                  chipper::MacroKind::lead,
                                                  0.1f,
                                                  0.6f,
                                                  0.5f,
    ...
    if (const auto* preset = chipper::presetById("opn-crystal-lead"))
    {
        const auto patch = chipper::patchConfigForPreset(*preset);
        ok &= expect(chipper::fmOperatorLevelsForPreset(*preset) == std::array<float, 4> { 0.42f, 0.82f, 0.44f, 0.78f },
                     "OPN Crystal Lead should carry curated operator-level trims");
        ok &= expect(patch.fmOperatorAttackRates == std::array<int, 4> { 32, 32, 31, 32 },
                     "OPN Crystal Lead should carry curated operator attack rates");
        ok &= expect(patch.fmOperatorDecayRates == std::array<int, 4> { 9, 17, 8, 12 },
                     "OPN Crystal Lead should carry curated operator decay rates");
        ok &= expect(patch.fmOperatorSustainRates == std::array<int, 4> { 5, 12, 4, 8 },
                     "OPN Crystal Lead should carry curated operator sustain rates");
        ok &= expect(patch.fmOperatorReleaseRates == std::array<int, 4> { 6, 8, 5, 7 },
                     "OPN Crystal Lead should carry curated operator release rates");
    }
    else
    {
        ok &= expect(false, "OPN Crystal Lead preset should exist");
    }
    ...
    auto opmSustainReleaseOverride = opmArp;
    opmSustainReleaseOverride.fmOperatorSustainRates = { 6, 0, 0, 32 };
    opmSustainReleaseOverride.fmOperatorReleaseRates = { 6, 0, 0, 16 };
    const auto opmSustainReleaseOp1 = chipper::ym2612EnvelopeRegistersForPatch(opmSustainReleaseOverride, 0);
    const auto opmSustainReleaseOp2 = chipper::ym2612EnvelopeRegistersForPatch(opmSustainReleaseOverride, 1);
    const auto opmSustainReleaseOp4 = chipper::ym2612EnvelopeRegistersForPatch(opmSustainReleaseOverride, 3);
    ok &= expect(opmSustainReleaseOp1.sustainRate == 5u, "YM2151 shared helper operator 1 sustain override should write native D2R 5");
    ok &= expect((opmSustainReleaseOp1.sustainRelease & 0x0fu) == 5u, "YM2151 shared helper operator 1 release override should write native RR 5");
    ok &= expect(opmSustainReleaseOp2.sustainRate == chipper::ym2612EnvelopeRegistersForPatch(opmArp, 1).sustainRate,
                 "YM2151 shared helper operator 2 sustain should follow Envelope Shape when not overridden");
    ok &= expect((opmSustainReleaseOp2.sustainRelease & 0x0fu) == (chipper::ym2612EnvelopeRegistersForPatch(opmArp, 1).sustainRelease & 0x0fu),
                 "YM2151 shared helper operator 2 release should follow Envelope Shape when not overridden");
    ok &= expect(opmSustainReleaseOp4.sustainRate == 31u, "YM2151 shared helper operator 4 sustain override should write native D2R 31");
    ok &= expect((opmSustainReleaseOp4.sustainRelease & 0x0fu) == 15u, "YM2151 shared helper operator 4 release override should write native RR 15");
...
int main()
{
    ...
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::nesVrc6).implemented, "NES + VRC6 descriptor should be implemented");
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::nesVrc6).supportsChipPoly, "NES + VRC6 should support Chip Poly across APU melodic and VRC6 lanes");
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::nesFds).implemented, "NES + FDS descriptor should be implemented");
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::nesFds).supportsChipPoly, "NES + FDS should support Chip Poly across APU melodic and FDS wave lanes");
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::nesSunsoft5b).implemented, "NES + Sunsoft 5B descriptor should be implemented");
    ...
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::saa1099).implemented, "SAA1099 descriptor should be partially implemented");
    ...
    ok &= expectSegmentedRegister(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::macroControl1, 4, "12.5%");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::pulse2Duty, 5, "Preset");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::snNoiseMode, 3, "Preset");
    ok &= expectSpec(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::source5Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "VRC6 Pulse 1");
    ok &= expectSpec(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::source6Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "VRC6 Pulse 2");
    ok &= expectSpec(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::source7Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "VRC6 Saw");
    ok &= expectSpec(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::source5Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "VRC6 Pulse 1 Level");
    ok &= expectSpec(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::source6Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "VRC6 Pulse 2 Level");
    ok &= expectSpec(chipper::ChipMode::nesVrc6, chipper::ChipParameterRole::source7Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "VRC6 Saw Level");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesFds, chipper::ChipParameterRole::macroControl1, 4, "12.5%");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesFds, chipper::ChipParameterRole::pulse2Duty, 5, "Preset");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesFds, chipper::ChipParameterRole::snNoiseMode, 3, "Preset");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesFds, chipper::ChipParameterRole::waveShape, 5, "Preset");
    ok &= expectSpec(chipper::ChipMode::nesFds, chipper::ChipParameterRole::source5Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "FDS Wave");
    ok &= expectSpec(chipper::ChipMode::nesFds, chipper::ChipParameterRole::source5Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "FDS Wave Level");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::macroControl1, 4, "12.5%");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::pulse2Duty, 5, "Preset");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::snNoiseMode, 3, "Preset");
    ok &= expectSpec(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::source5Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "5B Tone A");
    ok &= expectSpec(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::source6Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "5B Tone B");
    ok &= expectSpec(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::source7Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "5B Tone C");
    ok &= expectSpec(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::source5Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "5B Tone A Level");
    ok &= expectSpec(chipper::ChipMode::nesSunsoft5b, chipper::ChipParameterRole::source7Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "5B Tone C Level");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesMmc5, chipper::ChipParameterRole::macroControl1, 4, "12.5%");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesMmc5, chipper::ChipParameterRole::pulse2Duty, 5, "Preset");
    ok &= expectSegmentedRegister(chipper::ChipMode::nesMmc5, chipper::ChipParameterRole::snNoiseMode, 3, "Preset");
    ok &= expectSpec(chipper::ChipMode::nesMmc5, chipper::ChipParameterRole::source5Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "MMC5 Pulse 1");
    ok &= expectSpec(chipper::ChipMode::nesMmc5, chipper::ChipParameterRole::source6Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "MMC5 Pulse 2");
    ...
    ok &= expectSpec(chipper::ChipMode::dmg, chipper::ChipParameterRole::macroControl4, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "Pulse Initial Level");
    ...
    ok &= expectSpec(chipper::ChipMode::sn76489, chipper::ChipParameterRole::macroControl3, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "Preset Noise Bias");
    ok &= expectSpec(chipper::ChipMode::sn76489, chipper::ChipParameterRole::macroControl4, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "Native Noise Level");
    ...
    ok &= expectSpec(chipper::ChipMode::saa1099, chipper::ChipParameterRole::macroControl1, chipper::ParameterKind::macro, chipper::ControlSurface::slider, "Channel Spread");
    ok &= expectSpec(chipper::ChipMode::saa1099, chipper::ChipParameterRole::macroControl3, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "Noise Clock");
    ok &= expectSpec(chipper::ChipMode::saa1099, chipper::ChipParameterRole::macroControl4, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "Channel Level");
    ok &= expectSpec(chipper::ChipMode::saa1099, chipper::ChipParameterRole::source6Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "Channel 6");
    ok &= expectSpec(chipper::ChipMode::saa1099, chipper::ChipParameterRole::source6Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "Channel 6 Level");
    ok &= expectSegmentedRegister(chipper::ChipMode::saa1099, chipper::ChipParameterRole::ymEnvelopeShape, 5, "Preset");
    ok &= expectSegmentedRegister(chipper::ChipMode::saa1099, chipper::ChipParameterRole::snNoiseMode, 5, "Preset");
    ok &= expectSpec(chipper::ChipMode::pcSpeaker, chipper::ChipParameterRole::macroControl1, chipper::ParameterKind::macro, chipper::ControlSurface::slider, "Pulse Width");
    ok &= expectSpec(chipper::ChipMode::pcSpeaker, chipper::ChipParameterRole::macroControl3, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "Click Grit");
    ok &= expectSpec(chipper::ChipMode::pcSpeaker, chipper::ChipParameterRole::macroControl4, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "Speaker Level");
    ...
    ok &= expectPreset(chipper::ChipMode::nesVrc6, "vrc6-saw-bass");
    ok &= expectMacroLabel(chipper::ChipMode::nesFds, chipper::MacroKind::lead, "FDS Mod Lead");
    ok &= expectPreset(chipper::ChipMode::nesFds, "fds-wave-bass");
    ok &= expectPreset(chipper::ChipMode::nesFds, "fds-mod-lead");
    ok &= expectPreset(chipper::ChipMode::nesFds, "fds-liquid-arp");
    ok &= expectMacroLabel(chipper::ChipMode::nesSunsoft5b, chipper::MacroKind::lead, "5B Square Lead");
    ok &= expectPreset(chipper::ChipMode::nesSunsoft5b, "5b-psg-bass");
    ok &= expectPreset(chipper::ChipMode::nesSunsoft5b, "5b-square-lead");
    ok &= expectPreset(chipper::ChipMode::nesSunsoft5b, "5b-three-voice-arp");
    ok &= expectMacroLabel(chipper::ChipMode::nesMmc5, chipper::MacroKind::lead, "MMC5 Twin Pulse Lead");
    ...
    ok &= expectMacroLabel(chipper::ChipMode::saa1099, chipper::MacroKind::lead, "SAA Stereo Lead");
    ok &= expectPreset(chipper::ChipMode::saa1099, "saa-square-bass");
    ok &= expectPreset(chipper::ChipMode::saa1099, "saa-stereo-lead");
    ok &= expectPreset(chipper::ChipMode::saa1099, "saa-six-voice-arp");
    ok &= expectMacroLabel(chipper::ChipMode::pcSpeaker, chipper::MacroKind::drum, "PC Click Drum");
    ...
    ok &= expectChoiceRegister(chipper::ChipMode::ym2612, chipper::ChipParameterRole::macroControl2, chipper::ControlSurface::menu, 8, "FB 0");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Sustain");
    ok &= expectSpec(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator4SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Sustain");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator1SustainRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Release");
    ok &= expectSpec(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator4ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Release");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ControlSurface::menu, 17, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2612, chipper::ChipParameterRole::fmOperator1ReleaseRate, "Operators");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2151, chipper::ChipParameterRole::macroControl2, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "Feedback");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2151, chipper::ChipParameterRole::macroControl2, chipper::ControlSurface::menu, 8, "FB 0");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Sustain");
    ok &= expectSpec(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator4SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Sustain");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator1SustainRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Release");
    ok &= expectSpec(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator4ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Release");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ControlSurface::menu, 17, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2151, chipper::ChipParameterRole::fmOperator1ReleaseRate, "Operators");
    ...
    ok &= expectMacroLabel(chipper::ChipMode::ym2203, chipper::MacroKind::bass, "OPN Feedback Bass");
    ok &= expectPreset(chipper::ChipMode::ym2203, "opn-feedback-bass");
    ok &= expectPreset(chipper::ChipMode::ym2203, "opn-metallic-lead");
    ok &= expectPreset(chipper::ChipMode::ym2203, "opn-three-voice-arp");
    ok &= expectPreset(chipper::ChipMode::ym2203, "opn-electric-keys");
    ok &= expectPreset(chipper::ChipMode::ym2203, "opn-fm-perc");
    ok &= expectPreset(chipper::ChipMode::ym2203, "opn-pitch-laser");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::waveShape, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "Algorithm");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2203, chipper::ChipParameterRole::waveShape, chipper::ControlSurface::menu, 9, "Preset");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1Multiplier, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Mult");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator4Multiplier, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Mult");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1Multiplier, chipper::ControlSurface::menu, 17, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1Multiplier, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1AttackRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Attack");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator4AttackRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Attack");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1AttackRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1AttackRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1DecayRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Decay");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator4DecayRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Decay");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1DecayRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1DecayRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Sustain");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator4SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Sustain");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1SustainRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Release");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator4ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Release");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ControlSurface::menu, 17, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2203, chipper::ChipParameterRole::fmOperator1ReleaseRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2203, chipper::ChipParameterRole::ymEnvelopeShape, chipper::ParameterKind::chipRegister, chipper::ControlSurface::segmentedChoice, "Envelope Shape");
    ok &= expectSpecGroup(chipper::ChipMode::ym2203, chipper::ChipParameterRole::ymEnvelopeShape, "Envelope");
    ...
    ok &= expectMacroLabel(chipper::ChipMode::ym2608, chipper::MacroKind::bass, "OPNA Feedback Bass");
    ok &= expectPreset(chipper::ChipMode::ym2608, "opna-feedback-bass");
    ok &= expectPreset(chipper::ChipMode::ym2608, "opna-metallic-lead");
    ok &= expectPreset(chipper::ChipMode::ym2608, "opna-nine-voice-arp");
    ok &= expectPreset(chipper::ChipMode::ym2608, "opna-electric-keys");
    ok &= expectPreset(chipper::ChipMode::ym2608, "opna-ssg-perc-drum");
    ok &= expectPreset(chipper::ChipMode::ym2608, "opna-pitch-laser");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::waveShape, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "Algorithm");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2608, chipper::ChipParameterRole::waveShape, chipper::ControlSurface::menu, 9, "Preset");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::envelopeDecay, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "SSG Envelope Period");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1Multiplier, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Mult");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator4Multiplier, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Mult");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1Multiplier, chipper::ControlSurface::menu, 17, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1Multiplier, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1AttackRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Attack");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator4AttackRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Attack");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1AttackRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1AttackRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1DecayRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Decay");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator4DecayRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Decay");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1DecayRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1DecayRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Sustain");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator4SustainRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Sustain");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1SustainRate, chipper::ControlSurface::menu, 33, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1SustainRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP1 Release");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator4ReleaseRate, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "OP4 Release");
    ok &= expectChoiceRegister(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1ReleaseRate, chipper::ControlSurface::menu, 17, "Follow");
    ok &= expectSpecGroup(chipper::ChipMode::ym2608, chipper::ChipParameterRole::fmOperator1ReleaseRate, "Operators");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::dmgStereoRoute, chipper::ParameterKind::chipRegister, chipper::ControlSurface::segmentedChoice, "Pan");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::ymEnvelopeShape, chipper::ParameterKind::chipRegister, chipper::ControlSurface::segmentedChoice, "Envelope Shape");
    ok &= expectSpecGroup(chipper::ChipMode::ym2608, chipper::ChipParameterRole::ymEnvelopeShape, "Envelope");
    ok &= expectSegmentedRegister(chipper::ChipMode::ym2608, chipper::ChipParameterRole::ymEnvelopeShape, 5, "Preset");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::source6Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "OPNA FM 6");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::source7Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "OPNA SSG A");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::source8Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "OPNA SSG B");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::source9Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "OPNA SSG C");
    ok &= expectSpec(chipper::ChipMode::ym2608, chipper::ChipParameterRole::source6Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "OPNA FM 6 Level");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2610, chipper::ChipParameterRole::envelopeDecay, chipper::ParameterKind::chipRegister, chipper::ControlSurface::slider, "SSG Envelope Period");
    ...
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::ym2610b).implemented, "YM2610B / OPNB2 descriptor should be partially implemented");
    ok &= expect(chipper::descriptorFor(chipper::ChipMode::ym2610b).supportsChipPoly, "YM2610B / OPNB2 should support Chip Poly across six FM and three SSG lanes");
    ok &= expectMacroLabel(chipper::ChipMode::ym2610b, chipper::MacroKind::bass, "OPNB2 Feedback Bass");
    ok &= expectPreset(chipper::ChipMode::ym2610b, "opnb2-six-fm-bass");
    ok &= expectPreset(chipper::ChipMode::ym2610b, "opnb2-metallic-lead");
    ok &= expectPreset(chipper::ChipMode::ym2610b, "opnb2-nine-voice-arp");
    ok &= expectPreset(chipper::ChipMode::ym2610b, "opnb2-electric-keys");
    ok &= expectPreset(chipper::ChipMode::ym2610b, "opnb2-adpcm-perc-drum");
    ok &= expectPreset(chipper::ChipMode::ym2610b, "opnb2-pitch-laser");
    ok &= expectSpec(chipper::ChipMode::ym2610b, chipper::ChipParameterRole::waveShape, chipper::ParameterKind::chipRegister, chipper::ControlSurface::menu, "Algorithm");
    ...
    ok &= expectSpec(chipper::ChipMode::ym2610b, chipper::ChipParameterRole::source8Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "OPNB2 SSG B");
    ok &= expectSpec(chipper::ChipMode::ym2610b, chipper::ChipParameterRole::source9Enabled, chipper::ParameterKind::booleanToggle, chipper::ControlSurface::sourceCards, "OPNB2 SSG C");
    ok &= expectSpec(chipper::ChipMode::ym2610b, chipper::ChipParameterRole::source6Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "OPNB2 FM 6 Level");
    ok &= expectSpec(chipper::ChipMode::ym2610b, chipper::ChipParameterRole::source7Level, chipper::ParameterKind::continuous, chipper::ControlSurface::slider, "OPNB2 SSG A Level");
    ...
    ok &= expectPresetBrowserCatalog(chipper::ChipMode::nesVrc6, "vrc6-pulse-lead");
    ...
    ok &= expectSourceLaneCounts(chipper::ChipMode::ym2610b, 9u, 9u);
    ...
    ok &= expect(chipper::chipHasParameterSurface(chipper::ChipMode::saa1099,
                                                  chipper::ChipParameterRole::source6Enabled,
                                                  chipper::ControlSurface::sourceCards),
                 "SAA1099 channel 6 should expose source cards");
    ok &= expect(chipper::chipHasParameterSurface(chipper::ChipMode::pcSpeaker,
                                                  chipper::ChipParameterRole::source1Enabled,
                                                  chipper::ControlSurface::sourceCards),
